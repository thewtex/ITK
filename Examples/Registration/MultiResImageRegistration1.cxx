...
public:
  ...
  Execute(itk::Object * object, const itk::EventObject & event)
  {
    ...
    // (representing
    // the convergence criterion) to large values.  At each subsequent
    // resolution
    // level, we will reduce the minimum step length by a factor of 10 in order
    // to
    // allow the optimizer to focus on progressively smaller regions. The
    // maximum
    // step length is set up to the current step length. In this way, when the
    // optimizer is reinitialized at the beginning of the registration process
    // for
