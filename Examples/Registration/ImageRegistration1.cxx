...
public:
  ...
  typedef itk::RegularStepGradientDescentOptimizerv4         OptimizerType;
...
int main( int argc, char *argv[] )
{
  ...
  //  derivative. The initial length of the step is defined by the user manually
  //  via SetLearningRate().
  //  In addition to manual definition, in other optimizers of ITKv4 framework,
  ...
  //  In regular step gradient descent optimizer, each time the direction of the
  //  derivative abruptly changes, the optimizer assumes that a local extrema
  //  has been passed and reacts by reducing the step length by a half. The reducing
  //  factor can be set to a different value by SetRelaxationFactor().
  //  After several reductions of the step length, the optimizer may be moving
  //  in a very restricted area of the transform parameter space. The user can define
  //  how small the step length should be to consider convergence to have been reached.
  //  This is equivalent to defining the precision with which the final transform should
  //  be known. User can also set some other stop criteria manually like maximum number
  //  of iterations.
  //
  ...
  //
  //  Also note that unlike the previous version of \doxygen{GradientDescentOptimizer},
  ...
  //
  //  The initial step length is defined with the method \code{SetLearningRate()},
  //  while the tolerance for convergence is defined with the method
  //  \code{SetMinimumStepLength()}, while the tolerance for convergence is
