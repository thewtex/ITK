...
//
// The region over which metric evaluation is performed is called virtual domain.
// This space defines the resolution at which the evaluation is performed, as well as
// the physical coordinate system.
// The virtual reference space is taken from the virtual image buffered region, and
// input images are mapped to this reference sapce domain using fixed and moving transforms.
// User can define the virtual domain by calling either SetVirtualDomain or
// SetVirtualDomainFromImage. By default, the virtual space is set as fixed image domain.
//
// Also, instead of using the whole virtual domain (usually fixed image domain),
// we can use a spatial sample set by supplying an arbitrary point list over which to
// evaluate the metric. The point list is expected to be in the fixed image domain.
// Internally, the points are transformed into the virtual domain as needed. User can
...
// process. The use of a single sample set results in a much smoother cost function
// and hence allows the use of more intelligent optimizers. In this example, we will
