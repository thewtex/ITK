...
int main( int argc, char *argv[] )
{
  ...
  //  to initialize the registration configuration. That approach was good to get
  //  an intuition of the registration method specially when we aim to run
  ...
  //  Inside the registration filter of the first stage, this initial moving transform
  ...
  //  the current stage when the optimization process is converged. This imply the
  //  fact that the output of this stage does not include the initialization
  //  parameters, so we need to concatenate the output and the initialization transforms
  //  into a composite transform to be considered as the final transform of the first registration
  //  stage.
  //
  //  $T_{1}$(x) = $\Gamma_{mi}$($\Gamma_{stage_1}$(x) )
  //
  //  Then, the above transform will be used as the initial moving transform for the
  ...
  //  are concatenated into a final composite transform that can be considered as the final
  //  output of the whole registration process.
  //
  //  $I'_{m}$(x) = $I_{m}$($\Gamma_{mi}$($\Gamma_{stage_1}$($\Gamma_{stage_2}$($\Gamma_{stage_3}$(x) ) ) ) )
  //
  //  The above approach is specially useful if each stage is characterized by a different type of transform.
  //  For example we run a rigid registration process that is proceeded by an affine registration
  //  and is completed by a BSpline registration at the end.
  //
  //
