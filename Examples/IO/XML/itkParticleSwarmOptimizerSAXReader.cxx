...
ParticleSwarmOptimizerSAXReader::StartElement( const char* name, const char** atts )
{
  ...
  else if ( itksys::SystemTools::Strucmp( name,
                                          "ParametersConvergenceTolerance" ) == 0 && this->ContextIs( "/optimizer" ) )
    {
...
ParticleSwarmOptimizerSAXReader::EndElement( const char* name )
{
  if ( itksys::SystemTools::Strucmp( name, "optimizer" ) == 0 && this->ContextIs( "/optimizer" ) )
    ...
    // nothing to do as it does not have children
                                          ...
                                          "ParametersConvergenceTolerance" ) == 0 &&
...
void
ParticleSwarmOptimizerSAXReader::ProcessOptimizerAttributes( const char** atts, ParticleSwarmOptimizer* opt )
{
...
void
ParticleSwarmOptimizerSAXReader::ProcessBoundAttributes( const char** atts, std::vector<double>& bound )
{
  // go over all the attribute-value pairs
  for ( size_t i = 0; atts[i] != 0; i += 2 )
    {
