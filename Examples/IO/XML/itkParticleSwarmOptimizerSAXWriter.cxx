...
int ParticleSwarmOptimizerSAXWriter::WriteFile()
{
  ...
  {
    ExceptionObject e( __FILE__, __LINE__ );
    std::string message = "Object to be written is null!\n";
    e.SetDescription( message.c_str() );
    throw e;
  }
  ...
  ofs << " value=\"";
  for ( unsigned int i = 0; i < bounds.size(); i++ ) ofs << " " << bounds[i].first;
  ofs << "\"";
  ...
  ofs << " value=\"";
  for ( unsigned int i = 0; i < bounds.size(); i++ ) ofs << " " << bounds[i].second;
  ofs << "\"";
  ...
  for ( unsigned int i = 0; i < ptols.GetSize(); i++ ) ofs << " " << ptols[i];
