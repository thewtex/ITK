...
protected:
  ...
  static void ProcessOptimizerAttributes( const char** atts, ParticleSwarmOptimizer* opt );

  /** Process tag 'bound' attributes. */
  static void ProcessBoundAttributes( const char** atts, std::vector<double>& bound );

  /** Search for and return a particular attribute from the attribute list. */
  static const char* GetAttribute( const char** atts, const char* key );
