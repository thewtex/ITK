...
${UNCRUSTIFYBIN} -c $HOME/src/brains2/utils/uncrustify_itk.cfg -l CPP -f ${file} | sed 's/  *##  */##/g' | sed 's/#  */#/g' > ${file}_uncrustify
...
$HOME/src/KWStyle-build/bin/KWStyle -xml $HOME/Dashboards/ITK_TESTS/ITK-gcc-4.2/Utilities/KWStyle/ITK.kws.xml -v -o $HOME/Dashboards/ITK_TESTS/ITK/Utilities/KWStyle/ITKOverwrite.txt -gcc $file > ${KWERROS}
...
if [ -f ${KWERROS} ]; then
  ...
  if [ ${skipuncrustify} == "y" ]; then
    ${UNCRUSTIFYBIN} -c $HOME/src/brains2/utils/uncrustify_itk.cfg -l CPP -f ${file} -o ${KWERROS}_skip_uncrustify
  fi
