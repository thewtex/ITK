...
# These need a #if defined wrapper because they are only available on Windows.
WINDOWS_ONLY = set(('itkWin32OutputWindow.h',
    'itkWindows.h',))
...
def main():
    ...
    while(added_header_idx < len(h_files)):
        test_num += 1
        test_source_file = os.path.join(module_binary_path, 'test',
            str(module_name) + 'HeaderTest' + str(test_num) + '.cxx')
        with open(test_source_file, 'w') as test_src:
            ...
            for i in range(added_header_idx, max_idx):
                ...
                if os.path.exists(os.path.join(module_source_path, 'include',
                    hxx_file)):
                    to_include = hxx_file
                else:
                    to_include = h_files[i]

                if h_files[i] in WINDOWS_ONLY:
                    test_src.write('#if defined(_MSC_VER)\n')
                    test_src.write('  #include "')
                    test_src.write(to_include)
                    test_src.write('"\n')
                    test_src.write('#endif\n')
                else:
                    test_src.write('#include "')
                    test_src.write(to_include)
                    test_src.write('"\n')
