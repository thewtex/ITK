...
# "update-from-upstream.sh", which defines the following shell variables:
#
...
#      with "&&" and the like. The command typically removes extraneous files
...
#        e.g. "src/mylib/"
#
# The script, "update-from-upstream.sh", must be located in the associated
...
cd "$toplevel_path" && git archive --prefix="$snapshot_temp_dir/" $upstream_new_sha | tar x
...
\$ git archive --prefix=$snapshot_branch_name/ $upstream_new_sha_short | tar x
