...
::GradientDescentObjectOptimizer()
{
  ...
  // m_MaximumStepSizeInPhysicalSpaceUnits is used for automatic learning
  // rate estimation. it may be initialized either by calling
  // SetMaximumStepSizeInPhysicalSpaceUnits manually or by using m_ScalesEstimator
  // automatically. and the former has higher priority than the latter.
  this->m_MaximumStepSizeInPhysicalSpaceUnits = NumericTraits<InternalComputationValueType>::Zero;
}
...
::StartOptimization()
{
  ...
  if ( this->m_ScalesEstimator.IsNotNull() )
    ...
    if ( this->m_MaximumStepSizeInPhysicalSpaceUnits <=
      NumericTraits<InternalComputationValueType>::epsilon())
      {
      this->m_MaximumStepSizeInPhysicalSpaceUnits = this->m_ScalesEstimator->EstimateMaximumStepSize();
      }
    }
...
::EstimateLearningRate()
{
  if (this->m_ScalesEstimator.IsNotNull())
    ...
    if (stepScale <= NumericTraits<InternalComputationValueType>::epsilon())
      ...
      {
      this->m_LearningRate = this->m_MaximumStepSizeInPhysicalSpaceUnits / stepScale;
      }
