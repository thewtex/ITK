...
::QuasiNewtonObjectOptimizer()
{
  ...
  this->m_TrustedNewtonStepScale = 3.0;
...
::ModifyNewtonStep(IndexValueType loc)
{
  if (!this->m_Metric->HasLocalSupport())
    ...
    if (this->m_ScalesEstimator.IsNotNull())
      ...
      if (stepScale <= NumericTraits<InternalComputationValueType>::epsilon())
        ...
        {
        this->m_LearningRate = this->m_TrustedNewtonStepScale / stepScale;
  ...
  else if (this->m_NewtonStepValidFlags[loc])
    {
    const SizeValueType numLocalPara = this->m_Metric->GetNumberOfLocalParameters();
    const IndexValueType offset = loc * numLocalPara;

    InternalComputationValueType stepScale = NumericTraits<InternalComputationValueType>::Zero;
    for (SizeValueType p=0; p<numLocalPara; p++)
      {
      stepScale += this->m_NewtonStep[offset+p] * this->m_NewtonStep[offset+p];
      }
    stepScale = vcl_sqrt(stepScale);

    InternalComputationValueType learningRate;
    if (stepScale <= NumericTraits<InternalComputationValueType>::epsilon())
      {
      learningRate = NumericTraits<InternalComputationValueType>::One;
      }
    else
      {
      learningRate = this->m_TrustedNewtonStepScale / stepScale;
      if (learningRate > NumericTraits<InternalComputationValueType>::One)
        {
        // learning rate is at most 1 for a newton step
        learningRate = NumericTraits<InternalComputationValueType>::One;
        }
    ...
    for (SizeValueType p=0; p<numLocalPara; p++)
      {
      this->m_NewtonStep[offset+p] *= learningRate;
      }
    }
}
...
::EstimateNewtonStepOverSubRange( const IndexRangeType& subrange )
{
  ...
  for (IndexValueType loc = low; loc <= high; loc++)
    ...
    if (this->GetCurrentIteration() == 0)
      ...
      {
      this->m_NewtonStepValidFlags[loc] = this->ComputeHessianAndStep(loc);

      if (this->m_NewtonStepValidFlags[loc])
        {
        /**
         * If a Newton step is on the opposite direction of a gradient step,
         * we will use the gradient step.
         */
        InternalComputationValueType gradNewtonProduct = 0;
        for (SizeValueType p=0; p<numLocalPara; p++)
          {
          gradNewtonProduct += this->m_Gradient[offset+p] * this->m_NewtonStep[offset+p];
          }

        if ( gradNewtonProduct < 0 )
          {
          this->m_NewtonStepValidFlags[loc] = false;
          } // if ( gradNewtonProduct < 0 )
        } // if (this->m_NewtonStepIsValid)
      }
...
::ComputeHessianAndStep(IndexValueType loc)
{
  if (this->GetCurrentIteration() == 0)
    {
    //this->m_NewtonStepWarning = "Please use the gradient step for the first iteration.";
    //itkExceptionMacro(<< this->m_NewtonStepWarning);
    return false;
    }
  ...
  if (vcl_abs(dot_dg_dx) <= NumericTraits<InternalComputationValueType>::epsilon()
    ...
    {
    //this->m_NewtonStepWarning = "A denominator is too small in Hessian estimation.";
    //itkExceptionMacro(<< this->m_NewtonStepWarning);
    return false;
    }
  ...
  for (SizeValueType p=0; p<numLocalPara; p++)
    ...
    if (newHessian[p][p] < 0)
      {
      isConvex = false;
      //this->m_NewtonStepWarning = "The Hessian matrix is not convex for some dimensions.";
      //itkExceptionMacro(<< this->m_NewtonStepWarning);
      return false;
      }
    }
  ...
  if ( vcl_abs(vnl_determinant(newHessian)) <= threshold )
    {
    //this->m_NewtonStepWarning = "The Hessian matrix is almost singular.";
    //itkExceptionMacro(<< this->m_NewtonStepWarning);
    return false;
    }
  else
