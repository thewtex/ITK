...
#include "itkCommandQuasiNewtonUpdate.h"
                                                          ...
                                                          bool usePhysicalSpaceForShift = true)
{
  ...
  typedef itk::CommandQuasiNewtonUpdate< OptimizerType, itk::OptimizerParameterScalesEstimator >
    CommandScalesType;
  typename CommandScalesType::Pointer scalesCommand = CommandScalesType::New();
  scalesCommand->SetScalesEstimator( scalesEstimator.GetPointer() );
  scalesCommand->SetStepSize( shiftOfStep );
  scalesCommand->SetNewtonStepSize( shiftOfStep * 2);
  scalesCommand->SetOptimizer( optimizer.GetPointer() );
...
int itkQuasiNewtonObjectOptimizerTest(int argc, char ** const argv)
{
  if( argc > 3 )
    ...
    std::cerr << " [numberOfIterations=100 shiftOfStep=0.5] ";
    std::cerr << std::endl;
    return EXIT_FAILURE;
    }
  ...
  int ret1 = EXIT_SUCCESS, ret2 = EXIT_SUCCESS, ret3 = EXIT_SUCCESS, ret4 = EXIT_SUCCESS;

  std::cout << std::endl << "Optimizing affine transform with shift scales" << std::endl;
  typedef itk::AffineTransform<double, Dimension> AffineTransformType;
  ret1 = itkQuasiNewtonObjectOptimizerTestTemplated<AffineTransformType>(numberOfIterations, shiftOfStep, "shift");

  std::cout << std::endl<< "Optimizing affine transform with Jacobian scales" << std::endl;
  typedef itk::AffineTransform<double, Dimension> AffineTransformType;
  ret2 = itkQuasiNewtonObjectOptimizerTestTemplated<AffineTransformType>(numberOfIterations, shiftOfStep, "jacobian");
  ...
  ret3 = itkQuasiNewtonObjectOptimizerTestTemplated<TranslationTransformType>(numberOfIterations, shiftOfStep, "shift", usePhysicalSpaceForShift);

  std::cout << std::endl << "Optimizing translation transform with Jacobian scales" << std::endl;
  typedef itk::TranslationTransform<double, Dimension> TranslationTransformType;
  ret4 = itkQuasiNewtonObjectOptimizerTestTemplated<TranslationTransformType>(numberOfIterations, shiftOfStep, "jacobian");
