...
int itkRegistrationParameterScaleEstimatorTest(int , char* [])
{
  ...
  // Metric begins
  typedef RegistrationParameterScalesEstimatorTestMetric<FixedImageType, MovingImageType>   MetricType;
  ...
  typedef RegistrationParameterScaleEstimatorTest< MetricType > RegistrationParameterScaleEstimatorTestType;
  RegistrationParameterScaleEstimatorTestType::Pointer jacobianScaleEstimator = RegistrationParameterScaleEstimatorTestType::New();

  jacobianScaleEstimator->SetMetric(metric);
  jacobianScaleEstimator->SetTransformForward(true);
  jacobianScaleEstimator->SetSamplingStrategy(RegistrationParameterScaleEstimatorTestType::SamplingWithCorners);
  jacobianScaleEstimator->Print( std::cout );

  RegistrationParameterScaleEstimatorTestType::ScalesType jacobianScales(movingTransform->GetNumberOfParameters());
  ...
  RegistrationParameterScaleEstimatorTestType::ScalesType theoreticalJacobianScales(movingTransform->GetNumberOfParameters());
  ...
  for (itk::SizeValueType p = 0; p < jacobianScales.GetSize(); p++)
    {
    if (vcl_abs( (jacobianScales[p] - theoreticalJacobianScales[p])/theoreticalJacobianScales[p] ) > 0.5 )
      {
      randomPass = false;
      break;
      }
