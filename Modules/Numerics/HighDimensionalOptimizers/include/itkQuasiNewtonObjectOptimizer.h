...
public:
  ...
  itkSetMacro(MaximumNewtonStepSizeInPhysicalSpaceUnits, InternalComputationValueType);
...
protected:
  ...
  InternalComputationValueType  m_MaximumNewtonStepSizeInPhysicalSpaceUnits;
  ...
  /** Reset Hessian to Identity and m_NewtonStep to zeros. */
  virtual void ResetNewtonStep(IndexValueType location);

  /** Modify the Newton step with estimated learning rate(s). */
  virtual void ModifyNewtonStep();

  /**
   * Combine a gradient step with a Newton step over a given index range.
   * The Newton step will be used when it is valid. Otherwise the gradient
   * step is used.
   */
