...
::SampleImageDomainWithCorners()
{
  ...
  VirtualRegionType region = image->GetBufferedRegion();
...
::SampleImageDomainRandomly()
{
  ...
  if (m_NumberOfRandomSamples == 0)
    {
    const SizeValueType total = image->GetBufferedRegion().GetNumberOfPixels();
  ...
  RandomIterator randIter( image, image->GetBufferedRegion() );
...
::SampleImageDomainFully()
{
  ...
  RegionIterator regionIter( image, image->GetBufferedRegion() );
