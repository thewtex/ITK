  ...
  unsigned int numberOfNeighborsRequested, InstanceIdentifierVectorType &result ) const
{
  ...
  this->m_NearestNeighbors.resize( numberOfNeighborsRequested );
  ...
  this->m_Neighbors = this->m_NearestNeighbors.GetNeighbors();
  result = this->m_Neighbors;
}
  ...
  MeasurementVectorType &upperBound ) const
{
  ...
  if( node->IsTerminal() )
    ...
    for(  i = 0; i < node->Size(); ++i )
      ...
      if( tempDistance < this->m_NearestNeighbors.GetLargestDistance() )
        {
        this->m_NearestNeighbors.ReplaceFarthestNeighbor( tempId, tempDistance );
        }
      }

    if( this->BallWithinBounds( query, lowerBound, upperBound,
      this->m_NearestNeighbors.GetLargestDistance() ) )
      {
      return 1;
      }
  ...
  if( query[partitionDimension] <= partitionValue )
    ...
    tempValue = lowerBound[partitionDimension];
    lowerBound[partitionDimension] = partitionValue;
    if( NearestNeighborSearchLoop( node->Right(), query, lowerBound,
      upperBound ) )
      {
      return 1;
      }
  ...
  InstanceIdentifierVectorType & result ) const
{
  ...
  this->SearchLoop( this->m_Root, query, radius, lowerBound, upperBound );
  result = this->m_Neighbors;
}
  ...
  if( node->IsTerminal() )
    ...
    for( unsigned int i = 0; i < node->Size(); ++i )
      ...
      if( tempDistance <= radius )
        {
        this->m_Neighbors.push_back(tempId);
        }
      }
