...
//    typedef float              PCAResultsType;
  ...
  if (pcaCalc->GetProjection().rows() != fieldSetCount)
    {
    std::cout << "Projection Results Failed Dimension check:" << std::endl;
    std::cout << "Expected:  " << fieldSetCount << std::endl
              << " rows. Got:  "
              << pcaCalc->GetProjection().rows()
              << std::endl;
    return EXIT_FAILURE;
    }

  if (pcaCalc->GetProjection().cols() != pcaCount)
    {
    std::cout << "Projection Results Failed Dimension check:" << std::endl;
    std::cout << "Expected:  " << pcaCount << std::endl
              << " columns.  Got:  "
              << pcaCalc->GetProjection().cols()
              << std::endl;
    return EXIT_FAILURE;
    }
