...
int itkSampleClassifierWithLabelsFilterTest( int, char* [] )
{
  ...
  typedef itk::Statistics::SampleClassifierWithLabelsFilter<
    SampleType, ClassMaskSampleType > ClassifierType;

  ClassifierType::Pointer generator = ClassifierType::New();

  generator->SetInput( sample );
  generator->SetLabelInput ( mask );
  generator->SetNumberOfClasses( 2 );
  generator->Update();

  typedef ClassifierType::OutputType   MembershipSampleType;

  MembershipSampleType::ConstPointer membershipSample = generator->GetOutput();

  if (  membershipSample->GetMeasurementVectorSize() !=  3 )
    {
