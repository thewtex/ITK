 ...
 * learning rate at every iteration. This behavior can be changed via
 * SetDoEstimateLearningRateAtEveryIteration() and
 * SetDoEstimateLearningRateOnce(). For learning rate to be estimated only once,
 * the user must call SetDoEstimateLearningRateAtEveryIteration(false) and
 * SetDoEstimateLearningRateOnce(true). When enabled, the learning rate is
 * automatically restricted such that each iteration will change
 * voxels by less than m_MaximumStepSizeInPhysicalUnits in physical space.
 * The optimizer computes the learning rates as
 *      m_MaximumStepSizeInPhysicalUnits /
 *      m_ScalesEstimator->EstimateStepScale(scaledGradient).
 * m_MaximumStepSizeInPhysicalUnits defaults to the voxel spacing returned by
