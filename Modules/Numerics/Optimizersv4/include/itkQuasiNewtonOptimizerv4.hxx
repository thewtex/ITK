...
template<class InternalComputationType>
QuasiNewtonOptimizerTemplatev4<InternalComputationType>
::QuasiNewtonOptimizerTemplatev4()
{
  this->m_MaximumIterationsWithoutProgress = 30;
  this->m_LearningRate = NumericTraits<InternalComputationType>::One;
  ...
  this->m_MaximumNewtonStepSizeInPhysicalUnits = NumericTraits<InternalComputationType>::Zero;

  /** Threader for Quasi-Newton method */
  typename QuasiNewtonOptimizerv4EstimateNewtonStepThreaderTemplate<InternalComputationType>::Pointer estimateNewtonStepThreader = QuasiNewtonOptimizerv4EstimateNewtonStepThreaderTemplate<InternalComputationType>::New();
...
::StartOptimization( bool doOnlyInitialization )
{
  ...
  if ( this->m_ScalesEstimator.IsNotNull() )
    ...
    if ( this->m_MaximumNewtonStepSizeInPhysicalUnits <=
        NumericTraits<InternalComputationType>::epsilon())
      {
...
::CombineGradientNewtonStep()
{
  ...
  InternalComputationType ratio = NumericTraits<InternalComputationType>::One;
  if (validNewtonStepExists)
    {
    InternalComputationType gradStepScale
    = this->m_ScalesEstimator->EstimateStepScale(this->m_Gradient);
    InternalComputationType newtonStepScale
    = this->m_ScalesEstimator->EstimateStepScale(this->m_NewtonStep);
    if (gradStepScale > NumericTraits<InternalComputationType>::epsilon())
      {
...
::ModifyCombinedNewtonStep()
{
  ...
  InternalComputationType stepScale
  = this->m_ScalesEstimator->EstimateStepScale(this->m_NewtonStep);

  if (stepScale <= NumericTraits<InternalComputationType>::epsilon())
    {
    this->m_LearningRate = NumericTraits<InternalComputationType>::One;
    }
  else
    ...
    if (this->m_LearningRate > NumericTraits<InternalComputationType>::One)
      {
  ...
  if (vcl_abs(this->m_LearningRate - NumericTraits<InternalComputationType>::One)
      > 0.01)
    {
...
::ResetNewtonStep(IndexValueType loc)
{
  ...
  m_HessianArray[loc].Fill(NumericTraits<InternalComputationType>::Zero);

  for (unsigned int i=0; i<numLocalPara; i++)
    {
    m_HessianArray[loc][i][i] = NumericTraits<InternalComputationType>::One; //identity matrix
    }
  ...
  for (SizeValueType p=0; p<numLocalPara; p++)
    ...
    this->m_NewtonStep[offset+p] = NumericTraits<InternalComputationType>::Zero;
    }
}
...
::ComputeHessianAndStepWithBFGS(IndexValueType loc)
{
  ...
  InternalComputationType dot_dg_dx = inner_product(dg, dx);
  InternalComputationType dot_edg_dx = inner_product(edg, dx);

  if (vcl_abs(dot_dg_dx) <= NumericTraits<InternalComputationType>::epsilon()
      || vcl_abs(dot_edg_dx) <= NumericTraits<InternalComputationType>::epsilon())
    {
    return false;
    }

  vnl_matrix<InternalComputationType> plus  = outer_product(dg, dg) / dot_dg_dx;
  vnl_matrix<InternalComputationType> minus = outer_product(edg, edg) / dot_edg_dx;
  vnl_matrix<InternalComputationType> newHessian = this->m_HessianArray[loc] + plus - minus;
  ...
  InternalComputationType threshold
  = NumericTraits<InternalComputationType>::epsilon();
  ...
  if ( vcl_abs(vnl_determinant(newHessian)) <= threshold )
    ...
    {
    vnl_matrix<InternalComputationType> hessianInverse = vnl_matrix_inverse<InternalComputationType>(newHessian);
