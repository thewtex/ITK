  ...
  ::UpdateCurrentTime( void )
{
  if ( this->GetCurrentIteration() > 0 )
    {
    const double beta = m_SigmoidScale * vcl_log( - m_SigmoidMax / m_SigmoidMin );
    const double prod = inner_product( this->m_PreviousGradient, this->GetGradient() );
    const double x = ( static_cast< double >( -prod ) - beta ) / m_SigmoidScale;
    const double e = 1.0 / ( 1.0 + vcl_exp(-x) );
    const double sigmoidvalue = ( m_SigmoidMax - m_SigmoidMin ) * e + m_SigmoidMin;

    this->m_CurrentTime = this->m_CurrentTime + sigmoidvalue;
