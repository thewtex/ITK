  ...
  const VectorType & GlobalPt, VectorType & LocalPt) const
{
  ...
  for( i = 0; i < 4; i++ )
    {
    e1->SetNode(0, this->GetNode(LinearTris[i][0]).GetPointer() );
    e1->SetNode(1, this->GetNode(LinearTris[i][1]).GetPointer() );
    e1->SetNode(2, this->GetNode(LinearTris[i][2]).GetPointer() );
