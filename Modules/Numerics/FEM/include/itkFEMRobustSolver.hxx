    ...
    if(m_UseInterpolationGrid)
      ...
      if( this->m_InterpolationGrid->TransformPhysicalPointToIndex(point, index) )
        ...
        if(e != NULL)
          ...
          const IndexType numberOfDimensions = e->GetNumberOfSpatialDimensions();
...
::ComputeLandmarkSimulatedDisplacementAndWeightedError()
{
  ...
  while(it != container->End())
    ...
    if(!landmark->IsOutlier())
      ...
      const IndexType numberOfDOFs = ep->GetNumberOfDegreesOfFreedomPerNode();
      const IndexType numberOfNodes = ep->GetNumberOfNodes();

      VectorType error(numberOfDOFs, 0.0);
      VectorType nodeSolution(numberOfDOFs);

      for(IndexType nodeId = 0; nodeId < numberOfNodes; ++nodeId)
        {
        for(IndexType dofId = 0; dofId < numberOfDOFs; ++dofId)
          {
          const int degreeOfFreedom = ep->GetDegreeOfFreedom(nodeId * numberOfDOFs + dofId);
...
::ComputeLandmarkTensor()
{
  ...
  for(; it != container->End(); ++it)
    ...
    if(!landmark->IsOutlier())
      ...
      for(IndexType nodeId = 0; nodeId < numberOfNodes; ++nodeId)
        {
        for(IndexType dofXId = 0; dofXId < numberOfDOFs; dofXId++)
          {
          for(IndexType dofYId = 0; dofYId < numberOfDOFs; dofYId++)
            {
            unsigned nx = ep->GetDegreeOfFreedom(nodeId * numberOfDOFs + dofXId);
            unsigned ny = ep->GetDegreeOfFreedom(nodeId * numberOfDOFs + dofYId);
...
::AssembleMeshStiffnessMatrix()
{
  ...
  IndexType numberOfElements = this->m_FEMObject->GetNumberOfElements();
...
::AssembleElementMatrixWithID(Element::Pointer e, unsigned int matrixIndex)
{
  ...
  const IndexType numberOfDOFs = e->GetNumberOfDegreesOfFreedom();

  // step over all rows in element matrix
  for( IndexType j = 0; j < numberOfDOFs; j++ )
    {
    // step over all columns in element matrix
    for( IndexType k = 0; k < numberOfDOFs; k++ )
      {
  ...
  for(;it != container->End(); ++it)
    ...
    if(!landmark->IsOutlier())
      ...
      for(IndexType k = 0; k < numberOfNodes; ++k)
        ...
        for(IndexType n = 0; n < numberOfDOFs; n++)
          {
          for(IndexType m = 0; m < numberOfDOFs; m++)
            {
            const int dofn = ep->GetDegreeOfFreedom(k * numberOfDOFs + n);
            const int dofm = ep->GetDegreeOfFreedom(k * numberOfDOFs + m);
      ...
      for(IndexType i = 0; i < numberOfNodes - 1; i++)
        {
        for(IndexType j = i + 1; j < numberOfNodes; j++)
          {
          ...
          for(IndexType n = 0; n < numberOfDOFs; n++)
            ...
            for(IndexType m = 0; m < numberOfDOFs; m++)
              {
              const int dofn = ep->GetDegreeOfFreedom(i * numberOfDOFs + n);
              const int dofm = ep->GetDegreeOfFreedom(j * numberOfDOFs + m);
              const float value = static_cast<float>( barCoor * m_TradeOffImageMeshEnergy * pointTensorPonderation * (tens(n, m)) * confidence );
...
::AssembleF()
{
  ...
  for(;it != container->End(); ++it)
    ...
    if(!landmark->IsOutlier())
      ...
      for(IndexType m = 0;m < numberOfNodes; ++m)
        {
        ...
        for(IndexType j = 0; j < numberOfDOFs; ++j)
          {
          const int index = ep->GetDegreeOfFreedom(m * numberOfDOFs + j);
...
::InitializeInterpolationGrid()
{
  ...
  IndexType numberOfElements = this->m_FEMObject->GetNumberOfElements();
  for( IndexType index = 0; index < numberOfElements; index++ )
    {
    ...
    const IndexType NumberOfDimensions = e->GetNumberOfSpatialDimensions();
    for( IndexType n = 1; n < e->GetNumberOfNodes(); n++ )
      {
      const VectorType & v = e->GetNodeCoordinates(n);
      for( IndexType d = 0; d < NumberOfDimensions; d++ )
        {
        if( v[d] < v1[d] )
          {
          v1[d] = v[d];
