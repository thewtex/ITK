...
::RobustSolver()
{
  ...
  //force = external force + landmark force
  m_ForceIndex = 0;
...
::Initialization()
{
  ...
  unsigned int maxNumberOfNonZeroValues = this->m_NGFN *
      m_MaximumNonZeroMatrixEntriesFactor;
...
::InitializeLandmark()
{
    ...
    LoadNoisyLandmark * landmark = NULL;

    for(; it != this->m_FEMObject->GetLoadContainer()->End(); ++it)
    {
        landmark = dynamic_cast<LoadNoisyLandmark*>((it.Value()).GetPointer());
    ...
    for( unsigned int i = 0; i < this->FEMDimension; i++ )
      {
        ...
        if(m_UseInterpolationGrid)
        {
            ...
            if( this->m_InterpolationGrid->TransformPhysicalPointToIndex(point,
                ind) )
            {
                Element::ConstPointer e = this->m_InterpolationGrid->GetPixel(ind).
                    GetPointer();
...
::RunSolver()
{
      ...
      if(m_OutlierRejectionSteps != 0)
        this->IncrementalSolverWithOutlierRejection(m_OutlierRejectionSteps,
        m_ApproximationSteps);
                                        ...
                                        unsigned int ApproximationSteps)
{
  unsigned int numberOfRejectedLandmarksPerStep =
      (unsigned int) floor (double(this->m_FEMObject->GetNumberOfLoads()) *
      m_FracErrorRejected / (double(OutlierRejectionSteps)));
  ...
  for (unsigned int i = 0; i < OutlierRejectionSteps; ++i)
  {
    j++;
    ...
    double displacementPonderation)
{
    ...
    for(; it != this->m_FEMObject->GetLoadContainer()->End(); ++it)
    {
        LoadNoisyLandmark * landmark = dynamic_cast<LoadNoisyLandmark*>
            ((it.Value()).GetPointer());
        ...
        if(!landmark->IsOutlier())
        {
            ...
            for(unsigned int m = 0; m < Nnodes; m++)
            {
                for(unsigned int j = 0; j < NnDOF; ++j)
                    nodeSolution[j] = this->m_ls->GetSolutionValue(
        ...
        if(!landmark->IsOutlier())
        {
            ...
            for(unsigned int m = 0; m < Nnodes; ++m)
            {
                for(unsigned int i = 0; i < NnDOF; i++)
                    for(unsigned int j = 0; j < NnDOF; j++)
                    {
                        nodeTensor[i][j] = this->m_ls->GetMatrixValue(
...
::Nth_element_WRTDisplacementError(unsigned int nthPoint)
{
    ...
    LoadVectorType &loadVector = this->m_FEMObject->GetLoadContainer()
        ->CastToSTLContainer();
...
::DeleteLandmarksOutOfMesh()
{
    ...
    LoadContainerIterator iter;
    typedef itk::VectorContainer<typename FEMObjectType::LoadIdentifier,
        itk::fem::Load::Pointer> VectorContainerType;
    typename VectorContainerType::Pointer ToRemoveLoadContainer =
        VectorContainerType::New();
    typename VectorContainerType::Pointer NewLoadContainer =
        VectorContainerType::New();

    unsigned int count=0;
    unsigned int InitialNumLoads = this->m_FEMObject->GetLoadContainer()->Size();
    unsigned int NumToRemoveLoads = 0;

    LoadNoisyLandmark * landmark = NULL;
    for(iter = this->m_FEMObject->GetLoadContainer()->Begin();
        iter != this->m_FEMObject->GetLoadContainer()->End(); ++iter)
    {
        count++;
        landmark = dynamic_cast<LoadNoisyLandmark*> ((iter.Value()).GetPointer());
    ...
    this->m_FEMObject->GetLoadContainer()->clear();
    ToRemoveLoadContainer->clear();

    // add the new loads to Load container
    for(iter = NewLoadContainer->Begin(); iter != NewLoadContainer->End(); ++iter)
         this->m_FEMObject->AddNextLoad(iter.Value());
}
        ...
        if(!landmark->IsOutlier())
        {
            ...
            for(unsigned int k = 0; k < Nnodes; ++k)
            {
                ...
                for(unsigned int n = 0; n < NnDOF; n++)
                    ...
                    {
                        this->m_ls->AddMatrixValue(ep->GetDegreeOfFreedom(k * NnDOF + n),
                ...
                for(unsigned int j = i + 1; j < Nnodes; j++)
                {
                    ...
                    for(unsigned int n = 0; n < NnDOF; n++)
                        for(unsigned int m = 0; m < NnDOF; m++)
                        {
                            ...
                            this->m_ls->AddMatrixValue(ep->GetDegreeOfFreedom(
                                j * NnDOF + m),
                                ep->GetDegreeOfFreedom(i * NnDOF + n),
                                barCoor * m_TradeOffImageMeshEnergy *
                                pointTensorPonderation * (tens(n, m)) *
    ...
    for(;it != this->m_FEMObject->GetLoadContainer()->End(); ++it)
    {
        ...
        if(!landmark->IsOutlier())
        {
            ...
            for(unsigned int m = 0;m < Nnodes; ++m)
            {
                ...
                for(unsigned int j = 0; j < NnDOF; ++j)
                    this->m_ls->AddVectorValue(ep->GetDegreeOfFreedom(m * NnDOF + j),
                    weightedRealDisplacement[j], m_LandmarkForceIndex);
            }
...
{
    this->m_ls->MultiplyMatrixSolution(m_ExternalForceIndex, m_MeshStiffnessMatrixIndex,
    m_SolutionIndex);
}
                              ...
                              const InterpolationGridDirectionType& direction)
{
  ...
  for( unsigned int index = 0; index < numberOfElements; index++ )
    ...
    ImageRegionIterator<InterpolationGridType> iter(this->m_InterpolationGrid,
        interRegion);
...
::PrintForce()
{
  ...
  for( unsigned int i = 0; i < this->m_NGFN; i++ )
    {
    //itkDebugMacro(m_ls->GetVectorValue(i, m_ForceTIndex));
    }
}
