    ...
    for(; it != this->m_FEMObject->GetLoadContainer()->End(); ++it)
    {
        ...
        if(!landmark->IsOutlier())
        {
            ...
            if(landmark->HasStructureTensor())
              {
              /* FIX ME */
              //landmark->SetLandmarkTensor(landmarkTensor * landmark->GetStructureTensor());
              }
            else
              {
...
{
    this->m_ls->AddVectorVector(m_ExternalForceIndex,m_ForceIndex);
                              ...
                              const InterpolationGridDirectionType& direction)
{
  ...
  for( unsigned int index = 0; index < numberOfElements; index++ )
    ...
    ImageRegionIterator<InterpolationGridType> iter(this->m_InterpolationGrid, region);
