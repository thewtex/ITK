...
::Solve()
{
  ...
  for(unsigned int i=0; i<this->m_ls->GetSystemOrder(); i++)
    ...
    std::cout << "Calculate the predictors " << i << std::endl;
    }
...
::RunSolver()
{
  ...
  //this->InitializeInterpolationGrid(imagesize, meshorigin, meshsize);

  for (unsigned int nit = 0; nit < m_NumberOfIterations; nit++)
    {
    std::cout << "Solve " << nit << std::endl;
    this->AssembleF();            // Assemble the global load vector F
