...
FEMScatteredDataPointSetToImageFilter<TInputPointSet, TInputMesh,TOutputImage,
TInputConfidencePointSet,TInputTensorPointSet>
::InitializeMaterials(FEMObjectPointer femObject)
{
...
FEMScatteredDataPointSetToImageFilter<TInputPointSet, TInputMesh,TOutputImage,
TInputConfidencePointSet,TInputTensorPointSet>
::InitializeNodes(FEMObjectPointer femObject)
{
  ...
  PointsContainer::Pointer meshPoints = this->m_Mesh->GetPoints();
...
FEMScatteredDataPointSetToImageFilter<TInputPointSet, TInputMesh,TOutputImage,
TInputConfidencePointSet,TInputTensorPointSet>
::InitializeElements(FEMObjectPointer femObject)
{
  ...
  // mesh cell iterator
  CellsContainer::Pointer cells = this->m_Mesh->GetCells();
...
FEMScatteredDataPointSetToImageFilter<TInputPointSet, TInputMesh,TOutputImage,
TInputConfidencePointSet,TInputTensorPointSet>
::InitializeLoads(FEMObjectPointer femObject)
{
  ...
  PointDataIterator itDis = displacementVector->Begin();
  ConfidencePointDataIterator  itCon;
  TensorPointDataIterator itTen;

  if(this->m_ConfidencePointSet.IsNotNull() )
    {
    ConfidencePointDataContainerType *confidenceVector = this->m_ConfidencePointSet->GetPointData();
    itCon = confidenceVector->Begin();
    }

  if(this->m_TensorPointSet.IsNotNull() )
    {
    TensorPointDataContainerType *tensorVector = this->m_TensorPointSet->GetPointData();
    itTen = tensorVector->Begin();
    }
...
::ProduceDeformationField()
{
  ...
  for( iter.GoToBegin(); !iter.IsAtEnd(); ++iter )
    {
    output->TransformIndexToPhysicalPoint(iter.GetIndex(), pt);
    ...
    if( ep->GetLocalFromGlobalCoordinates(globalPoint, localPoint) )
      {
      const FEMVectorType & shape = ep->ShapeFunctions(localPoint);
      ...
      for(unsigned int m = 0; m < Nnodes; ++m)
        ...
        for(unsigned int j = 0; j < NnDOF; ++j)
          {
          unsigned int dofId = ep->GetDegreeOfFreedom(m * NnDOF + j);
