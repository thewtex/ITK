/* ----------------------------- MNI Header -----------------------------------
@NAME       : itkMincHelpers.cxx
@DESCRIPTION: MINC <-> ITK
@COPYRIGHT  :
...
namespace minc
{
  ...
  int get_image_limits (MINC_DEF3D::Pointer img, MINC_VOXEL_TYPE &min, MINC_VOXEL_TYPE &max)
  {
    ...
    for (it.GoToBegin (); !it.IsAtEnd (); ++it)
    {
      MINC_VOXEL_TYPE va=v_max(it.Value () );
      MINC_VOXEL_TYPE vi=v_min(it.Value () );
  ...
  void write_tags(const MINC_TAG_POINTS& tags, const char * file)
  {
    ...
    for(MINC_TAG_POINTS::const_iterator i = tags.begin(); i != tags.end(); i++)
    {
  ...
  void write_tags(const MINC_TAG_POINTS& tags,const std::vector<int>& labels, const char * file)
  {
    ...
    MINC_TAG_POINTS::const_iterator i;
  ...
  void write_2tags(const MINC_TAG_POINTS& tags,const MINC_TAG_POINTS& tags2, const char * file)
  {
    ...
    out<<"Points =";
    //TODO add all the things
    MINC_TAG_POINTS::const_iterator j=tags2.begin();
    MINC_TAG_POINTS::const_iterator i=tags.begin();
    for(;i!=tags.end();i++,j++)
    {
  ...
  void read_tags(MINC_TAG_POINTS& tags, const char * file,int vol)
  {
    ifstream in(file);
  ...
  void read_tags(MINC_TAG_POINTS& tags, std::vector<int>& labels, const char * file, int vol)
  {
    ifstream in(file);
    if(!in.good()) REPORT_ERROR("Can't open file!");
    //tags.clear();
  ...
  void read_tags(MINC_TAG_POINTS& tags, std::vector<double>& labels, const char * file, int vol)
  {
    ifstream in(file);
    if(!in.good()) REPORT_ERROR("Can't open file!");
    //tags.clear();
  ...
  void read_tags(MINC_TAG_POINTS& tags, std::vector<float>& labels, const char * file, int vol)
  {
    ifstream in(file);
    if(!in.good()) REPORT_ERROR("Can't open file!");
    //tags.clear();
