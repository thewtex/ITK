    ...
    if(transformType.find("CompositeTransform") != std::string::npos)
    {
      ...
      if(transformType.find("AffineTransform") != std::string::npos && curTransform->GetNumberOfParameters() == 12)
      {
        ...
        if( _grid_transform->GetDisplacementField() )
          {
          EncapsulateMetaData<std::string>(_grid_transform->GetModifiableDisplacementField()->GetMetaDataDictionary(),"storage_data_type",typeid(float).name());
          writer->SetInput( _grid_transform->GetDisplacementField() );
          }
        else if( _grid_transform->GetInverseDisplacementField() )
          {
            EncapsulateMetaData<std::string>(_grid_transform->GetModifiableInverseDisplacementField()->GetMetaDataDictionary(),"storage_data_type",typeid(float).name());
        ...
        if( _grid_transform->GetDisplacementField() )
          {
          EncapsulateMetaData<std::string>(_grid_transform->GetDisplacementField()->GetMetaDataDictionary(),"storage_data_type",typeid(float).name());
          writer->SetInput( _grid_transform->GetDisplacementField() );
          }
        else if( _grid_transform->GetInverseDisplacementField() )
          {
            EncapsulateMetaData<std::string>(_grid_transform->GetInverseDisplacementField()->GetMetaDataDictionary(),"storage_data_type",typeid(float).name());
            writer->SetInput( _grid_transform->GetInverseDisplacementField() );
            _inverse_grid=false;
          }
        else
