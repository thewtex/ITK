    ...
    if(transformType.find("CompositeTransform") != std::string::npos)
    {
      ...
      if(transformType.find("AffineTransform") != std::string::npos && curTransform->GetNumberOfParameters() == 12)
      {
        ...
        if( _grid_transform->GetDisplacementField() )
          {
          EncapsulateMetaData<std::string>(_grid_transform->GetDisplacementField()->GetMetaDataDictionary(),std::string("storage_data_type"),std::string(typeid(float).name()));
          writer->SetInput( _grid_transform->GetDisplacementField() );
          }
        else if( _grid_transform->GetInverseDisplacementField() )
          {
            EncapsulateMetaData<std::string>(_grid_transform->GetInverseDisplacementField()->GetMetaDataDictionary(),std::string("storage_data_type"),std::string(typeid(float).name()));
