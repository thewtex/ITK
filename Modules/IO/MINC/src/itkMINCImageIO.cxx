...
void MINCImageIO::WriteImageInformation(void)
{
  ...
  if(nComp>3) //first dimension will be either vector or time
    ...
    micreate_dimension(MItime, MI_DIMCLASS_TIME, MI_DIMATTR_REGULARLY_SAMPLED, nComp,
                       &this->m_MincApparentDims[minc_dimensions] );
  ...
  micreate_dimension(MIxspace,MI_DIMCLASS_SPATIAL, MI_DIMATTR_REGULARLY_SAMPLED, this->GetDimensions(
                       0), &this->m_MincApparentDims[minc_dimensions] );
  minc_dimensions++;

  if(nDims > 1)
    {
    micreate_dimension(MIyspace,MI_DIMCLASS_SPATIAL, MI_DIMATTR_REGULARLY_SAMPLED, this->GetDimensions(
                         1), &this->m_MincApparentDims[minc_dimensions] );
    minc_dimensions++;
    }

  if(nDims > 2)
    {
    micreate_dimension(MIzspace,MI_DIMCLASS_SPATIAL, MI_DIMATTR_REGULARLY_SAMPLED, this->GetDimensions(
                         2), &this->m_MincApparentDims[minc_dimensions] );
    minc_dimensions++;
    }
...
//       break;
  ...
  for(MetaDataDictionary::ConstIterator it=thisDic.Begin(); it!=thisDic.End(); ++it)
    ...
    if(d)
      ...
      }  else  { // else we have an attribute without variable name, most likely
