...
void MINCImageIO::Read(void *buffer)
{
  ...
  misize_t *start=new misize_t[numberOfDimensions+(nComp>1 ? 1 : 0)];
  misize_t *count=new misize_t[numberOfDimensions+(nComp>1 ? 1 : 0)];

  for ( unsigned int i = 0; i < numberOfDimensions; i++ )
    {
    if ( i < m_IORegion.GetImageDimension() )
      {
      start[numberOfDimensions-i-1] = m_IORegion.GetIndex()[i];
      count[numberOfDimensions-i-1] = m_IORegion.GetSize()[i];
      }
    else
      {
      start[numberOfDimensions-i-1] = 0;
      count[numberOfDimensions-i-1] = 1;
      }
    }
  if(nComp>1)
    {
    start[numberOfDimensions]=0;
    count[numberOfDimensions]=nComp;
    }
...
void MINCImageIO::AllocateDimensions(int numberOfDimensions)
{
  this->CleanupDimensions();

  m_NDims=numberOfDimensions;
...
void MINCImageIO::WriteImageInformation(void)
{
  ...
  this->CloseVolume();
  this->AllocateDimensions(numberOfDimensions+(nComp>1 ? 1 : 0) );
