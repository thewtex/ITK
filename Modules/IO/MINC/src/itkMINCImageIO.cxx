...
MINCImageIO::ReadImageInformation()
{
  ...
  if(this->m_DimensionIndices[0]!=-1) // have vector dimension
    ...
    //always use positive, vector dimension does not supposed to have notion of
    // positive step size, so leaving as is
    //miset_dimension_apparent_voxel_order(this->m_MincApparentDims[usable_dimensions],MI_POSITIVE);
    misize_t _sz;
    miget_dimension_size(this->m_MincApparentDims[usable_dimensions],&_sz);
...
//       break;
  ...
  if(ExposeMetaData< std::string >(thisDic,"dimension_order",dimension_order) )
    ...
    //std::cout<<"Restoring original dimension
    // order:"<<dimension_order.c_str()<<std::endl;
    if(dimension_order.length() == (minc_dimensions*2) )
      ...
      for(unsigned int i=0; i<minc_dimensions && dimorder_good; i++)
        ...
        if(dimorder_good)
          {
          //flip the sign
          if(!positive && dimension_order[i*2+1]!='V' && dimension_order[i*2+1]!='v' ) //Vector
                                                                                       //
                                                                                       //
                                                                                       // dimension
                                                                                       //
