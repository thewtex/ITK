...
MINCImageIO::MINCImageIO()
{
  ...
  this->m_CompressionLevel = 4; // Range 0-9; 0 = no file compression, 9 = maximum
                          // file compression
...
void MINCImageIO::ReadImageInformation()
{
  ...
  if(miget_attr_length(m_volume,"","history",&minc_history_length)==MI_NOERROR)
  {
    ...
    {
      itk::EncapsulateMetaData<std::string>(thisDic,"history", std::string(minc_history));
    }
    delete [] minc_history;
  }
  ...
  if(this->m_DimensionIndices[4]!=-1) //have time dimension
  {
    ...
    itk::EncapsulateMetaData<double>(thisDic,"tstart",_start);
    itk::EncapsulateMetaData<double>(thisDic,"tstep",_sep);
  }

  itk::EncapsulateMetaData< std::string >(thisDic,"dimension_order",dimension_order);
  ...
  if ( (milist_start(m_volume, "", 0, &grplist)) == MI_NOERROR )
  {
    ...
    while( milist_grp_next(grplist, group_name, sizeof(group_name)) == MI_NOERROR )
    {
      if((milist_start(m_volume, group_name, 1 , &attlist)) == MI_NOERROR)
      {
        ...
        while( milist_attr_next(m_volume,attlist,group_name,sizeof(group_name),attribute,sizeof(attribute)) == MI_NOERROR )
        {
            ...
            switch(att_data_type)
            {
              case MI_TYPE_STRING:
              {
                ...
                if(miget_attr_values(m_volume,att_data_type,group_name,attribute,att_length,tmp) == MI_NOERROR )
                  itk::EncapsulateMetaData< std::string >( thisDic, entry_key, std::string(tmp));
                delete [] tmp;
              }
                break;
              case MI_TYPE_FLOAT:
              {
                ...
                if(miget_attr_values(m_volume,att_data_type,group_name,attribute,att_length,tmp.data_block()) == MI_NOERROR )
                {
                  if(att_length==1)
                    itk::EncapsulateMetaData<float>(thisDic,entry_key,tmp[0]);
                  else
                    itk::EncapsulateMetaData<Array<float> >(thisDic,entry_key,tmp);
                }
              }
                break;
              case MI_TYPE_DOUBLE:
              {
                ...
                {
                  if(att_length==1)
                    itk::EncapsulateMetaData<double>(thisDic,entry_key,tmp[0]);
                  else
                    itk::EncapsulateMetaData<Array<double> >(thisDic,entry_key,tmp);
                }
              }
                break;
              case MI_TYPE_INT:
              {
                ...
                {
                  if(att_length==1)
                    itk::EncapsulateMetaData<int>(thisDic,entry_key,tmp[0]);
                  else
                    itk::EncapsulateMetaData<Array<int> >(thisDic,entry_key,tmp);
                }
              }
                break;
              default:
...
void MINCImageIO::WriteImageInformation(void)
{
  ...
  double tstart=0.0;
  double tstep=0.0;
  bool   time_metadata=false;

  if(nComp>3) //first dimension will be either vector or time
  {
     micreate_dimension(MItime, MI_DIMCLASS_TIME, MI_DIMATTR_REGULARLY_SAMPLED, nComp, &this->m_MincApparentDims[minc_dimensions] );

     if(itk::ExposeMetaData< double >(thisDic,"tstart",tstart))
       miset_dimension_start(this->m_MincApparentDims[minc_dimensions],tstart);

     if(itk::ExposeMetaData< double >(thisDic,"tstep",tstep))
      miset_dimension_separation(this->m_MincApparentDims[minc_dimensions],tstep);

     minc_dimensions++;
  } else if(nComp>1) {
     micreate_dimension(MIvector_dimension,MI_DIMCLASS_RECORD, MI_DIMATTR_REGULARLY_SAMPLED, nComp, &this->m_MincApparentDims[minc_dimensions] );
     minc_dimensions++;
  }

  micreate_dimension(MIxspace,MI_DIMCLASS_SPATIAL, MI_DIMATTR_REGULARLY_SAMPLED, this->GetDimensions(0), &this->m_MincApparentDims[minc_dimensions] );
  minc_dimensions++;

  if(nDims > 1)
  {
    micreate_dimension(MIyspace,MI_DIMCLASS_SPATIAL, MI_DIMATTR_REGULARLY_SAMPLED, this->GetDimensions(1), &this->m_MincApparentDims[minc_dimensions] );
    minc_dimensions++;
  }

  if(nDims > 2)
  {
    micreate_dimension(MIzspace,MI_DIMCLASS_SPATIAL, MI_DIMATTR_REGULARLY_SAMPLED, this->GetDimensions(2), &this->m_MincApparentDims[minc_dimensions] );
    minc_dimensions++;
  }
  ...
  for (int i = 0; i < nDims; i++ )
    {
      ...
      miset_dimension_separation(this->m_MincApparentDims[j],this->GetSpacing(i));
      miset_dimension_start(this->m_MincApparentDims[j],origin[i]);
      miset_dimension_cosines(this->m_MincApparentDims[j],dir_cos);
    }
...
//       break;
  ...
  //now let's create the same dimension order and positive/negative step size as in original image
  std::string dimension_order;
  bool dimorder_good=false;
  if(itk::ExposeMetaData< std::string >(thisDic,"dimension_order",dimension_order))
  {
    //the format should be ((+|-)(X|Y|Z|V|T))*
    if(dimension_order.length()==minc_dimensions*2)
    {
      dimorder_good=true;
      for(int i=0;i<minc_dimensions && dimorder_good;i++)
      {
        ...
        switch(dimension_order[i*2+1])
        {
          ...
          case 'V':
            if(nComp<=1)
            {
              itkDebugMacro(<<"Dimension order is incorrrect " << dimension_order.c_str() );
              dimorder_good=false;
            } else {
              j=0;
            }
  ...
  if(!dimorder_good) //use default order!
    for(int i=0;i<minc_dimensions;i++)
  ...
  if ( micreate_volume ( m_FileName.c_str(), minc_dimensions, this->m_MincFileDims, m_volume_type,
                    m_volume_class, hprops, &m_volume )<0 )
    {
    // Error opening the volume
    itkDebugMacro("Could not open file \"" << m_FileName.c_str() << "\".");
    return;
  ...
  for(MetaDataDictionary::ConstIterator it=thisDic.Begin();it!=thisDic.End();++it)
  {
    ...
    if(d)
    {
      ...
      } else if(!strcmp(tname,typeid(itk::Array<double>).name())) {
        const itk::Array<double> &tmp=dynamic_cast<MetaDataObject<itk::Array<double> > * >(bs)->GetMetaDataObjectValue();
        miset_attr_values(m_volume,MI_TYPE_DOUBLE,var.c_str(),att.c_str(),tmp.Size(),tmp.data_block());
      } else if(!strcmp(tname,typeid(itk::Array<float>).name())) {
        const itk::Array<float> &tmp=dynamic_cast<MetaDataObject<itk::Array<float> > * >(bs)->GetMetaDataObjectValue();
        miset_attr_values(m_volume,MI_TYPE_FLOAT,var.c_str(),att.c_str(),tmp.Size(),tmp.data_block());
      } else if(!strcmp(tname,typeid(itk::Array<int>).name())) {
        const itk::Array<int> &tmp=dynamic_cast<MetaDataObject<itk::Array<int> > * >(bs)->GetMetaDataObjectValue();
