...
void MINCImageIO::WriteImageInformation(void)
{
  ...
  if(thisDic.HasKey("tstart") && thisDic.HasKey("tstep"))
  {
    tstart=dynamic_cast<MetaDataObject<double > * >( thisDic.Find("tstart")->second.GetPointer() )->GetMetaDataObjectValue();
    tstep= dynamic_cast<MetaDataObject<double > * >( thisDic.Find("tstep")->second.GetPointer() )->GetMetaDataObjectValue();
  }
  ...
  if(nComp>3) //first dimension will be either vector or time
  {
     ...
     miset_dimension_separation(m_MincFileDims[minc_dimensions],tstep);
     miset_dimension_start(m_MincFileDims[minc_dimensions],tstart);
