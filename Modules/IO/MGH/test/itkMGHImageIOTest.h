...
template <>
typename itk::Image<itk::DiffusionTensor3D<float>,3>::Pointer
itkMGHImageIOTestGenerateRandomImage<itk::DiffusionTensor3D<float>,3>(unsigned int size)
{
  typedef typename itk::Image<itk::DiffusionTensor3D<float>,3> TensorImageType;
  typedef typename TensorImageType::Pointer                    TensorImagePointer;

  typename TensorImageType::SizeType      sz;
  ...
  for(unsigned i = 0; i < pix.Size(); ++i)
    {
    ScalarImagePointer xx = itkMGHImageIOTestGenerateRandomImage<double,3>(size);
    scalarImageVec.push_back(xx);
    }
  typename itk::ImageRegionIteratorWithIndex<TensorImageType>
  ...
  for(tensorIt.GoToBegin(); !tensorIt.IsAtEnd(); ++tensorIt)
    ...
    for(unsigned int i = 0; i < pix.Size(); ++i)
      {
      typename TensorImageType::IndexType index = tensorIt.GetIndex();
