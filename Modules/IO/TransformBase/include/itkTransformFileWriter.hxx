...
struct TransformIOHelper
{
  ...
  CreateNewTypeTransform(std::string transformName)
  {
    ...
    TransformName<TOutputScalarType>::CorrectPrecisionType( transformName );
  ...
  ConvertParametersType(const OptimizerParameters< TInputScalarType >  &sourceParams)
  {
    ...
    for( SizeValueType i = 0; i < sourceParams.GetSize(); ++i )
      {
      outputParams[i] = static_cast<TOutputScalarType>( sourceParams[i] );
      }
    return outputParams;
  }
...
struct SetInputTransformWithOutputPrecisionType
{
  ...
  if( transformName.find("CompositeTransform") == std::string::npos )
    ...
    */
    CompositeTransformIOHelperTemplate<TInputScalarType> inputHelper;
    ...
    */
    ...
    CompositeTransformIOHelperTemplate<TOutputScalarType> outputHelper;
