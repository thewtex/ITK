/*=========================================================================
  ...
  June 2010 Jason Dowling
    ...
    2. Now trims contour names to remove blank space
...
int itkGDCMRTStructReaderTest( int argc, char * argv[] )
{
  ...
  // NOTE ALEX: if we could read only the metadata that would be better as
  // we never use the Image per say. It would also be way faster.
  ...
  // NOTE ALEX: it looks to me like we are reading a second time the image here,
  // can't we extract spacing and other things directling from the gdcm reader?
  ...
  for(unsigned int pd = 0; pd < sqi->GetNumberOfItems(); ++pd)
    ...
    for(unsigned int i = 0; i < nitems; ++i)
      ...
      for(unsigned int j = 0; j < npts * 3; j+=3)
        ...
        // NOTE ALEX: check point type here
      ...
      // NOTE ALEX: just push in the mesh container here and you re done.
