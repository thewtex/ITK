                                                             ...
                                                             const ImageIORegion & pasteRegion) const
{
  ...
  const unsigned int valuesPerPiece = Math::Ceil< unsigned int >( range / double(numberOfRequestedSplits) );
  const unsigned int maxPieceUsed = Math::Ceil< unsigned int >( range / double(valuesPerPiece) );
  return maxPieceUsed;
}
                                                    ...
                                                    const ImageIORegion & pasteRegion) const
{
  ...
  // determine the actual number of pieces that will be generated
  const ImageIORegion::SizeType::value_type range = regionSize[splitAxis];
  const unsigned int valuesPerPiece = Math::Ceil< unsigned int >(range / (double)numberOfActualSplits);
