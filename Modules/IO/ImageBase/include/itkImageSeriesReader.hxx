    ...
    if ( !insideRequestedRegion )
      ...
      if( readSize == sliceRegionToRequest.GetSize() )
        ...
        if ( strcmp(output->GetNameOfClass(), "VectorImage") == 0 )
          ...
          readerOutput->GetPixelContainer()->SetImportPointer( outputSliceBuffer, numberOfPixelsInSlice*numberOfInternalComponentsPerPixel, bufferDelete );
          }
        else
          {
          // otherwise the actual number of pixels needs to be passed
          readerOutput->GetPixelContainer()->SetImportPointer( outputSliceBuffer, numberOfInternalComponentsPerPixel, bufferDelete );
          }
