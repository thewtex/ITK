...
int itkDOMTest5( int argc, char* argv[] )
{
  ...
  try
    ...
    testobj1->SetFooFileName( "itkDOMTest5-output_foo.txt" );
    // settings for bar
    testobj1->SetBarValue( 12345 );
    testobj1->SetBarFileName( "itkDOMTest5-output_bar.txt" );

    std::cout << "foo value: " << testobj1->GetFooValue() << std::endl;
    std::cout << "foo file : " << testobj1->GetFooFileName() << std::endl;
    std::cout << std::endl;

    std::cout << "bar value: " << testobj1->GetBarValue() << std::endl;
    std::cout << "bar file : " << testobj1->GetBarFileName() << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "returned bar value: " << testobj2->GetBarValue() << std::endl;
    std::cout << std::endl;
    if ( testobj1->GetFooValue() != testobj2->GetFooValue() )
      {
      throw "foo values mismatch";
      }
    if ( testobj1->GetBarValue() != testobj2->GetBarValue() )
      {
      throw "bar values mismatch";
      }
