...
protected:
   ...
   */
  virtual void GenerateData( DOMNodeType* domnode, const void* );
...
inline void
DOMTestObjectDOMWriter::GenerateData( DOMNodeType* domnode, const void* )
{
  const InputType* input = this->GetInput();

  std::ofstream ofs;

  domnode->SetName( "DOMTestObject" );

  // write child foo
  DOMNodePointer foo = DOMNodeType::New();
  foo->SetName( "foo" );
  foo->SetAttribute( "fname", input->GetFooFileName() );
  domnode->AddChild( foo );
  // write the foo value to file
  this->CreateFile( input->GetFooFileName().c_str() );
  ...
  domnode->AddChildAtEnd( bar );
  // write the bar value to file
  this->CreateFile( input->GetBarFileName().c_str() );
