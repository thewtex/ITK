 ...
 *          i.e., the requests for update on this class won't propogate upwards, because the input of this class
 *          is a general object (not a DataObject). So, if it is connected to the end of a pipeline, the pipeline
 *          update cannot be initiated from this class. This design decision was made to handle XML writing for not
 *          only data objects, but also any ITK objects that can be serialized on disk, e.g., transformations,
 *          optimizers, registrations, user objects, and so on.
 *
 ...
 */
template< class T >
class DOMWriter : public ProcessObject
{
public:
  /** Standard class typedefs. */
  typedef DOMWriter Self;

  itkTypeMacro(DOMWriter, ProcessObject);
   ...
   */
  void Update( DOMNodeType* domnode, const void* userdata = 0 );
...
protected:
   ...
   * the optional argument 'userdata' can be used to provide the missed information.
   */
  virtual void GenerateData( DOMNodeType* domnode, const void* userdata ) = 0;

  // The following helper functions may be useful when user writers write XML fields that are disk files.

  /** Helper function to create the directory for a file, if it does not exist. */
  void CreateDirectory( const char* fn );
