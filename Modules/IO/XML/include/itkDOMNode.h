...
public:
  ...
  /** Add a child in front of another child (throw exception if not able to
    add). */
  ...
  /** Add a child at the end of the children list (throw exception if not able
    to add). */
  ...
  /** Retrieve a child by tag name and an index (multiple children can have a
    same tag name, return NULL if no such child). */
  ...
  /** Return the path of this node within its root, in the form of a query
    string that uses only indices. */
  virtual std::string GetPath() const;

  /** Get a child and cast it to a text node (return NULL if out of range or not
    a text node). */
  ...
  /** Generate a text node from a string and add/insert in front of all
    children. */
