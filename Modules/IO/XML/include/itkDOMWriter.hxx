...
template< class T >
void
DOMWriter<T>::Update( DOMNodeType* domnode, const void* userdata )
{
  if ( domnode == NULL )
    {
...
DOMWriter<T>::Update()
{
  ...
  // create the new WD if it does not exist
  this->CreateDirectory( fn );
  // get the current and new WDs
  ...
  // change WD to the new directory
...
template< class T >
void
DOMWriter<T>::CreateDirectory( const char* fn )
{
  ...
  if ( itksys::SystemTools::FileExists( dir, true ) )
    {
    itkExceptionMacro( "directory cannot be created" );
    }
...
template< class T >
void
DOMWriter<T>::CreateFile( const char* fn )
{
