...
public:
  ...
  static void Convert(const ImageRegionType & inImageRegion,
                      ImageIORegionType & outIORegion,
                      const ImageIndexType & largestRegionIndex,
                      const CollapseDimensionsType & collapseDimensions,
                      const CollapseIndexesType & collapseIndexes)
  {
    ...
    // make sure that collapseDimensions and collapseIndexes are of the io dimension
    CollapseDimensionsType cd = collapseDimensions;
    // cd.resize( ioDimension, false );
    CollapseIndexesType ci = collapseIndexes;
    // ci.resize( ioDimension, 0 );
    ...
    for ( unsigned int k = ioi; k < ioDimension; k++ )
      {
      // we do have to collapse, but we use the collapse index even if the
      // user have not requested it explicitly, so it is easier to define which
      // dimension to collapse in the dimensions higher than the requested image
      // dimension. The default value, 0, reproduce the default behavior
      // without the collapse feature
      outIORegion.SetIndex(k, ci[k]);
      outIORegion.SetSize(k, 1);    // Note that default size in IO is 1 not 0
      }
  }
