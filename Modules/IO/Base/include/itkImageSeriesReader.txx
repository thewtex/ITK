...
::ComputeMovingDimensionIndex(ReaderType *reader)
{
  ...
  const TOutputImage * outputImage = reader->GetOutput();
  SizeType dimSize = outputImage->GetLargestPossibleRegion().GetSize();
...
::GenerateOutputInformation(void)
{
  ...
  for ( int i = 0; i < 2 && i < numberOfFiles; ++i )
    ...
    if ( m_FileNames.size() == 1 )
      ...
      direction = outputImage->GetDirection();
      largestRegion = outputImage->GetLargestPossibleRegion();
      numberOfComponents = outputImage->GetNumberOfComponentsPerPixel();
      ...
      for ( j = 0; j < TOutputImage::ImageDimension; j++ )
        {
        position1[j] = static_cast< float >( outputImage->GetOrigin()[j] );
        }
      ...
      for ( j = 0; j < TOutputImage::ImageDimension; j++ )
        ...
        if ( j < position1.size() )
          ...
          {
          origin[j] = static_cast< float >( outputImage->GetOrigin()[j] );
          }
        }
      }
    else if ( i == 1 )
      ...
      for ( j = 0; j < TOutputImage::ImageDimension; j++ )
        {
        position2[j] = static_cast< float >( outputImage->GetOrigin()[j] );
        }
      // Override the position if there is an ITK_ImageOrigin
...
::GenerateData()
{
  ...
  for ( int i = 0; i != numberOfFiles; ++i )
    ...
    TOutputImage * outputImage = reader->GetOutput();
    ...
    outputImage->SetRequestedRegion(sliceRegionToRequest);
    ...
    if ( !insideRequestedRegion )
      ...
      outputImage->UpdateOutputInformation();

      // propagate the requested region to determin what the region
      // will actually be read
      outputImage->PropagateRequestedRegion();

      // check that the size of each slice is the same
      if ( outputImage->GetLargestPossibleRegion().GetSize() != validSize )
        {
      ...
      SizeType readSize = outputImage->GetRequestedRegion().GetSize();
      ...
      if( readSize == sliceRegionToRequest.GetSize() )
        ...
        outputImage->GetPixelContainer()->SetImportPointer( outputSliceBuffer, numberOfPixelsInSlice, filterWillDelete );
        outputImage->UpdateOutputData();
        }
      else
        {
        ...
        ImageRegionConstIterator< TOutputImage > it (outputImage, sliceRegionToRequest);
