 ...
 *
 * Inputs:
 * Four images are required as inputs: fixedImage, movingImage,
 * fixedMask, and movingMask.
 * In the context of correlation, inputs are often defined as: "image"
 * and "template".  In this filter, the fixedImage playes the role of the
 ...
 * movingMask can be set to an image of ones.
 *
 * Image size:
...
public:
  ...
  itkSetInputMacro(FixedImage,InputImageType,0);
  itkGetInputMacro(FixedImage,InputImageType,0);

  /** Set and get the moving image */
  itkSetInputMacro(MovingImage,InputImageType,1);
  itkGetInputMacro(MovingImage,InputImageType,1);

  /** Set and get the fixed mask */
  itkSetInputMacro(FixedMask,InputImageType,2);
  itkGetInputMacro(FixedMask,InputImageType,2);

  /** Set and get the moving mask */
  itkSetInputMacro(MovingMask,InputImageType,3);
  itkGetInputMacro(MovingMask,InputImageType,3);

  /** Set and get the required percentage of overlapping pixels */
  itkSetMacro(RequiredPercentageOfOverlappingPixels,float);
  itkGetMacro(RequiredPercentageOfOverlappingPixels,float);
...
protected:
  MaskedFFTNormalizedCorrelationImageFilter()
  {
    ...
    m_RequiredPercentageOfOverlappingPixels = 0.05;
  }
  ...
  typename TInputImage::Pointer CastImage( const InputImageType * inputImage );
  typename TInputImage::Pointer RotateImage( InputImageType * inputImage );

  template< class LocalInputImageType >
  void PreprocessImagesAndMasks( LocalInputImageType * inputImage, LocalInputImageType * inputMask);
  ...
  template< class LocalInputImageType >
  void ElementPositive( LocalInputImageType * inputImage );

  template< class LocalInputImageType >
  void ElementRound( LocalInputImageType * inputImage );
...
private:
  ...
  float m_RequiredPercentageOfOverlappingPixels;
};
} // end namespace itk
