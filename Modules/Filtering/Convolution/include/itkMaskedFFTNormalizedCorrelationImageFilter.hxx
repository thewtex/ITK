...
::CalculatePrecisionTolerance( LocalInputImageType * inputImage )
{
  ...
  if( typeid(inputImage->GetPixel(index)) == typeid(doubleValue) )
    {
    precisionTolerance = 1000.0 * vcl_pow(2.0,-52) * vcl_pow(2,vcl_floor(vcl_log(calculator->GetMaximum())/vcl_log(2)));
    }
  else if( typeid(inputImage->GetPixel(index)) == typeid(floatValue) )
    {
    precisionTolerance = 1000.0 * vcl_pow(2.0,-23) * vcl_pow(2,vcl_floor(vcl_log(calculator->GetMaximum())/vcl_log(2)));
    }
