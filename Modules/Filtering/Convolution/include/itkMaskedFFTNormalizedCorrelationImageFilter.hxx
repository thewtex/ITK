...
{
/**
  * Functor to post-process the NCC result.
...
::GenerateData()
{
  // Here the main purpose of casting is to copy the images locally and
  // remove their constness since the pixels of these images need to be modified.
  InputImagePointer fixedImage = this->CastImage( this->GetFixedImage() );
  InputImagePointer movingImage = this->CastImage( this->GetMovingImage() );
  InputImagePointer fixedMask = this->CastImage( this->GetFixedImageMask() );
  InputImagePointer movingMask = this->CastImage( this->GetMovingImageMask() );
  ...
  this->PreProcessImageAndMask<InputImageType>(fixedImage,fixedMask);
  this->PreProcessImageAndMask<InputImageType>(rotatedMovingImage,rotatedMovingMask);
...
::CastImage( const InputImageType * inputImage )
{
  // Here the main purpose of casting is to copy the images locally and
  // remove their constness since the pixels of these images need to be modified.
  // Therefore, the casting cannot be run in-place.
...
void
MaskedFFTNormalizedCorrelationImageFilter<TInputImage, TOutputImage>
::PreProcessImageAndMask( LocalInputImageType * inputImage, LocalInputImageType * inputMask )
{
  ...
  inputImage->Graft( multiplier->GetOutput() );
  inputMask->Graft( thresholder->GetOutput() );
}
...
::VerifyInputInformation()
 {
  ...
  if( this->GetFixedImage()->GetLargestPossibleRegion().GetSize() != this->GetFixedImageMask()->GetLargestPossibleRegion().GetSize() )
  {
  ...
  if( this->GetMovingImage()->GetLargestPossibleRegion().GetSize() != this->GetMovingImageMask()->GetLargestPossibleRegion().GetSize() )
  {
...
::GenerateInputRequestedRegion()
{
  ...
  {
    // If inputPtr of fixedMask is NULL, set the region to the fixedImage region instead of the mask.
    inputPtr = InputImageType::New();
    inputPtr->SetRequestedRegion( this->GetFixedImage()->GetLargestPossibleRegion() );
  }
  ...
  {
    // If inputPtr of movingMask is NULL, set the region to the movingImage region instead of the mask.
    inputPtr = InputImageType::New();
    inputPtr->SetRequestedRegion( this->GetMovingImage()->GetLargestPossibleRegion() );
  }
}
