...
::GenerateData()
{
  ...
  if( !m_CanUseDestructiveAlgorithm )
    {
    // If the input is about to be destroyed, there is no need to
    // force FFTW to use a non-destructive algorithm. If it is not
    // released however, we must be careful to not destroy it.
