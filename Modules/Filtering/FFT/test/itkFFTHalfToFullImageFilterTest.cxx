...
int itkFFTHalfToFullImageFilterTest(int argc, char *argv[])
{
  ...
  // full complex image.
  typedef itk::VnlForwardFFTImageFilter< ImageType, ComplexImageType > FFTFilter;
  ...
  if ( halfToFullOutputSize != size )
    {
    std::cerr << "FFTHalfToFullImageFilter did not produce an image of the expected size. "
              << std::endl;
  ...
  ComplexImageType::IndexType conjugateIndex;
  conjugateIndex.Fill( 0 );
  conjugateIndex[0] = static_cast< ComplexImageType::IndexValueType >( fftSize[0] );
  ComplexImageType::SizeType conjugateSize( size );
  conjugateSize[0] -= fftSize[0];
  itk::ImageRegion< ComplexImageType::ImageDimension > conjugateRegion( conjugateIndex,
                                                                        conjugateSize );

  std::cout << "conjugateRegion: " << conjugateRegion << std::endl;
  ...
  for (it.GoToBegin(); !it.IsAtEnd(); ++it)
    ...
    for (unsigned int i = 0; i < ComplexImageType::ImageDimension; ++i)
      ...
      if ( conjugateIndex[i] != 0 )
        {
        index[i] = size[i] - conjugateIndex[i];
        }
      }
