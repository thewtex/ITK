...
int itkFFTWD_RealFFTTest(int, char *[])
{
  ...
  if((test_fft<double,1,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageD1> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCD1> >(SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "FFTWD:double,2 (4,4,4)" << std::endl;
  if((test_fft<double,2,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageD2> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCD2> >(SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "FFTWD:double,3 (4,4,4)" << std::endl;
  if((test_fft<double,3,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageD3> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCD3> >(SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "FFTWD:double,1 (3,5,4)" << std::endl;
  if((test_fft<double,1,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageD1> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCD1> >(SizeOfDimensions2)) != 0)
    rval++;
  std::cerr << "FFTWD:double,2 (3,5,4)" << std::endl;
  if((test_fft<double,2,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageD2> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCD2> >(SizeOfDimensions2)) != 0)
    rval++;
  std::cerr << "FFTWD:double,3 (3,5,4)" << std::endl;
  if((test_fft<double,3,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageD3> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCD3> >(SizeOfDimensions2)) != 0)
    rval++;

  // Exercise the plan rigor methods
  itk::FFTWRealToComplexForwardFFTImageFilter< ImageD3 >::Pointer fft =
    itk::FFTWRealToComplexForwardFFTImageFilter< ImageD3 >::New();
  ...
  itk::FFTWComplexToRealInverseFFTImageFilter< ImageCD3 >::Pointer ifft =
    itk::FFTWComplexToRealInverseFFTImageFilter< ImageCD3 >::New();
