...
int itkFFTWF_RealFFTTest(int argc, char *argv[])
{
  ...
  if((test_fft<float,1,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageF1> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCF1> >(SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "FFTWF:float,2 (4,4,4)" << std::endl;
  if((test_fft<float,2,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageF2> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCF2> >(SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "FFTWF:float,3 (4,4,4)" << std::endl;
  if((test_fft<float,3,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageF3> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCF3> >(SizeOfDimensions1)) != 0)
    rval++;
  std::cerr << "FFTWF:float,1 (3,5,4)" << std::endl;
  if((test_fft<float,1,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageF1> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCF1> >(SizeOfDimensions2)) != 0)
    rval++;
  std::cerr << "FFTWF:float,2 (3,5,4)" << std::endl;
  if((test_fft<float,2,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageF2> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCF2> >(SizeOfDimensions2)) != 0)
    rval++;
  std::cerr << "FFTWF:float,3 (3,5,4)" << std::endl;
  if((test_fft<float,3,
      itk::FFTWRealToComplexForwardFFTImageFilter<ImageF3> ,
      itk::FFTWComplexToRealInverseFFTImageFilter<ImageCF3> >(SizeOfDimensions2)) != 0)
    rval++;

  // Exercise the plan rigor methods
  itk::FFTWRealToComplexForwardFFTImageFilter< ImageF3 >::Pointer fft =
    itk::FFTWRealToComplexForwardFFTImageFilter< ImageF3 >::New();
  ...
  itk::FFTWComplexToRealInverseFFTImageFilter< ImageCF3 >::Pointer ifft =
    itk::FFTWComplexToRealInverseFFTImageFilter< ImageCF3 >::New();
