  ...
  m_WisdomCacheBase("")
{
  ...
  if( !fftw_init_threads() )
    ...
    if( WisdomCacheBaseEnvSet ) //The environment variable overrides application
                                // settings.
      {
      this->m_WisdomCacheBase=envSetPath;
      }
    else if( this->m_WisdomCacheBase.size() < 1 ) //Use home account if nothing
                                                  // specified
      {
#ifdef _WIN32
...
#if defined(ITK_USE_FFTWF)
      {
      // import the wisdom files again to be sure to not erase the wisdom saved
      // in another process
      ImportWisdomFileFloat(cachePath+"f");
...
#if defined(ITK_USE_FFTWF) //Only define if ITK_USE_FFTWF, to avoid compiler
                           // warning
                         path
#endif
                         )
{
  ...
  if ( !_sopen_s( &fd, path.c_str(), _O_RDONLY, _SH_DENYNO, _S_IREAD) )
    ...
    if ( (f = _fdopen(fd, "r") ) != NULL )
      { // strange but seems ok under VC++ not so friendly with checking the
        // return values of affectations
...
#if defined(ITK_USE_FFTWD) //Only define if ITK_USE_FFTWD, to avoid compiler
                           // warning
                          path
#endif
                          )
