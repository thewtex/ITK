...
::SetNonConnectivity( unsigned connect ) throw ( ExceptionObject )
{
  if ( connect < ImageDimension )
    ...
    for ( SizeValueType i = 0, n = neighborhood.Size(); i < n; i++ )
      ...
      if ( connect <= zeros && zeros < ImageDimension )
        {
    ...
    {
    itkExceptionMacro( "Cannot set non-connectivity to " << connect << ", allowed 0.." << ImageDimension - 1 << "." );
    }
...
::GenerateData()
{
  ...
  typedef typename SelectionMapType::PixelType MapPixelType;
  if ( mask.IsNull() )
    {
    ...
    for ( itMask.GoToBegin(), itMap.GoToBegin(); !itMask.IsAtEnd(); ++itMask, ++itMap )
      {
      itMap.Set( static_cast< MapPixelType >( itMask.Get() ) );
      }
    }
