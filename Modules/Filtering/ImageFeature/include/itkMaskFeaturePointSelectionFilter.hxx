  ...
  while ( rit != pointMap.rend() && numberOfPointsInserted < maxNumberPointsToInserted)
    ...
    if ( selectionMap->GetPixel( indexOfPointToPick ) && region.IsInside(indexOfPointToPick) )
      ...
      if ( m_ComputeStructureTensors )
        ...
        Matrix < typename ImageType::SpacePrecisionType, ImageDimension, 1 > gradI; // vector declared as column matrix
        ...
        for ( SizeValueType i = 0; i < gradientItr.Size(); i++ )
          ...
          for ( unsigned j = 0; j < ImageDimension; j++ )
            ...
            const typename ImageType::SpacePrecisionType doubleSpacing = voxelSpacing[ j ] * 2.0;
          ...
          const vnl_matrix< typename ImageType::SpacePrecisionType > tnspose=gradI.GetTranspose();
