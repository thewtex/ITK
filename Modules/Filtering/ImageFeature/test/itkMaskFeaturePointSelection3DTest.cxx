  ...
  try
    {
      ...
      typedef itk::PointSet< itk::Index< 3 >, 3 >                PointSetType;
      typedef itk::MaskFeaturePointSelectionFilter< ImageType,
        ImageType, PointSetType >                                FilterType;
      ...
      for ( int i = 0; i < 50; i++)
        {
          if ( preselectedPoints[i] != selectedPoints[i] )
            {
              std::cout << "ERROR: Selected " << selectedPoints[i]
                        << " which does not match any of the preselected points" << std::endl;
              return EXIT_FAILURE;
            }
        }
    }
