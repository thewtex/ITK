...
::Initialize()
{
  ...
  else
  {
    ...
    for (unsigned int cc = 0; cc < m_ComponentsPerPixel; ++cc)
    {
      ...
      if (m_GaussianKernelSigma[cc] <= m_MinSigma)
      {
        itkExceptionMacro (<< "Gaussian kernel sigma ("
                           << m_GaussianKernelSigma[cc]
                           << ") must be larger than "
                           << m_MinSigma);
      }
    }
...
::EnforceConstraints()
{
  ...
  for (unsigned int cc = 0; cc < m_ComponentsPerPixel; ++cc)
  {
    ...
    itkExceptionMacro( << "Each image component must be nonconstant.  "
                       << "Component " << cc
                       << " has the constant value " << m_ImageMax[cc]
                       << ".\n"; );
    }
  }
    ...
    for (SampleIteratorType sampleIt = inList->Begin(); sampleIt != inList->End(); ++sampleIt)
    {
      ...
      if (fidelityWeight > 0)
      {
        ...
        case Superclass::RICIAN:
        {
          for (unsigned int cc = 0; cc < m_ComponentsPerPixel; ++cc)
          {
            ...
            const RealValueType gradientFidelity
              = (in[cc] * (gOper.ModifiedBesselI1(alpha) / gOper.ModifiedBesselI0(alpha)) - out[cc])
                / m_NoiseSigmaSquared[cc];
            const RealValueType stepSizeFidelity = m_NoiseSigmaSquared[cc];
            // update
            result[cc] += fidelityWeight * ( stepSizeFidelity * gradientFidelity );
            // ensure that the result is non-negative
            result[cc] = vnl_math_max(result[cc], static_cast<RealValueType>(0.0));
          }
          break;
        }
        case Superclass::POISSON:
        {
          for (unsigned int cc = 0; cc < m_ComponentsPerPixel; ++cc)
          {
            const RealValueType gradientFidelity = (in[cc] - out[cc]) / (out[cc] + 0.00001);
            // prevent large unstable updates when out[cc] less than 1
            const RealValueType stepSizeFidelity
              = vnl_math_min(out[cc], static_cast<PixelValueType>(0.99999)) + 0.00001;
            // update
            result[cc] += fidelityWeight * ( stepSizeFidelity * gradientFidelity );
            // ensure that the result is positive
            result[cc] = vnl_math_max(result[cc], static_cast<RealValueType>(0.00001));
          }
          break;
        }
        default:
