...
::Initialize() throw(DataObjectError)
{
  ...
  m_IntensityRescaleInvFactor.Fill(1.0); // default: updated in InitializeKernelSigma()

  if (this->GetDoKernelBandwidthEstimation())
  {
    // kernel sigma initialization for automatic sigma estimation
    this->InitializeKernelSigma(this->GetInput());
  }
  else
  {
...
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>
::InitializeKernelSigma(const Image<RGBPixel<PixelValueType>, ImageDimension>* imgPtr)
{
  this->ArrayInitializeKernelSigma(imgPtr);
}

template <class TInputImage, class TOutputImage>
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>
::InitializeKernelSigma(const Image<RGBAPixel<PixelValueType>, ImageDimension>* imgPtr)
{
  this->ArrayInitializeKernelSigma(imgPtr);
}
...
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>
::InitializeKernelSigma(const Image<Vector<PixelValueType,PixelType::Dimension>,
...
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>
::InitializeKernelSigma(const Image<FixedArray<PixelValueType,PixelType::Dimension>,
                        ImageDimension>* imgPtr)
{
  this->ArrayInitializeKernelSigma(imgPtr);
}
...
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>
::InitializeKernelSigma(const Image<Array<PixelValueType>, ImageDimension>* imgPtr)
{
  this->ArrayInitializeKernelSigma(imgPtr);
}
...
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>
::InitializeKernelSigma(const TImage* imgPtr)
{
  ...
  itkAssertOrThrowMacro (maxVal > minVal, "Image can't be all constant");
  ...
  m_GaussianKernelSigma[0] = 10.0 / m_IntensityRescaleInvFactor[0];
  {
    std::ostringstream msg;
    msg << "Image Intensity range: [" << minVal << "," << maxVal << "], "
        << "IntensityRescaleInvFactor: " << m_IntensityRescaleInvFactor[0] << " , "
        << "GaussianKernelSigma initialized to: " << m_GaussianKernelSigma[0] << std::endl;
    this->m_Logger->Write(itk::LoggerBase::INFO, msg.str());
  }
  //
  m_GaussianKernelSigma[0] *= m_SigmaMultiplicationFactor;
  {
    std::ostringstream msg;
    msg << "GaussianKernelSigma after applying multiplicatio factor: "
        << m_GaussianKernelSigma[0] << std::endl;
    this->m_Logger->Write(itk::LoggerBase::INFO, msg.str());
  }
}
...
void
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>
::ArrayInitializeKernelSigma(const InputImageType* imgPtr)
{
  ...
  for (unsigned int cc = 0; cc < m_ComponentsPerPixel; ++cc)
  {
    ...
    const PixelValueType minVal = minmax->GetMinimum();
    const PixelValueType maxVal = minmax->GetMaximum();
    itkAssertOrThrowMacro (maxVal > minVal, "Image can't be all constant");
    ...
    m_GaussianKernelSigma[cc] = 10.0 / m_IntensityRescaleInvFactor[cc];
    {
      ...
      msg << "For component " << cc << ", "
          ...
          << static_cast<float>(minVal) << "," << static_cast<float>(maxVal) << "], "
  ...
  for (fIt = faceList.begin(); fIt != faceList.end(); ++fIt)
  {
    ...
    for (SampleIteratorType sampleIt = inList->Begin(); sampleIt != inList->End(); ++sampleIt)
    {
      ...
      if (fidelityWeight > 0)
      {
        ...
        {
          itkExceptionMacro(<< "RICIAN noise model isn't currently implemented!");
        }
        case Superclass::POISSON:
        {
          itkExceptionMacro(<< "POISSON noise model isn't currently implemented!");
          break;
        }
        default:
        {
