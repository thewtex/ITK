...
public PatchBasedDenoisingBaseImageFilter<TInputImage, TOutputImage>
{
 ...
 protected:
   ...
   *
   * The enable if idom is used to over this method for scalar and
   * multi-component type. By exploinging the numeric trait ValueType
   * is the per-element type for multi-component type and is
   * differnent then the parameterize type, the bracket operator is
   * used only for these multi-component type.
  */
  template< typename T>
    typename EnableIfC<
