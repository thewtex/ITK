...
int itkParametricBlindLeastSquaresDeconvolutionImageFilterTest(int argc, char* argv[])
{
  ...
  // Create a parametric Gaussian image source for the kernel
  ...
  if ( vcl_abs( kernelSource->GetParameters()[0] - 3.42181 ) > 1e-5 )
    {
    std::cerr << "Kernel parameter[0] should have been 3.42181, was "
              << kernelSource->GetParameters()[0] << "." << std::endl;
    return EXIT_FAILURE;
    }
  if ( vcl_abs( kernelSource->GetParameters()[1] - 3.53356 ) > 1e-5 )
    {
    std::cerr << "Kernel parameter[1] should have been 3.40992, was "
              << kernelSource->GetParameters()[0] << "." << std::endl;
    return EXIT_FAILURE;
    }
