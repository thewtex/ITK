...
itkDanielssonDistanceMapImageFilterTest(int, char* [] )
{
  ...
  // Create a large 3D image with a small foreground object.  The foreground is
  // denoted by a pixel value of 0,
  // and the background by a non-zero pixel value.  This will test speedups to
  // the code that ignore all background
  ...
  // Set a few pixels in the middle of the image to 0.  These are the foreground
  // pixels for which the distance will be solved.
