                       ...
                       ThreadIdType threadId)
{
  ...
  // This variable provides the amount by which to divide the dimensionless
  // index in order to get the index for each dimension.
  ...
  // It must be divided by each dimension size in order to get the index for
  // that dimension.
  // The result of this division is the offsetIndex, which is the index offset
  // relative to the region of this thread.
  // The true pixel location (idx) is provided by the sum of the offsetIndex and
  // the startIndex.
  ...
  for ( InputSizeValueType n = 0; n < tempRow; n++ )
    ...
    for ( unsigned int d = m_CurrentDimension + 1;
      ...
      idx[d %
          InputImageDimension] =
        offsetIndex[d %
                    InputImageDimension] + static_cast< OutputIndexValueType >( startIndex[d % InputImageDimension] );
