                       ...
                       ThreadIdType threadId)
{
  ...
  while ( !inIt.IsAtEnd() )
    ...
    if ( inIt.Get() > m_LevelSetValue )
      ...
      {
      outIt.Set( m_FarValue * static_cast< PixelType >( -1. ) );
      }
    else
      {
                           ...
                           ThreadIdType threadId)
{
  ...
  InputNeighbordIteratorType inNeigIt( radius_in, inputPtr,
                                      inputPtr->GetRequestedRegion() );
  OutputNeighborhoodIteratorType outNeigIt( radius_out, outputPtr,
  ...
  for ( unsigned int n = 0; n < ImageDimension; n++ )
    ...
    if ( sign != neigh_sign )
      ...
      if ( norm > NumericTraits< PixelType >::min() )
        ...
        PixelType val0_new = val0 * val;
        PixelType val1_new = val1 * val;

        if ( vcl_fabs( val0_new ) < vcl_fabs( outNeigIt.GetNext(n, 0) ) )
          {
          outNeigIt.SetNext( n, 0, static_cast< PixelType >( val0_new ) );
          }
        if ( vcl_fabs( val1_new ) < vcl_fabs( outNeigIt.GetNext(n, 1) ) )
          {
