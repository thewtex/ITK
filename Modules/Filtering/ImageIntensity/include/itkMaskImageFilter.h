...
private:
  ...
  {
    if ( this->GetFunctor().GetOutsideValue().GetSize() == 0 )
      {
      // Initialize the vector to be the vector length of the output
      // image and filled with zeros.
      VariableLengthVector< TValue > outsideValue;
      outsideValue.SetSize( this->GetOutput()->GetVectorLength() );
      outsideValue.Fill( NumericTraits< TValue >::Zero );
      this->GetFunctor().SetOutsideValue( outsideValue );
      }
