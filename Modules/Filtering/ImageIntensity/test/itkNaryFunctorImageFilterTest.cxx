...
namespace itk
{
  ...
  public:
    ...
    inline TOutput operator()(const TInput & values) const
    {
      return static_cast< TOutput >( values[0] * values[1] * values[2] * values[3] );
    }
  };
  }
...
int itkNaryFunctorImageFilterTest(int, char*[])
{
  typedef float                                          PixelType;
  typedef itk::Image< PixelType, 3 >                     ImageType;
  typedef std::vector< PixelType >                       ValueArrayType;
  typedef itk::Functor::MultHelper< ValueArrayType,
  ...
  ImageType::PixelType expectedValue = 3.0f * 4.0f * 5.0f * 6.0f;
  while ( !outIt.IsAtEnd() )
    {
