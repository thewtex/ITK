...
::GenerateData()
{
  ...
  for ( int slice = requestedIndex[m_Dimension]; slice < sliceRangeMax; ++slice )
    ...
    for ( unsigned int i = 0; i < numberOfIndexedInputs; i++ )
      ...
      InputIteratorType inputIterator( inputs[i], currentRegion );
    ...
    for ( unsigned int i = 0; i < numberOfIndexedOutputs; i++ )
      ...
      InputIteratorType inputIterator( outputFilterOutputs[i], internalRegion );
      inputIterator.GoToBegin();

      OutputIteratorType outputIterator( outputs[i], currentRegion );
      outputIterator.GoToBegin();
