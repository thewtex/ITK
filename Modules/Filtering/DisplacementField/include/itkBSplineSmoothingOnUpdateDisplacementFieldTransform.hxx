...
::BSplineSmoothDisplacementField()
{
  ...
  typename DisplacementFieldType::IndexType startIndex = bufferedRegion.GetIndex();
  typename DisplacementFieldType::SizeType size = bufferedRegion.GetSize();
  unsigned int numberOfPixels = bufferedRegion.GetNumberOfPixels();
  ...
  memcpy( displacementField->GetBufferPointer(), bspliner->GetOutput()->GetBufferPointer(),
