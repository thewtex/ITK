...
::BSplineSmoothDisplacementField()
{
  ...
  typename DisplacementFieldType::SizeType size = displacementField->GetBufferedRegion().GetSize();
  typename DisplacementFieldType::IndexType startIndex = displacementField->GetBufferedRegion().GetIndex();

  SizeValueType numberOfPixels = 1;
  for( unsigned int d = 0; d < Dimension; d++ )
    {
    numberOfPixels *= size[d];
    }
  ...
  importer->SetImportPointer( displacementField->GetBufferPointer(), numberOfPixels, false );
  ...
  ImageRegionIterator<DisplacementFieldType> ItF( displacementField, displacementField->GetBufferedRegion() );
  ImageRegionIterator<DisplacementFieldType> ItB( bspliner->GetOutput(), bspliner->GetOutput()->GetBufferedRegion() );
  for( ItF.GoToBegin(), ItB.GoToBegin(); !ItF.IsAtEnd(); ++ItF, ++ItB )
    {
    ItF.Set( ItB.Get() );
    }
}
