...
public:
  ...
  inline TValueType Evaluate(const TValueType & u) const
  {
    ...
    TValueType envelope = vcl_exp(static_cast< TValueType >(-0.5) * parameter);
    TValueType phase = static_cast< TValueType >(2.0 * vnl_math::pi) * this->m_Frequency * u
                   + this->m_PhaseOffset;
...
protected:
  GaborKernelFunction()
    ...
    this->m_Sigma = NumericTraits< TValueType >::One;
    this->m_Frequency = static_cast<TValueType>(0.4);
    this->m_PhaseOffset = NumericTraits< TValueType >::Zero;
    }
  ~GaborKernelFunction() {};
