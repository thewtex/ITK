 ...
 * displayed at the bottomw of each render window. If ShareCamera is
 * true, a single vtkCamera will be used for each render window if
 * (default is false). Each image is rescaled to have a range between
 * 0 and 255. Currently, the size of each render window is
 * fixed at 300,300 and the text size for descriptions is fixed at
 * 10. The Visualize method displays the render windows and starts a
 * vtk interactor. The layout and background color of each render
 * window is fixed. The optional boolean for the constructor, if false,
 * bypasses the interacotr. This is useful for running tests.
 *
 * Typical usage:
 *  QuickView viewer;
 *  viewer.AddImage(someFilter->GetOurtput().
 *                  true (to flip image) or false.
 *                  "text to display with the image");
 *
 *  viewer.AddRGBImage(someFilter->GetOuttput().
 *                     true (to flip image) or false.
 *                     "text to display with the image");
 *
 */
...
public:
  ...
  /** Provide the image format to bse used when taking snapshot */
