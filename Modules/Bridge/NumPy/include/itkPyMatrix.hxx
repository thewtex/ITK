 ...
 *
 *=========================================================================*/
#ifndef itkPyMatrix_hxx
#define itkPyMatrix_hxx

#include "itkPyMatrix.h"
#include <stdexcept>

namespace itk
{

template<typename TElement, unsigned int NRows, unsigned int NColumns>
PyObject *
PyMatrix<TElement, NRows, NColumns>
::_GetArrayViewFromMatrix( MatrixType * matrix )
{
  ...
  auto vnlMatrix = matrix->GetVnlMatrix();

  DataType *buffer =  vnlMatrix.data_block();

  void * matrixBuffer = (void *)( buffer );

  // Computing the length of data
  Py_ssize_t len = vnlMatrix.size();
  len *= elementSize;
...
template<typename TElement, unsigned int NRows, unsigned int NColumns>
const typename PyMatrix<TElement, NRows, NColumns>::MatrixType
PyMatrix<TElement, NRows, NColumns>
::_GetMatrixFromArray( PyObject *arr, PyObject *shape )
{
  PyObject *                  obj           = NULL;
  ...
  const DataType * data = (const DataType *)buffer;
  typename MatrixType::InternalMatrixType vnlMatrix(data);
  MatrixType output(vnlMatrix);
  PyBuffer_Release(&pyBuffer);
