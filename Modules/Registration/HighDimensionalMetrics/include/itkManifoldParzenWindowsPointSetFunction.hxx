...
::SetInputPointSet( const InputPointSetType * ptr )
{
  ...
  this->m_PointsLocator->SetPoints( const_cast<PointsContainer *>(
    this->GetInputPointSet()->GetPoints() ) );
  ...
  while( It != this->GetInputPointSet()->GetPoints()->End() )
    ...
    if( this->m_CovarianceKNeighborhood > 0
      ...
      this->m_PointsLocator->Search(
        point, this->m_CovarianceKNeighborhood, neighbors );
