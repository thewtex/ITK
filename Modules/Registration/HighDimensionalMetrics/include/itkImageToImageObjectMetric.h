...
protected:
   ...
   *    ThreadedImageRegionPartitioner, ThreadedIndexedContainerPartitioner, etc
   *  \tparam TUnused This is a dummy template parameter to make the compiler
   *    use partial template specialization instead of explicit template
   *    specialization, which will fail because you get the error:
   *    "enclosing class templates are not explicitly specialized "
   *
   * \ingroup ITKHighDimensionalMetrics
   *  */
  template< class TDomainPartitioner, int TUnused = 0 >
  class GetValueAndDerivativeThreader
    : public DomainThreader< TDomainPartitioner, ImageToImageObjectMetric< TFixedImage, TMovingImage, TVirtualImage > >
  {
  public:
    /** Standard class typedefs. */
    typedef GetValueAndDerivativeThreader                                  Self;
    typedef DomainThreader< TDomainPartitioner, ImageToImageObjectMetric< TFixedImage, TMovingImage, TVirtualImage > >
                                                                           Superclass;
    typedef SmartPointer< Self >                                           Pointer;
    typedef SmartPointer< const Self >                                     ConstPointer;
  ...
  //template<>
  //class GetValueAndDerivativeThreader< ThreadedImageRegionPartitioner< VirtualImageDimension > >
  //{
  //protected:
    /** Go through each point in the virtual image domain, get its index and
     * point location, then call \c ProcessVirtualPoint on the point. */
    //virtual void ThreadedExecution( const DomainType& subdomain,
                                    //const ThreadIdType threadId );
  //}

  //template< ThreadedIndexedContainerPartitioner TDomainPartitioner >
  //class GetValueAndDerivativeThreader
    //: public DomainThreader< TDomainPartitioner >, ImageToImageObjectMetric<TFixedImage, TMovingImage, TVirtualImage> >
  //{
  //protected:
  ...
  typename GetValueAndDerivativeThreader< ThreadedImageRegionPartitioner< VirtualImageDimension > >::Pointer m_DenseGetValueAndDerivativeThreader;
  /* A SparseGetValueAndDerivativeThreader
   * Derived classes must define this class and assign it in their constructor
   * if threaded processing in GetValueAndDerivative is performed. */
  typename GetValueAndDerivativeThreader< ThreadedIndexedContainerPartitioner >::Pointer m_SparseGetValueAndDerivativeThreader;
