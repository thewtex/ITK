...
PerformSimpleImageRegistration( int argc, char *argv[] )
{
  if( argc < 7 )
    {
    std::cout << argv[0] <<
    " pixelType imageDimension fixedImage movingImage outputImage numberOfAffineIterations numberOfDeformableIterations"
              << std::endl;
    exit( 1 );
    }
  ...
  typename DisplacementFieldTransformType::Pointer fieldTransform =
    const_cast<DisplacementFieldTransformType *>( displacementFieldSimple->GetOutput()->Get() );
  ...
  typedef itk::GaussianSmoothingOnUpdateDisplacementFieldTransformParametersAdaptor<DisplacementFieldTransformType>
  ...
  compositeTransform->AddTransform( const_cast<DisplacementFieldTransformType *>( displacementFieldSimple->GetOutput()->
                                                                                  Get() ) );
