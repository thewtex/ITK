...
::ImageRegistrationMethodv4()
{
  ...
  this->m_OutputTransform = Self::GetTransformOutput();
...
::AllocateOutputs()
{
  ...
  if ( decoratedInitialTransform )
    ...
    if (initialTransform && initialAsOutputTransform)
      {
      if (  !this->GetInPlace() )
        {
        // manually graft the input decorator to the output
        this->m_OutputTransform = const_cast<OutputTransformType*>( initialAsOutputTransform );
        decoratedOutputTransform->Set( this->m_OutputTransform );

        const_cast< DecoratedInitialTransformType *>( decoratedInitialTransform )->Set(ITK_NULLPTR);

        // successful in-place
        itkDebugMacro("inplace allocattion");
        return;
        }
      ...
      itkDebugMacro("clone copy allocation");
      return;
      }
    }
  ...
  itkDebugMacro("fallback allocattion");
  this->m_OutputTransform = this->GetTransformOutput();
