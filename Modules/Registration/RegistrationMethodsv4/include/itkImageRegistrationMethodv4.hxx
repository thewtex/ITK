...
::InitializeCenterOfLinearOutputTransform()
{
  ...
  SizeValueType optimalIndex = numberOfTransforms - 1;
  bool optimalIndexFound = false;

  for( SizeValueType i = numberOfTransforms - 1; i >= 0; i-- )
    {
  ...
  typename CompositeTransformType::InverseTransformBasePointer inverseTransform =
    this->m_CompositeTransform->GetInverseTransform();
  ...
  if( ! inverseTransform )
    ...
    {
    typename TTransform::InputPointType initializedCenter = inverseTransform->TransformPoint( center );
    matrixOffsetOutputTransform->SetCenter( initializedCenter );
    }
}
