...
::StartOptimization()
{
  ...
  while( this->m_CurrentIteration++ < this->m_NumberOfIterationsPerLevel[this->m_CurrentLevel] && !this->m_IsConverged )
    ...
    if( fixedImageMask )
      {
      typename ImageMaskSpatialObjectType::Pointer imageMaskPtr = dynamic_cast<ImageMaskSpatialObjectType *>( const_cast<FixedImageMaskType *>( fixedImageMask.GetPointer() ) );
      if( imageMaskPtr.IsNotNull() )
        {
        maskImage = imageMaskPtr->GetImage();
        }
      else
        {
