...
::StartOptimization()
{
  ...
  while( iteration++ < this->m_NumberOfIterationsPerLevel[this->m_CurrentLevel] && !isConverged )
    ...
    for( IndexValueType timePoint = 0; timePoint < numberOfTimePoints; timePoint++ )
      ...
      numberOfMovingIntegrationSteps = numberOfFixedIntegrationSteps = numberOfIntegrationSteps ;
    ...
    if( convergenceValue < this->m_ConvergenceThreshold || iteration == this->m_NumberOfIterationsPerLevel[this->m_CurrentLevel] )
      ...
      RealType tvnormlddmm = NumericTraits< RealType >::Zero;
      ...
      for( ItV.GoToBegin(); !ItV.IsAtEnd(); ++ItV)
        ...
        RealType locnormlddmm = NumericTraits< RealType >::Zero;
        ...
        if ( ! onBoundary )
          ...
          for (unsigned int i = 0; i < ImageDimension + 1; i++ )
            ...
            RealType temp = vec.GetNorm();
            locnorm += temp;
            if ( i < ImageDimension ) locnormlddmm += temp;
            }
          }
        tvnorm += ( locnorm / static_cast< RealType > ( ImageDimension + 1 ) );
        tvnormlddmm += ( locnormlddmm / static_cast< RealType > ( ImageDimension ) );
        }
      tvnorm /= static_cast< RealType > ( numberOfPixels );
      tvnormlddmm /= static_cast< RealType > ( numberOfPixels );
      std::cout << "VSTN," << tvnorm << " VSN, " <<  tvnormlddmm << std::endl;
      }
    }
}
