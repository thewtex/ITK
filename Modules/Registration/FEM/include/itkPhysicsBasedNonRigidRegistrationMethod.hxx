...
void
PhysicsBasedNonRigidRegistrationMethod<TFixedImage, TMovingImage, TMaskImage, TMesh, TDeformationField>
::CreateWriteDeformedMovingImage(std::string deformedImageName,bool bInterpolate)
{
  ...
  typename MovingImageType::Pointer pDeformedImage = MovingImageType::New();
  ...
  if(bInterpolate)
    {
    std::cout << "Iterpolation will used." << std::endl;
    }
  else
    {
    std::cout << "No iterpolation will used." << std::endl;
    }
  ...
  for(it = this->m_FEMFilter->GetFEMSolver()->GetOutput()->GetElementContainer()->Begin(); it != this->m_FEMFilter->GetFEMSolver()->GetOutput()->GetElementContainer()->End(); ++it)
    ...
    for( iter.GoToBegin(); !iter.IsAtEnd(); ++iter )
      ...
      if(elem->GetLocalFromGlobalCoordinates(vGlobal,vLocal))
        ...
        // No Interpolation
          ...
          {
          // Point inside the image buffer
          typename MovingImageType::IndexType index1;
          if( this->GetMovingImage()->TransformPhysicalPointToIndex(point,index1) )
            {
            // Get the pixel value without intepolation
            PixelValue = this->GetMovingImage()->GetPixel(index1);
            }
          }
    ...
    if(!deformedImageName.empty())
      {
      std::cout << "Saving Deformed Image at  : " << deformedImageName << std::endl;
      typedef itk::ImageFileWriter<MovingImageType> WriterType;
      typename WriterType::Pointer deformedImageWriter = WriterType::New();
      deformedImageWriter->SetFileName(deformedImageName);
      deformedImageWriter->SetInput(pDeformedImage);
