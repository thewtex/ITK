...
FEMRegistrationFilter<TMovingImage, TFixedImage, TFemObject>::SmoothDisplacementField()
{
  ...
  typename FieldType::Pointer m_TempField = FieldType::New();
  m_TempField->SetOrigin( m_Field->GetOrigin() );
  m_TempField->SetSpacing( m_Field->GetSpacing() );
  m_TempField->SetDirection( m_Field->GetDirection() );
  m_TempField->SetRegions( m_Field->GetLargestPossibleRegion() );
  m_TempField->Allocate();

  typedef typename VectorType::ValueType                                ScalarType;
  typedef GaussianOperator< ScalarType, ImageDimension >                OperatorType;
  typedef VectorNeighborhoodOperatorImageFilter< FieldType, FieldType > SmootherType;

  OperatorType *oper = new OperatorType;
  typename SmootherType::Pointer smoother = SmootherType::New();

  typedef typename FieldType::PixelContainerPointer
  ...
  for ( unsigned int j = 0; j < ImageDimension; j++ )
  {
    ...
    if ( j < ImageDimension - 1 )
    {
      ...
      smoother->GraftOutput( m_Field );
      m_Field->SetPixelContainer(swapPtr);
      smoother->Modified();
    }
  }

  // graft the output back to this filter
  m_TempField->SetPixelContainer( m_Field->GetPixelContainer() );
