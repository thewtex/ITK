...
int RunTest(testImageType* fixed, testImageType* moving, FieldType* initField,
            FieldType* outField, SolverType* solver, unsigned int metricType)
{
...
#ifdef WRITE_DISP_TO_FILE
  // Write to vector image
  typedef itk::ImageFileWriter<FieldType> FieldWriterType;
  FieldWriterType::Pointer fieldWriter = FieldWriterType::New();
  char outFilename[256];
  sprintf(outFilename, "%sForcesWithMetric%u.vtk", filenamePrefix, metricType);
  fieldWriter->SetFileName(outFilename);
...
#ifdef WRITE_DISP_TO_FILE
  // Write to vector image
  sprintf(outFilename, "%sNodalForcesWithMetric%u.vtk", filenamePrefix, metricType);
  fieldWriter->SetFileName(outFilename);
  fieldWriter->SetInput(outField);
  fieldWriter->Update();
#endif
...
int itkFEMFiniteDifferenceFunctionLoadTest(int argc, char* argv[])
{
  ...
  writer->SetFileName((filename + "Fixed.mhd").c_str());
  writer->Update();
#endif
  ...
  for (unsigned int i = 0; i < 4; i++)
    {
    if (RunTest(fixed, moving, initField, outField, solver, i) != EXIT_SUCCESS)
      {
