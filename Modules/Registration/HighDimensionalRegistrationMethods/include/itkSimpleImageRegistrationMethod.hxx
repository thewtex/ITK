...
::SimpleImageRegistrationMethod()
{
  ...
  this->m_NumberOfIterationsPerLevel.SetSize( this->m_NumberOfLevels );
  this->m_NumberOfIterationsPerLevel[0] = 1000;
  this->m_NumberOfIterationsPerLevel[1] = 1000;
  this->m_NumberOfIterationsPerLevel[2] = 1000;

  this->m_ShrinkFactorsPerLevel.SetSize( this->m_NumberOfLevels );
  this->m_ShrinkFactorsPerLevel[0] = 3;
  ...
  this->m_SmoothingSigmasPerLevel[0] = 4;
  this->m_SmoothingSigmasPerLevel[1] = 2;
  this->m_SmoothingSigmasPerLevel[2] = 1;

  this->m_LearningRatesPerLevel.SetSize( this->m_NumberOfLevels );
  this->m_LearningRatesPerLevel[0] = 10;
  this->m_LearningRatesPerLevel[1] = 10;
  this->m_LearningRatesPerLevel[2] = 10;
  ...
  this->m_ScalesEstimator = scalesEstimator;
}
...
::InitializeRegistrationAtEachLevel( const SizeValueType level )
{
  ...
  // Update the optimizer and (optionally) estimate the scales

  typename OptimizerType::ScalesType scales( this->m_Transform->GetNumberOfLocalParameters() );
  scales.Fill( 1.0 );
  if( this->m_ScalesEstimator )
    {
    this->m_ScalesEstimator->SetMetric( this->m_Metric );
    this->m_ScalesEstimator->SetTransformForward( true );
    this->m_ScalesEstimator->EstimateScales( scales );
    }

  this->m_Optimizer->SetMetric( this->m_Metric );
  this->m_Optimizer->SetLearningRate( this->m_LearningRatesPerLevel[level] );
  this->m_Optimizer->SetNumberOfIterations( this->m_NumberOfIterationsPerLevel[level] );
...
::SetNumberOfLevels( const SizeValueType numberOfLevels )
{
  if( this->m_NumberOfLevels != numberOfLevels )
    ...
    this->m_NumberOfIterationsPerLevel.SetSize( this->m_NumberOfLevels );
    this->m_NumberOfIterationsPerLevel.Fill( 100 );

    this->m_LearningRatesPerLevel.SetSize( this->m_NumberOfLevels );
    this->m_LearningRatesPerLevel.Fill( 1.0 );
...
//      }
  ...
  os << indent << "Learning rates: " << this->m_LearningRatesPerLevel << std::endl;
}
