...
::SetRequiredFixedParameters( const ParametersType fixedParameters )
{
  ...
  for( unsigned int di = 0; di < SpaceDimension; di++ )
    {
    for( unsigned int dj = 0; dj < SpaceDimension; dj++ )
      {
  ...
  for( unsigned int i = 0; i < SpaceDimension; i++ )
    {
    this->m_RequiredTransformDomainMeshSize[i] = this->m_RequiredFixedParameters[i] - SplineOrder;
    }
...
::UpdateRequiredFixedParameters()
{
  ...
  for( unsigned int i = 0; i < SpaceDimension; i++ )
    {
    this->m_RequiredFixedParameters[i] = this->m_RequiredTransformDomainMeshSize[i] + SplineOrder;
    }
