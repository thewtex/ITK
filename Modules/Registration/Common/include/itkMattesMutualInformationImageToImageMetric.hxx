...
throw ( ExceptionObject )
{
  ...
  this->Superclass::MultiThreadingInitialize();
    ...
    while( !fi.IsAtEnd() )
      ...
      if( this->m_FixedImageMask.IsNull()  // A null mask implies entire space
                                           // is to be used.
      ...
      while( !mi.IsAtEnd() )
        ...
        if( this->m_MovingImageMask.IsNull()  // A null mask implies entire
                                              // space is to be used.
  ...
  if( this->m_UseExplicitPDFDerivatives )
    ...
    this->m_PRatioArray.SetSize(0, 0);          // Not needed if
                                                //
                                                // this->m_UseExplicitPDFDerivatives
    ...
    for( ThreadIdType threadID = 0; threadID < this->m_NumberOfThreads; ++threadID )
      ...
      this->m_MMIMetricPerThreadVariables[threadID].JointPDFDerivatives = NULL;    //
                                                                                   ...
                                                                                   // if
                                                                                   //
                                                                                   // this->m_UseExplicitPDFDerivatives=false
      }
...
::GetValue(const ParametersType & parameters) const
{
  ...
  //NOTE:  Since the m_ThreaderFixedImageMarginalPDF is the sum of mass in the
  // fixed image dimension, accumulating these
  //       values gives the same answer as computing the the sum of individual
  // values over the the entire histogram.
  //       IMPORTANT NOTICE:  THIS MAKES AN ASSUMPTION OF CONSERVATION OF MASS
  // OF THE BSPLINE SMOOTHING.
  //       The sum of all the values should equal the number of samples being
  // used, since each sample contributes
