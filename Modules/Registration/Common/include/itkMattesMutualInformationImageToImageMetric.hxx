...
::MattesMutualInformationImageToImageMetric() :
  ...
  m_MMIMetricPerThreadVariables(0),
...
::~MattesMutualInformationImageToImageMetric()
{
  this->m_MMIMetricPerThreadVariables.clear();
}
...
::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  if( !this->m_MMIMetricPerThreadVariables.empty() && this->m_MMIMetricPerThreadVariables[0].JointPDF.IsNotNull() )
    {
    os << indent << "JointPDF: ";
    os << this->m_MMIMetricPerThreadVariables[0].JointPDF << std::endl;
    }
  if( !this->m_MMIMetricPerThreadVariables.empty() && this->m_MMIMetricPerThreadVariables[0].JointPDFDerivatives.IsNotNull() )
    {
...
throw ( ExceptionObject )
{
  ...
  this->m_MMIMetricPerThreadVariables.clear();
  this->m_MMIMetricPerThreadVariables.resize( this->m_NumberOfThreads );
  this->m_FixedImageMarginalPDFPerThread.clear();
  this->m_FixedImageMarginalPDFPerThread.resize( this->m_NumberOfThreads );
                           ...
                           bool withinSampleThread) const
{
  ...
  this->m_FixedImageMarginalPDFPerThread[threadID] = MarginalPDFType(m_NumberOfHistogramBins, 0.0F);
}
                              ...
                              double movingImageValue) const
{
  ...
  this->m_FixedImageMarginalPDFPerThread[threadID][fixedImageParzenWindowIndex] += 1;
                             ...
                             bool itkNotUsed(withinSampleThread) ) const
{
  ...
  for( int i = this->m_MMIMetricPerThreadVariables[threadID].JointPDFStartBin; i <= this->m_MMIMetricPerThreadVariables[threadID].JointPDFEndBin; i++ )
    {
    PDFValueType PDFacc = this->m_FixedImageMarginalPDFPerThread[0][i];
    for( unsigned int t = 1; t < this->m_NumberOfThreads; t++ )
      {
      PDFacc += this->m_FixedImageMarginalPDFPerThread[t][i];
      }
    this->m_FixedImageMarginalPDFPerThread[0][i]  = PDFacc;
    }
...
::GetValue(const ParametersType & parameters) const
{
  ...
  //NOTE:  Since the m_FixedImageMarginalPDFPerThread[i] is the sum of mass in the fixed image dimension, accumulating these
  ...
  for( unsigned int i = 0; i < this->m_NumberOfHistogramBins; i++ )
    {
    totalMassOfPDF += this->m_FixedImageMarginalPDFPerThread[0][i];
    }
  ...
  for( unsigned int bin = 0; bin < this->m_NumberOfHistogramBins; bin++ )
    {
    this->m_FixedImageMarginalPDFPerThread[0][bin] /= totalMassOfPDF;
    }
  ...
  for( unsigned int fixedIndex = 0;
       ...
       ++fixedIndex )
    {
    const PDFValueType fixedImagePDFValue = this->m_FixedImageMarginalPDFPerThread[0][fixedIndex];
                        ...
                        DerivativeType & derivative) const
{
  ...
  if( !( this->m_UseExplicitPDFDerivatives ) )
    ...
    for( unsigned int t = 1; t < this->m_NumberOfThreads; t++ )
      {
      DerivativeType const * const source = &( this->m_MMIMetricPerThreadVariables[t].MetricDerivative );
