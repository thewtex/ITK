...
int itkKappaStatisticImageToImageMetricTest(int, char* [] )
{
  ...
  fixedImage->Allocate(true); // 'true' initializes buffer to zero
  fixedImage->Update();
  ...
  movingImage->Allocate(true); // 'true' initializes buffer to zero
  movingImage->Update();
  ...
  xGradImage->Allocate(true); // 'true' initializes buffer to zero
  xGradImage->Update();

  DoubleImage2DType::Pointer yGradImage = DoubleImage2DType::New();
  yGradImage->SetRegions(imageSize);
  yGradImage->Allocate(true); // 'true' initializes buffer to zero
  yGradImage->Update();
