...
int itkBlockMatchingImageFilterTest( int argc, char * argv[] )
{
  if( argc < 2 )
    ...
    std::cerr << " itkitkBlockMatchingImageFilterTest inputImageFile outputImageFile [Mask File] ";
    return EXIT_FAILURE;
    }
  ...
  typedef FilterType::OutputPointSetTypePixelType                 PointSetPixelType;
...
//  InputImageType::Pointer dupImage = duplicator->GetOutput();
  ...
  BMFilter->SetFloatingImage( reader->GetOutput() );
  BMFilter->SetFeaturePoints( filter->GetOutput() );

  // parameters (all optional)
  s.Fill( 2 );
  BMFilter->SetBlockHalfSize( s );
  s.Fill( 7 );
  BMFilter->SetBlockHalfWindow( s );
  s.Fill( 1 );
  BMFilter->SetBlockStep( s );

  // off by default and should be off in most cases
  BMFilter->CheckBoundaryOn();
