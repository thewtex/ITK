...
metric->m_doInMovingSpaceDebug = true;
  metric->Initialize();
  ...
  for( unsigned int n=0; n < numberOfPoints; n++ )
    {
    PointType transformedPoint = metric->GetMovingTransform()->TransformPoint( movingPoints->GetPoint( n ) );
    PointType difference;
    difference[0] = transformedPoint[0] - fixedPoints->GetPoint( n )[0];
    difference[1] = transformedPoint[1] - fixedPoints->GetPoint( n )[1];
    std::cout << fixedPoints->GetPoint( n ) << "\t" << movingPoints->GetPoint( n )
              << "\t" << transformedPoint << "\t" << difference << std::endl;
