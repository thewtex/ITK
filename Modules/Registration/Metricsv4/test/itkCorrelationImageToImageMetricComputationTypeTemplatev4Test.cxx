...
double itkCorrelationImageToImageMetricComputationTypeTemplatev4Test_GetToyImagePixelValue(TIndexType index, TPointType offset, const unsigned int Dim, double c)
{
  double v = 0.0;
...
int itkCorrelationImageToImageMetricComputationTypeTemplatev4Test_WithSpecifiedThreads(TMetricPointer &metric,
                                                                TValueType &value,
...
int itkCorrelationImageToImageMetricComputationTypeTemplatev4Test(int, char ** const)
{
  ...
  while( !itFixed.IsAtEnd() )
    ...
    double v = itkCorrelationImageToImageMetricComputationTypeTemplatev4Test_GetToyImagePixelValue(ind, p0, imageDimensionality, 0);
    itFixed.Set( v  );
    count++;
    ++itFixed;
    }
  ...
  while( !itMoving.IsAtEnd() )
    ...
    double v = itkCorrelationImageToImageMetricComputationTypeTemplatev4Test_GetToyImagePixelValue(ind, p1, imageDimensionality, 0);
    itMoving.Set( v );
    count++;
    ++itMoving;
    }
  ...
  /* The metric */
  typedef itk::CorrelationImageToImageMetricComputationTypeTemplatev4< double, ImageType, ImageType, ImageType > MetricType;
  ...
  ret = itkCorrelationImageToImageMetricComputationTypeTemplatev4Test_WithSpecifiedThreads(metric, value1, derivative1);
  if( ret == EXIT_FAILURE )
    {
    result = EXIT_FAILURE;
    }
  ...
  std::cerr << "Setting number of metric threads to " << metric->GetMaximumNumberOfThreads() << std::endl;
  ret = itkCorrelationImageToImageMetricComputationTypeTemplatev4Test_WithSpecifiedThreads(metric, value2, derivative2);
  if( ret == EXIT_FAILURE )
    {
    result = EXIT_FAILURE;
    }
