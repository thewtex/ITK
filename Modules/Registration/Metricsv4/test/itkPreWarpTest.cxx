                              ...
                              std::string & outputPath )
{
  ...
  while ( !it.IsAtEnd() )
    ...
    for ( unsigned int j = 0; j < ImageDimensionality; j++ )
      {
      if ( it.GetIndex()[j] < imageBorder || fabs( it.GetIndex()[j] - size[j] ) < imageBorder )
        {
...
int itkPreWarpTest( int argc, char *argv[] )
{
  ...
  unsigned int dimensionality = 2; //3;
  double minimumAverage = itk::NumericTraits<double>::max();
  double rotationDegrees = static_cast<double>( 20.0 ); // (3.0);
  double maxDegrees = static_cast<double>( 359.0 );
  double degreeStep = static_cast<double>( 20.0 ); //(3.0);
  ...
  for (; rotationDegrees < maxDegrees; rotationDegrees += degreeStep )
    ...
    if ( dimensionality == 2 )
      ...
      average = itkPreWarpTestRunTest<2, typename MetricType::MovingTransformType>( imageSize,
          typename MetricType::MovingTransformType::Pointer( transform ), rotationDegrees, rotationDegrees == 45.0,
          outputPath );
      }
    ...
    if ( dimensionality == 3 )
      ...
      average = itkPreWarpTestRunTest<3, typename MetricType::MovingTransformType>( imageSize,
          typename MetricType::MovingTransformType::Pointer( transform ), rotationDegrees, rotationDegrees == 45.0,
          outputPath );
      }
