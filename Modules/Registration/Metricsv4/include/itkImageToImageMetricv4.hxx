                             ...
                             FixedImageGradientType & gradient ) const
{
  ...
  if ( this->m_DoFixedImagePreWarp )
  {
    //typename FixedTransformType::InputVectorType locgrad;
    //for (unsigned int i=0; i < FixedImageDimension; i++ ) locgrad[i]=gradient[i];
    //gradient=this->GetFixedTransform()->TransformVector(locgrad, mappedPoint );
  }
