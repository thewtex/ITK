...
::PointSetToPointSetMetricv4()
{
  ...
  this->m_DoInMovingSpaceDebug = false;
}
...
std::cout << "** doInMovingSpace: " << this->m_DoInMovingSpaceDebug << std::endl;
...
{
  // Transform the moving point set into the virtual domain
  if( ( this->m_MovingTransform->GetMTime() > this->GetMTime() ) || !this->m_MovingTransformedPointSet )
    ...
    while( It != this->m_MovingPointSet->GetPoints()->End() )
      {
      PointType point;
      if( m_DoInMovingSpaceDebug )
        {
        point = this->m_MovingTransform->TransformPoint( It.Value() );
        }
      else
        {
...
{
  // Transform the fixed point set into the virtual domain
