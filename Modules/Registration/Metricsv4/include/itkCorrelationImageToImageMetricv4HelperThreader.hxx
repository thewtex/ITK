...
::BeforeThreadedExecution()
{
  ...
  this->m_CorrelationAssociate = dynamic_cast<TCorrelationMetric *>(this->m_Associate);
   ...
   this->m_CorrelationMetricPerThreadVariables.resize(this->GetNumberOfThreadsUsed());
    ...
    TImageToImageMetric, TCorrelationMetric>::AfterThreadedExecution()
{
  ...
  for (size_t i = 0; i < this->m_GetValueAndDerivativePerThreadVariables.size(); i++)
    {
    sumF += this->m_CorrelationMetricPerThreadVariables[i].FixSum;
    sumM += this->m_CorrelationMetricPerThreadVariables[i].MovSum;
    }
