...
/** \class ObjectToObjectMultiMetricv4
  ...
  * to ensure proper sampling within the point set metrics. For completeness, such a virtual domain
  * must include points within the fixed point set in order to generate valid shift estimations.
  ...
  * be a virtual domain that matches the space of the transform field. Note that both
  * Image and PointSet metrics will automatically create a matching virtual domain during
  * initialization if one has not been assigned by the user.
  */
