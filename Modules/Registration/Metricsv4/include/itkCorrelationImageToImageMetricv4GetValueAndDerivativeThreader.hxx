...
::BeforeThreadedExecution()
{
  ...
  const NumberOfParametersType globalDerivativeSize =
      associate->GetNumberOfParameters();
  ...
  for (ThreadIdType i = 0; i < this->GetNumberOfThreadsUsed(); i++)
    {
    itkDebugMacro(
        "CorrelationImageToImageMetricv4::Initialize: transform does NOT have local support\n");
    ...
    TImageToImageMetric, TCorrelationMetric>::AfterThreadedExecution()
{
  ...
  if (m2 * f2 == 0)
    ...
    associate->m_Value = 0;
    associate->m_DerivativeResult->SetSize(globalDerivativeSize);
    associate->m_DerivativeResult->Fill(
        NumericTraits<DerivativeValueType>::Zero);
  ...
  *(associate->m_DerivativeResult) =
      -1.0 * 2 *fm/(f2*m2)*(fdm - fm/m2*mdm);
}
...
bool
CorrelationImageToImageMetricv4GetValueAndDerivativeThreader<TDomainPartitioner,
TImageToImageMetric, TCorrelationMetric>::ProcessPoint(
...
const ThreadIdType threadID) const
{
  ...
  /* this makes the following code not really mean anything
  ...
  associate->GetMovingTransform()->ComputeJacobianWithRespectToParameters(
