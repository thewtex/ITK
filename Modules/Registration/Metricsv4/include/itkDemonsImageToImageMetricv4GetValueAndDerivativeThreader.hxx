                ...
                const ThreadIdType ) const
{
  ...
  const DerivativeValueType floatingPointCorrectionResolution = associate->GetFloatingPointCorrectionResolution();

  for ( SizeValueType dim = 0; dim < associate->GetNumberOfLocalParameters(); dim++ )
    {
    InternalComputationValueType val = speedValue * (*gradient)[dim] / denominator;

    intmax_t test = static_cast< intmax_t >( static_cast<DerivativeValueType>(val) * floatingPointCorrectionResolution );
    localDerivativeReturn[dim] = static_cast<DerivativeValueType>( test / floatingPointCorrectionResolution );
    }
