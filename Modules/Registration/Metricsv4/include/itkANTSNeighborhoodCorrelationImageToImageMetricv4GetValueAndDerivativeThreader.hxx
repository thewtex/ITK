...
// try to reuse the routine from the dense sampling by reinitialize the sampling each time
// try implement virtual method ProcessVirtualPoint
template < class TDomainPartitioner, class TImageToImageMetric, class TNeighborhoodCorrelationMetric >
bool
ANTSNeighborhoodCorrelationImageToImageMetricv4GetValueAndDerivativeThreader< TDomainPartitioner, TImageToImageMetric, TNeighborhoodCorrelationMetric >
::ProcessVirtualPoint_impl(
    ...
    const VirtualIndexType & virtualIndex,
                       const VirtualPointType & /* Not Used: virtualPoint*/,
                       const ThreadIdType threadId )
{
...
// convert virtualPoint to a single point region
  ...
  this->m_ANTSAssociate->InitializeScanning( singlePointRegion, scanIt, scanMem, scanParameters );
