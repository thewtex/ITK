...
::BeforeThreadedExecution()
{
  ...
  /* Per-thread results */
  this->m_GetValueAndDerivativePerThreadVariables.clear();
  this->m_GetValueAndDerivativePerThreadVariables.resize( this->GetNumberOfThreadsUsed() );
...
::AfterThreadedExecution()
{
  ...
  if( this->m_Associate->VerifyNumberOfValidPoints( this->m_Associate->m_Value, *(this->m_Associate->m_DerivativeResult) ) )
    ...
    for(size_t i=0; i< this->m_GetValueAndDerivativePerThreadVariables.size(); i++)
      {
      this->m_Associate->m_Value += this->m_GetValueAndDerivativePerThreadVariables[i].Measure;
      }
