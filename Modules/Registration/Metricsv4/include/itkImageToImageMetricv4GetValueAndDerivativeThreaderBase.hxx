...
::BeforeThreadedExecution()
{
  ...
  /* Per-thread results */
  this->m_PerThreadCacheVariables.clear();
  this->m_PerThreadCacheVariables.resize( this->GetNumberOfThreadsUsed() );
  ...
  if( this->m_Associate->GetComputeDerivative() )
    ...
    for (ThreadIdType i=0; i<this->GetNumberOfThreadsUsed(); ++i)
       ...
       * derived classes */
      this->m_PerThreadCacheVariables[i].LocalDerivatives.SetSize( this->m_Associate->GetNumberOfLocalParameters() );
      this->m_PerThreadCacheVariables[i].MovingTransformJacobian.SetSize( this->m_Associate->VirtualImageDimension, this->m_Associate->GetNumberOfLocalParameters() );
      if ( this->m_Associate->m_MovingTransform->GetTransformCategory() == MovingTransformType::DisplacementField )
        ...
        this->m_PerThreadCacheVariables[i].Derivatives.SetData( this->m_Associate->m_DerivativeResult->data_block(),
        ...
        this->m_PerThreadCacheVariables[i].CompensatedDerivatives.resize( globalDerivativeSize );
        }
      }
    }
  ...
  for (ThreadIdType thread = 0; thread<this->GetNumberOfThreadsUsed(); thread++)
    {
    this->m_PerThreadCacheVariables[thread].NumberOfValidPoints = NumericTraits< SizeValueType >::Zero;
    this->m_PerThreadCacheVariables[thread].Measure = NumericTraits< InternalComputationValueType >::Zero;
    if( this->m_Associate->GetComputeDerivative() )
      {
      if ( this->m_Associate->m_MovingTransform->GetTransformCategory() != MovingTransformType::DisplacementField )
        {
        ...
        for( NumberOfParametersType p = 0; p < this->m_Associate->GetNumberOfParameters(); p++ )
          {
          this->m_PerThreadCacheVariables[thread].CompensatedDerivatives[p].ResetToZero();
          }
        }
      }
    }
...
::AfterThreadedExecution()
{
  ...
  for (ThreadIdType i=0; i<this->GetNumberOfThreadsUsed(); i++)
    {
    this->m_Associate->m_NumberOfValidPoints += this->m_PerThreadCacheVariables[i].NumberOfValidPoints;
    }
  ...
  if( this->m_Associate->GetComputeDerivative() )
    ...
    if ( this->m_Associate->m_MovingTransform->GetTransformCategory() != MovingTransformType::DisplacementField )
      ...
      for (NumberOfParametersType p = 0; p < this->m_Associate->GetNumberOfParameters(); p++ )
        ...
        for (ThreadIdType i=0; i<this->GetNumberOfThreadsUsed(); i++)
          {
          sum += this->m_PerThreadCacheVariables[i].CompensatedDerivatives[p].GetSum();
          }
  ...
  if( this->m_Associate->VerifyNumberOfValidPoints( this->m_Associate->m_Value, *(this->m_Associate->m_DerivativeResult) ) )
    ...
    for(size_t i=0; i< this->m_PerThreadCacheVariables.size(); i++)
      {
      this->m_Associate->m_Value += this->m_PerThreadCacheVariables[i].Measure;
      }
                       ...
                       const ThreadIdType threadId )
{
  ...
  try
    ...
    pointIsValid = this->ProcessPoint(
                                   ...
                                   metricValueResult,
                                   this->m_PerThreadCacheVariables[threadId].LocalDerivatives,
                                   threadId );
    }
  catch( ExceptionObject & exc )
    {
  ...
  if ( this->m_Associate->m_MovingTransform->GetTransformCategory() != MovingTransformType::DisplacementField )
    ...
    if ( this->m_Associate->GetUseFloatingPointCorrection() )
      ...
      for (NumberOfParametersType p = 0; p < this->m_CachedNumberOfParameters; p++ )
        {
        intmax_t test = static_cast< intmax_t >( this->m_PerThreadCacheVariables[threadId].LocalDerivatives[p] * correctionResolution );
        this->m_PerThreadCacheVariables[threadId].LocalDerivatives[p] = static_cast<DerivativeValueType>( test / correctionResolution );
        }
      }
    for (NumberOfParametersType p = 0; p < this->m_CachedNumberOfParameters; p++ )
      {
      this->m_PerThreadCacheVariables[threadId].CompensatedDerivatives[p] += this->m_PerThreadCacheVariables[threadId].LocalDerivatives[p];
      }
    }
  else
    {
    ...
    try
      ...
      for (NumberOfParametersType i=0; i < this->m_CachedNumberOfLocalParameters; i++)
        ...
        this->m_PerThreadCacheVariables[threadId].Derivatives[offset+i] += this->m_PerThreadCacheVariables[threadId].LocalDerivatives[i];
        }
      }
    catch( ExceptionObject & exc )
      {
