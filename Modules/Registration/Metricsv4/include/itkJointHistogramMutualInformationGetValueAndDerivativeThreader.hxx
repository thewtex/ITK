...
::BeforeThreadedExecution()
{
  ...
  for( ThreadIdType i = 0; i < this->GetNumberOfThreadsUsed(); ++i )
    {
    if( this->m_PerThreadJointHistogramMI[i].JointPDFInterpolator.IsNull() )
      {
      this->m_PerThreadJointHistogramMI[i].JointPDFInterpolator = JointPDFInterpolatorType::New();
      }
    this->m_PerThreadJointHistogramMI[i].JointPDFInterpolator->SetInputImage( this->m_JointAssociate->m_JointPDF );
    if( this->m_PerThreadJointHistogramMI[i].FixedImageMarginalPDFInterpolator.IsNull() )
      {
      this->m_PerThreadJointHistogramMI[i].FixedImageMarginalPDFInterpolator = MarginalPDFInterpolatorType::New();
      }
    this->m_PerThreadJointHistogramMI[i].FixedImageMarginalPDFInterpolator->SetInputImage( this->m_JointAssociate->m_FixedImageMarginalPDF );
    if( this->m_PerThreadJointHistogramMI[i].MovingImageMarginalPDFInterpolator.IsNull() )
      {
      this->m_PerThreadJointHistogramMI[i].MovingImageMarginalPDFInterpolator = MarginalPDFInterpolatorType::New();
      }
    this->m_PerThreadJointHistogramMI[i].MovingImageMarginalPDFInterpolator->SetInputImage( this->m_JointAssociate->m_MovingImageMarginalPDF );
    }
}
                ...
                const ThreadIdType              threadId ) const
{
  ...
  if ( ! this->m_PerThreadJointHistogramMI[threadId].JointPDFInterpolator->IsInsideBuffer( jointPDFpoint ) )
    {
    return false;
    }
  InternalComputationValueType jointPDFValue = this->m_PerThreadJointHistogramMI[threadId].JointPDFInterpolator->Evaluate( jointPDFpoint );
  SizeValueType ind = 1;
  ...
  InternalComputationValueType movingImagePDFValue =
    this->m_PerThreadJointHistogramMI[threadId].MovingImageMarginalPDFInterpolator->Evaluate(mind);
  ...
  FixedTransformJacobianType & jacobian =
    const_cast< FixedTransformJacobianType &   >(this->m_PerThreadCacheVariables[threadId].MovingTransformJacobian);
                                           ...
                                           const ThreadIdType threadId ) const
{
  ...
  if ( delta > NumericTraits< InternalComputationValueType >::Zero )
    ...
    InternalComputationValueType deriv =
      this->m_PerThreadJointHistogramMI[threadId].MovingImageMarginalPDFInterpolator->Evaluate(rightpoint) -
      this->m_PerThreadJointHistogramMI[threadId].MovingImageMarginalPDFInterpolator->Evaluate(leftpoint);
    return deriv/delta;
    }
  else
    {
                             ...
                             const SizeValueType ind ) const
{
  ...
  if ( delta > NumericTraits< InternalComputationValueType >::Zero )
    {
    deriv = this->m_PerThreadJointHistogramMI[threadId].JointPDFInterpolator->Evaluate(rightpoint)-
          this->m_PerThreadJointHistogramMI[threadId].JointPDFInterpolator->Evaluate(leftpoint);
    return deriv/delta;
    }
  return deriv;
}
