...
::BeforeThreadedExecution()
{
   ...
   * FIXME: most of these aren't used in this metric, so there
   * is some memory inefficiency.
   */
  ...
  /* These next blocks of code are from MattesMutualImageToImageMetric::Initialize */
                ...
                const ThreadIdType                 threadID) const
{
  ...
  while( pdfMovingIndex <= pdfMovingIndexMax )
    {
    *( pdfPtr++ ) += static_cast<PDFValueType>( associate->m_CubicBSplineKernel ->Evaluate( movingImageParzenWindowArg) );
...
::AfterThreadedExecution()
{
  ...
  /* This code is from MattesMutualInformationImageToImageMetric::GetValueAndDerivativeThreadPostProcess */

  /* Note: skipping this because it sums things we're notusing. */
