...
::FinalizeThread( const ThreadIdType threadId )
{
  ...
  if( this->GetComputeDerivative() && ( ! this->HasLocalSupport() ) )
    ...
    const size_t numberVoxelDerivativeValuesPerSubsection = std::max<size_t>(1, static_cast<size_t>( std::ceil(static_cast<double>( histogramTotalElementsSize )/static_cast<double>(numberOfSubsections ) ) ) );
    for( size_t subsection = 0; subsection < numberOfSubsections; ++subsection )
      {
      JointPDFDerivativesValueType const * const threadPdfDPtrEnd = threadPdfDPtrStart + std::min<size_t>(histogramTotalElementsSize,(subsection+1)*numberVoxelDerivativeValuesPerSubsection);
