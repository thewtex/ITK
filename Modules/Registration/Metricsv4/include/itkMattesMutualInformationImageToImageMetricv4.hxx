...
::FinalizeThread( const ThreadIdType threadId )
{
  ...
  // separately, but rather as a part of all post-processing.
    ...
    //NOTE: 1 + is to ensure that all voxels are processed, since integer division is truncated instead of rounding up as we need.
    ...
    for( size_t subsection = 0; subsection < numberOfSubsections; ++subsection )
      ...
      JointPDFValueType const * const threadPdfEnd = threadPdfStart + std::min<size_t>(numberOfVoxels,subsection*numberVoxelsPerSubsection);
  ...
  if( this->GetComputeDerivative() && ( ! this->HasLocalSupport() ) )
    ...
    const size_t histogramTotalElementsSize = rowSize * this->m_NumberOfHistogramBins;
      ...
      for( size_t subsection = 0; subsection < numberOfSubsections; ++subsection )
        ...
        JointPDFDerivativesValueType const * const threadPdfDPtrEnd = threadPdfDPtrStart + std::min<size_t>(histogramTotalElementsSize, subsection*histogramTotalElementsPerSubsection );
...
::ComputeResults() const
{
  ...
  const size_t numberOfPoints = this->GetNumberOfDomainPoints();
  ...
  if( this->GetComputeDerivative() )
    ...
    if( this->HasLocalSupport() )
      {
      for( size_t i = 0, derivativeSize = this->m_DerivativeResult->Size(); i < derivativeSize; ++i )
        {
        for( size_t bin = 0; bin < 4; ++bin )
          ...
          const size_t pRatioIndex = this->m_JointPdfIndex1DArray[i] + bin;
