...
::FinalizeThread( const ThreadIdType threadId )
{
  ...
  const ThreadedIndexedContainerPartitioner::IndexRangeType completeDerivativeIndexRange = {{0, histogramTotalElementsSize}};
    ...
    if( needDerivativesComputation )
      ...
      for( IndexValueType subsection = 0; subsection < numberOfDerivativesSubsections; ++subsection )
        ...
        if( ! isPDFDerivativeSectionDone[subsection] )
          ...
          MutexLockHolder< MutexLock > lockHolder(*(m_JointPDFDerivativeSubsectionLocks[subsection]),false);
          if(lockHolder.GetLockCaptured())
            {
            this->m_IndexedContainerPartitioner->PartitionDomain( subsection, numberOfDerivativesSubsections, completeDerivativeIndexRange, indexRange );
            JointPDFDerivativesValueType       * threadPdfDPtr = threadPdfDPtrStart + indexRange[0];
            JointPDFDerivativesValueType * accumPDFDerivPtr = accumPDFDerivPtrStart + indexRange[0];
            JointPDFDerivativesValueType const * const accumPDFDerivPtrEnd = accumPDFDerivPtrStart + indexRange[1];
