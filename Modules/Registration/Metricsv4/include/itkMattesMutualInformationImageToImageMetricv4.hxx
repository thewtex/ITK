 ...
 *  See the License for the specixfic language governing permissions and
...
::FinalizeThread( const ThreadIdType threadId )
{
  ...
  // separately, but rather as a part of all post-processing.
    ...
    const SizeValueType numberOfSubsections = static_cast< SizeValueType >( m_JointPDFSubsectionLocks.size() );
  ...
  if( this->GetComputeDerivative() && ( ! this->HasLocalSupport() ) )
    ...
    const SizeValueType histogramTotalElementsSize = this->GetNumberOfLocalParameters() * this->m_NumberOfHistogramBins * this->m_NumberOfHistogramBins;
    ...
    const SizeValueType numberOfSubsections = static_cast< SizeValueType >( m_JointPDFDerivativeSubsectionLocks.size() );
