...
::FinalizeThread( const ThreadIdType threadId )
{
  ...
  // loop while there's work left to do
  ...
  do
    ...
    for( IndexValueType subsection = 0; subsection < numberOfPDFSubsections; ++subsection )
      ...
      if( ! isPDFSectionDone[subsection] )
        ...
        if(lockHolder.GetLockCaptured())
          {
          this->m_IndexedContainerPartitioner->PartitionDomain( subsection,
                                                                numberOfPDFSubsections,
                                                                completeIndexRange,
                                                                indexRange );
    ...
    if( needDerivativesComputation )
      ...
      JointPDFDerivativesValueType * const accumPDFDerivPtrStart =
        this->m_AccumulatorJointPDFDerivatives->GetBufferPointer();
      JointPDFDerivativesValueType  * const threadPdfDPtrStart =
        this->m_ThreaderJointPDFDerivatives[threadId]->GetBufferPointer();
      ...
      for( IndexValueType subsection = 0; subsection < numberOfDerivativesSubsections; ++subsection )
        ...
        if( ! isPDFDerivativeSectionDone[subsection] )
          ...
          if(lockHolder.GetLockCaptured())
            {
            this->m_IndexedContainerPartitioner->PartitionDomain( subsection,
                                                                  numberOfDerivativesSubsections,
                                                                  completeDerivativeIndexRange,
                                                                  derivativeIndexRange );
            ...
            JointPDFDerivativesValueType const * const accumPDFDerivPtrEnd =
              accumPDFDerivPtrStart + derivativeIndexRange[1];
