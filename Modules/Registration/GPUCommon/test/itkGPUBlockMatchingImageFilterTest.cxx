...
static bool local_CompareValues(const double a, const double b, const double tol)
{
  bool testPassing = true;
  if( std::abs( a - b ) > tol )
    {
    testPassing = false;
    std::cout << "FAIL: std::abs( " << a << " - " << b << " ) > " << tol << std::endl;
    }
  else
    {
    std::cout << "PASS: std::abs( " << a << " - " << b << " ) > " << tol << std::endl;
    }
  return testPassing;
}
...
int itkGPUBlockMatchingImageFilterTest(int argc, char *argv[])
{
  ...
  CasterType::Pointer fixedCaster = CasterType::New();
  fixedCaster->SetInput( resampleFilter->GetOutput() );
  fixedCaster->Update();
  CasterType::Pointer movingCaster = CasterType::New();
  ...
  std::cout << "STARTING GPUBlockMatchingFilter" << std::endl;
  try
    {
  ...
  std::cout << "STOPPING GPUBlockMatchingFilter" << std::endl;

  std::cout << "STARTING CPUBlockMatchingFilter" << std::endl;
  // CPU BM
  try
    {
  ...
  std::cout << "STOPPING CPUBlockMatchingFilter" << std::endl;
  ...
  while ( pointsItr != pointsEnd )
    {
    const double e = 1e-6;
    testPassing &= local_CompareValues(simsItrCPU.Value()    , simsItrGPU.Value()    , e );
    testPassing &= local_CompareValues(dispsItrCPU.Value()[0], dispsItrGPU.Value()[0], e );
    testPassing &= local_CompareValues(dispsItrCPU.Value()[1], dispsItrGPU.Value()[1], e );
    testPassing &= local_CompareValues(dispsItrCPU.Value()[2], dispsItrGPU.Value()[2], e );
  ...
  return testPassing ? EXIT_SUCCESS : EXIT_FAILURE;
}
