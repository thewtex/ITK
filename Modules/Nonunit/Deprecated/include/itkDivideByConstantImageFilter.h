...
class ITK_EXPORT DivideByConstantImageFilter :
    public DivideImageFilter<TInputImage,TInputImage,TOutputImage>
{
public:
  typedef DivideByConstantImageFilter                             Self;
  typedef DivideImageFilter<TInputImage,TInputImage,TOutputImage> Superclass;
  ...
  /** Set the constant that will be used to subtract from the image
    * pixels */
  void SetConstant(TConstant ct)
    {
      this->Superclass::SetConstant2(ct);
    }
  const TConstant & GetConstant() const
    {
      return this->Superclass::GetConstant2();
    }
  virtual void SetInput(const TInputImage *input)
  {
    this->Superclass::SetInput1(input);
  }
protected:
