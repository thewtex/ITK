...
int LabelGeometryImageFilterTest(std::string labelImageName,std::string intensityImageName,std::string outputImageName,
                                 std::string outputFileName, std::string compareFileName);
...
itkLabelGeometryImageFilterTest( int argc, char * argv[] )
{
  ...
  itk::ImageIOBase::Pointer imageIO = itk::ImageIOFactory::CreateImageIO(
      intensityImageName.c_str(), itk::ImageIOFactory::ReadMode);
  ...
  if( ImageDimension == 2 )
    {
    return LabelGeometryImageFilterTest<2>(labelImageName,intensityImageName,outputImageName,outputFileName,
                                           compareFileName);
    }
  else if( ImageDimension == 3 )
    {
    return LabelGeometryImageFilterTest<3>(labelImageName,intensityImageName,outputImageName,outputFileName,
                                           compareFileName);
    }
  return EXIT_SUCCESS;
}
...
template < const unsigned int NDimension >
int
LabelGeometryImageFilterTest(std::string labelImageName,std::string intensityImageName,std::string outputImageName,
                             std::string outputFileName,std::string compareFileName)
{
  ...
  // If an optional csv file was passed in, compare the results of this analysis
  ...
  if( strcmp(compareFileName.c_str(),"") )
    ...
    // This is better than comparing against the values in memory because some
    // truncation occurs when writing to file.
  ...
  for (unsigned int i = 0; i < m1.rows(); i++)
    ...
    for (unsigned int j = 0; j < m1.cols(); j++)
      ...
      if (vcl_fabs(m1[i][j] - m2[i][j]) > epsilon)
        {
        std::cout << "Matrix difference:" << "abs(m2[" << i << "][" << j << "] - m1[" << i << "][" << j << "]): " <<
          vcl_fabs(m1[i][j] - m2[i][j]) << std::endl;
        pass = false;
        return pass;
        }
      }
