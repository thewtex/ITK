...
int itkCSVFileToArray2DReaderTest (int argc, char *argv[])
{
 if (argc < 2)
 {
...
/*
 The following code is commented out because it relies on a specific input file.
 */
/*
 filename = argv[1];
 reader->SetFileName( filename );
 ...
 DataFrameObjectType::Pointer dfo = reader->GetDataFrameObject();
 ...
 try
   ...
   {
   std::cerr << "Expeced Exception caught!" << std::endl;
   std::cerr << exp << std::endl;
   }
