...
::Parse()
{
  ...
  if (!this->m_InputStream.is_open())
    ...
    if ( this->m_InputStream.fail() )
      {
      itkExceptionMacro(<< "The file cannot be opened! " );
      }
    }
  ...
  // initialize the matrix to NaN so that missing data will automatically be set to this value
  ...
  for (unsigned int i = 0; i < rows; i++)
    ...
    for (unsigned int j = 0; j < columns; j++)
      {
      this->GetNextField(entry);
      this->m_DataFrameObject->SetMatrixData(i,j,this->ConvertStringToValueType<TData>(entry));

      // if the file contains missing data, m_Line will contain less data fields. So
      // we check if m_Line is empty and if it is, we break out of this loop and move to the next line.
      if (this->m_Line.empty())
        {
        break;
        }
