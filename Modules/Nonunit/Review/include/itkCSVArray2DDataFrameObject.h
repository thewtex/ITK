...
namespace itk
{
/** \class DataFrameObject contains methods for accessing
  * data from the Array2D object. It is used in the CSVFileToArray2DReader
  * class for storing parsed data. The DataFrameObject comprises of an
  * Array2D object and two std string vectors for storing row and column
  * headers. Individual rows, columns and data fields can be accessed
  * either through the names of the headers or through indices. There are
  * also some mutator methods that can be used to modify values
  * in the Array2D object as well as PushBack functions for pushing
  * column and row headers into their respective vectors.
  */
...
public:
 ...
 /** Get Row method. Input to the method is a row header string */
 const NumericVectorType    GetRow (const std::string);

 /** Get Row method. Input to the method is a row index */
 const NumericVectorType    GetRow (const unsigned int);

 /** Get Column method. Input to the method is a column header string */
 ...
 /** Method to access a data field from the Array2D object. Inputs are row and column header strings
   * in that order */
 const TData                GetData (const std::string, const std::string);

 /** Method to access a data field from the Array2D object. Inputs are row and column indices
   * in that order */
 ...
 /** Method to access a data field from the Array2D object using the () operator.
   * Inputs are the row and column header strings in that order. */
 const TData operator() (const std::string, std::string);

 /** Method to access a data field from the Array2D object using the () operator.
   * Inputs are the row and column indices in that order. */
