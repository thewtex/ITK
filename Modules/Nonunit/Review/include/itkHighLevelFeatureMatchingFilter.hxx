...
::GenerateData()
{
  ...
  m_GeoFilter=LabelGeometryImageFilterType::New();
  m_GeoFilter->SetInput(l2i->GetOutput());
  m_GeoFilter->CalculatePixelIndicesOn();
  m_GeoFilter->CalculateOrientedBoundingBoxOn();
  m_GeoFilter->CalculateOrientedLabelRegionsOn();
  m_GeoFilter->CalculateOrientedIntensityRegionsOn();
  m_GeoFilter->Update();

  m_AllLabels = m_GeoFilter->GetLabels();
  NumberOfLabelBase = m_GeoFilter->GetNumberOfLabels();

  signed long i=0;
  std::cout<<"Number of labels: in base image "<< m_GeoFilter->GetNumberOfLabels() << std::endl;
  ...
  typedef itk::Vector<float,ObjectFeatureDimension> PixelVectorImageType;
  ...
  typename LabelGeometryImageFilterType::LabelsType allLabels = m_GeoFilter->GetLabels();
  ...
  for( allLabelsIt = allLabels.begin()+1; allLabelsIt != allLabels.end(); allLabelsIt++ )
    ...
    pixelValue[1]=m_GeoFilter->GetVolume(labelValue);
    pixelValue[2]=m_GeoFilter->GetEccentricity(labelValue);
    m_LabelPoint=m_GeoFilter->GetCentroid(labelValue);
    pixelValue[3]=m_LabelPoint[0];
    pixelValue[4]=m_LabelPoint[1];
    pixelValue[5]=m_GeoFilter->GetOrientation(labelValue);
    pixelIndexBase[0]=i;
    i++;
  ...
  m_GeoFilterFloat=LabelGeometryImageFilterType::New();
  m_GeoFilterFloat->SetInput(l2i_F->GetOutput());

  m_GeoFilterFloat->CalculatePixelIndicesOn();
  m_GeoFilterFloat->CalculateOrientedBoundingBoxOn();
  m_GeoFilterFloat->CalculateOrientedLabelRegionsOn();
  m_GeoFilterFloat->CalculateOrientedIntensityRegionsOn();

  m_GeoFilterFloat->Update();
  typename LabelGeometryImageFilterType::LabelsType allLabelsFloat = m_GeoFilterFloat->GetLabels();
  typename LabelGeometryImageFilterType::LabelsType::iterator allLabelsItFloat;

  std::cout << "Number of labels in Float image: " << m_GeoFilterFloat->GetNumberOfLabels() << std::endl;
  i=0;
  for( allLabelsItFloat = allLabelsFloat.begin()+1; allLabelsItFloat != allLabelsFloat.end(); allLabelsItFloat++ )
    ...
    pixelValue[1]=m_GeoFilter->GetVolume(labelValueFloat);
    pixelValue[2]=m_GeoFilter->GetEccentricity(labelValueFloat);
    m_LabelPoint=m_GeoFilter->GetCentroid(labelValueFloat);
    pixelValue[3]=m_LabelPoint[0];
    pixelValue[4]=m_LabelPoint[1];
    pixelValue[5]=m_GeoFilter->GetOrientation(labelValueFloat);
    pixelIndexFloat[0]=i;
    i++;
        ...
        for(unsigned long index2=0;index2<NumberOfMatchingPairs;index2++)
          ...
          if ((PairValue1[0]!=PairValue[0])&&(PairValue1[1]!=PairValue[1]))
            ...
            if(SubDist<=MinDistance*m_DistTol)
              ...
              Theta=std::atan(
                (pixelValueFloat[4]-pixelValueFloatl[4])/(pixelValueFloat[3]-pixelValueFloatl[3]))
                - std::atan((pixelValueBase[4]-pixelValueBasek[4])/(pixelValueBase[3]-pixelValueBasek[3]));
