...
::CSVFileReader()
{
  ...
  m_UseStringDelimiterCharacter = (this->m_HasColumnHeaders or this->m_HasRowHeaders);
  m_DataFrameObject = DataFrameObjectType::New();
}
...
::Parse()
{
  ...
  if (this->m_HasColumnHeaders)
    ...
    if (this->m_UseStringDelimiterCharacter)
      ...
      while(std::getline(linestream,cell,this->m_StringDelimiterCharacter))
        ...
        if ( cellnum%2 != 0)
          {
          m_DataFrameObject->ColumnHeadersPushBack( cell );
          }
        cellnum++;
        }
      }
    ...
    m_DataFrameObject->EraseFirstColumnHeader();

    prev_cols = m_DataFrameObject->GetColumnHeaders().size();
    max_cols = prev_cols;
    }
  ...
  if (!isSame)
    {
    std::cerr << "Warning: Data appears to contain missing data!";
    std::cerr << "These will be set to NaN!";
    std::cerr << std::endl;
    }
  // if same
  else
  ...
  if (this->m_HasColumnHeaders)
    {
    unsigned int num_col_headers = m_DataFrameObject->GetColumnHeaders().size();
    if (max_cols > num_col_headers)
      ...
      for (unsigned int k = num_col_headers; k < max_cols; k++)
        ...
        m_DataFrameObject->ColumnHeadersPushBack ( col_name );
        col_name = "Col ";
        }
      }
    }
  ...
  m_DataFrameObject->SetMatrixSize(rows,max_cols);
  ...
  while(std::getline(inputStream,line))
    ...
    if (this->m_HasRowHeaders)
      ...
      if (this->m_UseStringDelimiterCharacter)
        ...
        m_DataFrameObject->RowHeadersPushBack(cell);
        }
    ...
    while(std::getline(linestream,cell,this->m_FieldDelimiterCharacter))
      {
      m_DataFrameObject->SetMatrixData( i, j, this->StringToNumeric(cell) );
      j++;
      }

    // set missing data to NaN
    for (;j < m_DataFrameObject->GetMatrix().cols();j++)
      {
      m_DataFrameObject->SetMatrixData( i, j, vcl_numeric_limits<TData>::quiet_NaN() );
      }
    i++;
    }
  inputStream.close();

  if (this->m_HasColumnHeaders) m_DataFrameObject->SetHasColumnHeaders(true);
  else m_DataFrameObject->SetHasColumnHeaders(false);

  if (this->m_HasRowHeaders)    m_DataFrameObject->SetHasRowHeaders(true);
  else m_DataFrameObject->SetHasColumnHeaders(false);
}
