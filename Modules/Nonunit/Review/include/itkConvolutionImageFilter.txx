                      ...
                      ProgressAccumulator * progress )
{
  ...
  if ( m_OutputRegionMode == Self::VALID )
    {
    // To the NeighborhoodOperatorImageFilter, the largest possible
    // region of the output is the same as the largest possible region
    // of the input, and this->GraftOutput() overwrites the largest
    // possible region in the output image. We therefore set the largest
    // possible region in the output back to what it should be here.
    OutputRegionType validRegion = this->GetValidRegion();
    this->GetOutput()->SetLargestPossibleRegion( validRegion );
    }
}
