...
::GenerateData()
{
  ...
  if ( m_Normalize )
    ...
    typedef NormalizeToConstantImageFilter< InputImageType, RealImageType >
      NormalizeFilterType;
    typename NormalizeFilterType::Pointer normalizeFilter =
                      ...
                      ProgressAccumulator * progress )
{
  ...
  typedef ImageKernelOperator< KernelPixelType, ImageDimension >
    KernelOperatorType;
  ...
  // Need to update the kernel pad filter because the
  // ImageKernelOperator does not update its inputs.
  if ( kernelOperator.GetImageKernel()->GetSource() )
    {
    kernelOperator.GetImageKernel()->GetSource()->UpdateLargestPossibleRegion();
    }
...
::GetKernelNeedsPadding() const
{
  InputSizeType kernelSize =
    this->GetImageKernelInput()->GetLargestPossibleRegion().GetSize();
  for ( unsigned int i = 0; i < ImageDimension; i++ )
    {
