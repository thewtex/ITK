...
::PrepareForWriting()
{
  ...
  // output a warning if the number of column headers and number of columns in the object are not the same
  if(!this->m_ColumnHeaders.empty())
    {
    if (!this->m_RowHeaders.empty() && this->m_ColumnHeaders.size() != (this->m_cols+1) )
      {
      std::cerr << "Warning: The number of column headers and the number of columns in the input object is not consistent." << std::endl;
      }
    if (this->m_RowHeaders.empty() && this->m_ColumnHeaders.size() != this->m_cols)
      {
      std::cerr << "Warning: The number of column headers and the number of columns in the input object is not consistent." << std::endl;
      }
    }
