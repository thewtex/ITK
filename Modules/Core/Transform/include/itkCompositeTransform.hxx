...
::ComputeJacobianWithRespectToParameters( const InputPointType & p, JacobianType & j ) const
{
  ...
  for( signed long tind = (signed long) this->GetNumberOfTransforms() - 1;
       tind >= 0; tind-- )
    {
    const TransformType * transform = this->GetNthTransformRaw( tind );
...
::GetNumberOfParameters(void) const
{
  ...
  for( signed long tind = (signed long) this->GetNumberOfTransforms() - 1;
    ...
    if( this->GetNthTransformToOptimize( tind ) )
      {
      transform = this->GetNthTransformRaw( tind );
                              ...
                              ScalarType  factor )
{
  ...
  for( signed long tind = (signed long) this->GetNumberOfTransforms() - 1;
    ...
    if( this->GetNthTransformToOptimize( tind ) )
      {
      subtransform = const_cast<TransformType*>( this->GetNthTransformRaw( tind ) );
...
::HasLocalSupport() const
{
  ...
  for( signed long tind = (signed long) this->GetNumberOfTransforms() - 1;
    ...
    if( this->GetNthTransformToOptimize( tind ) )
      {
      if( !this->GetNthTransformRaw( tind )->HasLocalSupport() )
        {
        result = false;
        }
      }
