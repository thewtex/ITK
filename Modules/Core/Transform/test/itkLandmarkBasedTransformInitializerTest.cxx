...
itkLandmarkBasedTransformInitializerTest(int, char * [])
{
    ...
    // Transform the landmarks now. For the given set of landmarks, since we
    // computed the
    // moving landmarks explicitly from the rotation and translation specified,
    // we should
    // get a transform that does not give any mismatch. In other words, if the
    // fixed
    // landmarks are transformed by the transform computed by the
    // LandmarkBasedTransformInitializer, they should coincide exactly with the
    // moving
    // landmarks. Note that we specified 4 landmarks, although three
    // non-collinear
