...
bool testValue( const double v1, const double v2, double precision=1 )
{
  const double fv2 = vcl_fabs( v2 );

  if( fv2 < precision * vnl_math::eps ) // v2 is almost zero
    {
    return vcl_fabs( v1 ) < precision * vnl_math::eps;
    }

  return vcl_fabs( ( v1 - v2 ) / v2 ) <= precision * vnl_math::eps;
}

template <typename TMatrix>
bool testMatrix( const TMatrix & m1, const TMatrix & m2, double precision=1.0 )
{
  bool pass = true;
  ...
  for( unsigned int  i = 0; i < TMatrix::RowDimensions; i++ )
    ...
    for( unsigned int j = 0; j < TMatrix::ColumnDimensions; j++ )
      {
      if( !testValue( m1[i][j], m2[i][j], precision ) )
        {
        pass = false;
        }
      }
...
bool testVector( const TVector & v1, const TVector & v2, double precision=1.0 )
{
  bool pass = true;

  for( unsigned int i = 0; i < TVector::Dimension; i++ )
    {
    if( !testValue( v1[i], v2[i], precision ) )
      {
      pass = false;
      }
    }
...
template <typename TVector>
bool testVariableVector( const TVector & v1, const TVector & v2, double precision=1 )
{
  bool               pass = true;
...
int itkAffineTransformTest(int, char *[])
{
  ...
  if( !testMatrix( affInv2->GetMatrix(), matrix2Truth, 10.0 ) ||
      !testVector( affInv2->GetOffset(), vector2Truth, 10.0 ) )
    {
  ...
  if( !testVariableVector( m4, m4T, 10000.0 ) )
    {
  ...
  if( !testMatrix( aff3->GetMatrix(), matrix3Truth, 1000.0 ) )
    {
  ...
  if( !testMatrix( inv3->GetMatrix(), matrix3Truth, 100.0 ) )
    {
  ...
  if( !testMatrix( inv4->GetMatrix(), matrix3Truth, 100.0 ) )
    {
