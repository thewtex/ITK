...
// We are now using thread pool
  int threadId = m_ThreadPool->GetJobIdForThreadHandle(threadHandle);
...
::DispatchSingleMethodThread(MultiThreader::ThreadInfoStruct *threadInfo)
{
  ...
  itkDebugMacro(<< std::endl << "Got job id :" << id );
  pthread_t returnHandle = m_ThreadPool->GetThreadHandleForJob(id);
  return returnHandle;
