...
::ReportProgress(Object *who, const EventObject & event)
{
  ...
  if ( typeid( event ) == typeid( pe ) )
    ...
    {
    // When a filter is restarted, we can capture the progress it has made so
    // far and add it
    // to the accumulated value.
    // This is especially helpful when streaming is used since the filter is
    // restarted multiple
    // times for different regions of the input.
    // By capturing the start event, it is no longer necessary for filters that
    // use the ProgressAccumulator
    ...
    for ( it = m_FilterRecord.begin(); it != m_FilterRecord.end(); ++it )
      ...
      if( who == it->Filter )
        {
        // On a start event, we need to capture the accumulated progress for
        // this filter
        // and then reset this filter's progress.
        // It is not necessary to call UpdateProgress(0.0f) explicitly on the
        // filter because this is done
