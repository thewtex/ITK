...
::AddThread()
{
  ...
  if( rc )
    ...
    ThreadIdHandlePairingContainerPairType tHPCPair(newlyAddedThreadHandle,
                                                    ThreadProcessStatusPair(JOB_THREADHANDLE_JUST_ADDED,
                                                                            newlyAddedThreadHandle));
    m_ThreadIdHandlePairingQueue.insert(tHPCPair);

    ThreadSemHandlePairingQueuePairType
      newTSHPFW(newlyAddedThreadHandle,new ThreadSemaphorePair(newlyAddedThreadHandle));
    m_ThreadSemHandlePairingForWaitQueue.insert(newTSHPFW);
    ThreadSemHandlePairingQueuePairType
      newTSHPQP(newlyAddedThreadHandle,new ThreadSemaphorePair(newlyAddedThreadHandle));
    m_ThreadSemHandlePairingQueue.insert(newTSHPQP);
...
::DeallocateThreadSemSet(ThreadSemHandlePairingQueueType &q)
{
  for(ThreadSemHandlePairingQueueType::iterator it = q.begin(), end = q.end();
     ...
     {
 #if !defined(__APPLE__)
    sem_destroy(&(it->second->m_Semaphore));
 #endif
    delete it->second;
     }
  q.clear();
...
::DestroyPool(int pollTime = 2)
{
  ...
  ThreadIdHandlePairingContainerType(m_ThreadIdHandlePairingQueue).swap(m_ThreadIdHandlePairingQueue);
