...
::AddThread()
{
  ...
  tcLock.Lock();
  ThreadCountType threadCount = ++this->m_ThreadCount;
  tcLock.Unlock();
  ...
  if( rc )
    ...
    itkDebugMacro(<< "Thread created with handle :" << m_ThreadHandles[threadCount - 1] << std::endl );
    }
...
::DestroyPool(int pollTime = 2)
{
  ...
  for( ThreadCountType i = 0; i < m_ThreadCount; ++i )
    {
