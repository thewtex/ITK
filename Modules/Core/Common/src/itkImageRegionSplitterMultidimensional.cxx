                            ...
                            unsigned int requestedNumber) const
{
  ...
  std::vector<unsigned int> splits(dim); // Note: stack allocation prefered

  unsigned int numPieces = this->ComputeSplits(dim, requestedNumber, regionIndex, regionSize, splits);
...
{
  unsigned int splits[dim]; // number of splits in each dimension
  unsigned int splittedRegionIndex[dim]; // index into splitted regions

  numberOfPieces = this->ComputeSplits(dim, numberOfPieces, regionIndex, regionSize, splits);
...
//                 const RegionType & region)
{
  ...
  std::vector<double> splitRegionSize(dim); // Note: stack allocation prefered
