...
::~ProcessObject()
{
  ...
  for ( DataObjectPointerMap::iterator it = m_NamedOutputs.begin(); itk != m_NamedOutputs.end(); it++ )
    {
    if ( it->second )
      {
      // let the output know we no longer want to associate with the object
...
::GetAllInputs() const
{
  ...
  for ( DataObjectPointerMap::const_iterator it = m_NamedInputs.begin(); itk != m_NamedInputs.end(); it++ )
    {
...
::GetAllInputs()
{
  ...
  for ( DataObjectPointerMap::iterator it = m_NamedInputs.begin(); itk != m_NamedInputs.end(); it++ )
    {
    res.push_back( it->second.GetPointer() );
    }
...
::GetAllOutputs() const
{
  ...
  for ( DataObjectPointerMap::const_iterator it = m_NamedOutputs.begin(); itk != m_NamedOutputs.end(); it++ )
    {
    res.push_back( it->second.GetPointer() );
    }
...
::SetReleaseDataFlag(bool val)
{
  ...
  for ( DataObjectArray::iterator it=outputs.begin(); itk != outputs.end(); it++ )
    {
    if ( *it )
      {
...
::PropagateResetPipeline()
{
  ...
  for ( DataObjectArray::iterator it=inputs.begin(); itk != inputs.end(); it++ )
    {
    if ( *it )
      {
