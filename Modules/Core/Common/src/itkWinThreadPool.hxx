...
GetThreadHandleForThreadId(ThreadIdType id)
{
  ...
  for (ThreadIdHandlePairingContainerType::iterator tpIt = m_ThreadIdHandlePairingQueue.begin(),
         end = m_ThreadIdHandlePairingQueue.end(); tpIt != end; ++tpIt)
    {
...
::ThreadExecute(void *param)
{
  ...
  ThreadIdHandlePairingContainerType::iterator tpIter = winThreadPool->m_ThreadIdHandlePairingQueue.begin(),
    end = winThreadPool->m_ThreadIdHandlePairingQueue.end();
  for (; tpIter != end; ++tpIter)
    {
    if (tpIter->m_ThreadProcessHandle == threadId)
      {
