...
bool ThreadPool::WaitForJobOnThreadHandle(ThreadProcessIdType threadHandle)
{
  try
    ...
    if(GetSemaphoreForThreadWait(threadHandle)->SemaphoreWait() != 0 )
      ...
      ThreadIdHandlePairingContainerType::const_iterator tpIter =
        m_ThreadIdHandlePairingQueue.find(threadHandle);
      if(tpIter == m_ThreadIdHandlePairingQueue.end())
        {
        itkExceptionMacro(<< "can't find ThreadIdHandlePairing object for thread handle"
                          << threadHandle);
        }
      if( tpIter->second.m_ThreadNumericId == JOB_THREADHANDLE_IS_FREE )
        {
...
void ThreadPool::RemoveActiveId(int id)
{
  try
    ...
    for(ThreadIdHandlePairingContainerType::iterator it =
          m_ThreadIdHandlePairingQueue.begin(),
          end = m_ThreadIdHandlePairingQueue.end();
        it != end; ++it)
      {
      if( it->second.m_ThreadNumericId == id )
        {
        // set this to JOB_THREADHANDLE_IS_DONE so that now the thread
        // is in wait state
        it->second.m_ThreadNumericId = JOB_THREADHANDLE_IS_FREE;
        break;
        }
      }
...
ThreadPool::ThreadSemaphorePair* ThreadPool::GetSemaphoreForThreadWait(ThreadProcessIdType threadHandle)
{
  ...
  MutexLockHolder<SimpleFastMutexLock> threadStructMutexHolder(m_ThreadIdHandlePairingQueueMutex);
  ThreadSemHandlePairingQueueType::iterator it = m_ThreadSemHandlePairingForWaitQueue.find(threadHandle);
  if(it == m_ThreadSemHandlePairingQueue.end())
    {
...
ThreadPool::ThreadSemaphorePair* ThreadPool::GetSemaphoreForThread(ThreadProcessIdType threadHandle){
  MutexLockHolder<SimpleFastMutexLock> threadStructMutexHolder(m_ThreadIdHandlePairingQueueMutex);
  ThreadSemHandlePairingQueueType::iterator it = m_ThreadSemHandlePairingQueue.find(threadHandle);
  if(it == m_ThreadSemHandlePairingQueue.end())
    {
    itkExceptionMacro(<< "Error occured finding semaphore for thread handle "<<threadHandle);
    }
...
ThreadJob ThreadPool::FetchWork(ThreadProcessIdType threadHandle)
{
  ...
  ThreadIdHandlePairingContainerType::const_iterator tpIter;
  if((tpIter = m_ThreadIdHandlePairingQueue.find(threadHandle)) ==
     m_ThreadIdHandlePairingQueue.end())
    {
    itkExceptionMacro(<< "Can't find thread with handle "
                      << threadHandle);
    }

  workId = tpIter->second.m_ThreadNumericId;
...
ThreadPool::ThreadProcessStatusPair *
ThreadPool
::FindThreadToRun()
{
  ThreadProcessStatusPair *returnValue(ITK_NULLPTR);
  //
  ...
  for(unsigned j = 0; j < 2 && returnValue == ITK_NULLPTR; ++j)
    {
    for(ThreadIdHandlePairingContainerType::iterator tpIter = m_ThreadIdHandlePairingQueue.begin(),
          end = m_ThreadIdHandlePairingQueue.end();
        tpIter != end; ++tpIter)
      {
  ...
  try
    {
      ...
      ThreadProcessStatusPair *threadToRun = this->FindThreadToRun();
