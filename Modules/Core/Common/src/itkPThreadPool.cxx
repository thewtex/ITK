...
void ThreadPool::RemoveActiveId(int id)
{
  try
    {
    itkDebugMacro(
      << std::endl << "ActiveThreadids size : " << m_ActiveThreadIds.size() << ". Removing id " <<
      id << std::endl );
      {
      MutexLockHolder<SimpleFastMutexLock> mutexHolderAct(m_ActiveThreadMutex);
      int                                  index = -1;
...
void * ThreadPool::ThreadExecute(void *param)
{
  ...
  while( !pThreadPool->m_ScheduleForDestruction )
    ...
    try
      ...
      currentPThreadJob.Ptid = pthread_self();
      itkDebugMacro(
        << "Fetched workerthread (executing now) with ptid: " << currentPThreadJob.Ptid << std::endl );
