...
void ThreadPool::RemoveActiveId(int id)
{
  try
    ...
    bool foundToDelete = false;
      ...
      int pos = find(m_JobIds.begin(), m_JobIds.end(), id) - m_JobIds.begin();
        {
...
pthread_t ThreadPool::GetThreadHandleForJob(int jobId){
  ...
  int pos = find(m_JobIds.begin(), m_JobIds.end(), jobId) - m_JobIds.begin();
    {
    MutexLockHolder<SimpleFastMutexLock> threadIdMutexHolder(m_ThreadIdAccessMutex);
...
int ThreadPool::GetJobIdForThreadHandle(pthread_t pt){
  ...
  int pos = find(m_ThreadIds.begin(), m_ThreadIds.end(), pt) - m_ThreadIds.begin();
    {
    MutexLockHolder<SimpleFastMutexLock> threadIdMutexHolder(m_ThreadIdAccessMutex);
    return m_JobIds[pos];
    }
