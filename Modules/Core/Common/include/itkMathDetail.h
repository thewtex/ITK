...
inline int32_t
FloatDistance( TFloat x1, TFloat x2 )
{
  typedef Detail::FloatRepresentation< TFloat > FloatRepresentationType;
  ...
  float maxAbsoluteDifference )
{
  ...
  const int32_t intDifference = FloatDistance< float >( x1, x2 );
  ...
  double maxAbsoluteDifference )
{
  ...
  const int32_t intDifference = FloatDistance< double >( x1, x2 );
  if ( intDifference <= maxUlps )
    {
    return true;
    }
