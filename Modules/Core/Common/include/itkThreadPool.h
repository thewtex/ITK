...
public:
      ...
      ThreadProcessHandle(tph)
    {
        ...
        sName << "/Semaphore"<<tph<<"_"<<count<<"_"<<m_SemCount++;
        ...
        retry = (Semaphore == SEM_FAILED) ? true : false;
        count ++;
        //time to stop
        ...
        {
        std::cout<<std::endl<<"Semaphore with name "<<sName.str().c_str()<<"cannot be initialized. Error code SEM_FAILED = "<<Semaphore;
        exit(-1);
