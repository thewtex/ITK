                   ...
                   DomainType& subDomain ) const
{
  ...
  typename DomainType::IteratorType it = completeDomain.m_Begin;
  ThreadIdType count = NumericTraits< ThreadIdType >::Zero;
  for( it = completeDomain.m_Begin; it != completeDomain.m_End; ++it )
    {
    ++count;
    }
  ...
  if (threadId == maxThreadIdUsed)
    ...
    subDomain.m_End = completeDomain.m_End;
    }
