                   ...
                   DomainType& subDomain ) const
{
  ...
  typename DomainType::IteratorType it = completeDomain.GetBegin();
  ThreadIdType count = NumericTraits< ThreadIdType >::Zero;
  for( it = completeDomain.GetBegin(); it != completeDomain.GetEnd(); ++it )
    {
    ++count;
    }
  ...
  if (threadId == maxThreadIdUsed)
    ...
    subDomain.SetEnd( completeDomain.GetEnd() );
    }
