...
void ImageAlgorithm::DispatchedCopy( const ImageType *inImage, ImageType *outImage,
                                     ...
                                     TrueType )
{
  ...
  if ( inRegion.GetSize()[0] != outRegion.GetSize()[0] )
    {
    ImageAlgorithm::DispatchedCopy<ImageType, ImageType>( inImage, outImage, inRegion, outRegion );
    }

  // Get the number of bytes of each pixel in the buffer.
  size_t PixelSize = ImageAlgorithm::PixelSize<ImageType>::Get( inImage );
