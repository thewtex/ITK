...
inline bool ExposeMetaData(const MetaDataDictionary & Dictionary, const std::string key, T & outval)
{
  ...
  MetaDataObjectBase::ConstPointer baseObjectSmartPointer = Dictionary[key];
  ...
  if ( strcmp( typeid( T ).name(), baseObjectSmartPointer->GetMetaDataObjectTypeName() ) != 0 )
    ...
    const MetaDataObjectBase::ConstPointer tempConstPointer = Dictionary[key];
    if ( tempConstPointer.IsNotNull() )
      {
      MetaDataObject< T > const * const TempMetaDataObject = dynamic_cast< MetaDataObject< T > const * >( tempConstPointer.GetPointer() );
