 ...
 */
inline bool
FloatAlmostEqual( float x1, float x2, long maxUlps = 4 )
{
  ...
  const long intDiff = abs( static_cast< long >( x1Representation.asInt - x2Representation.asInt ));
  if ( intDiff <= maxUlps )
    {
    return true;
    }
...
inline bool
FloatAlmostEqual( double x1, double x2, long maxUlps = 4 )
{
  union FloatRepresentation
    {
    double asFloat;
