 ...
 *
 *=========================================================================*/
#ifndef __itkImageFastLinearConstIterator_h
#define __itkImageFastLinearConstIterator_h

#include "itkImageIterator.h"

namespace itk
{
/** \class ImageFastLinearConstIterator
 * \brief A multi-dimensional iterator templated over image type that walks a
 * region of pixels.
 *
 * The itk::ImageFastLinearConstIterator is optimized...
 *
 * \code
 *
 *      it = it.Begin();
 ...
 *
 * \wiki
 * \wikiexample{Iterators/ImageFastLinearConstIterator,Iterate over a region of an image (without write access)}
 * \endwiki
 */
template< typename TImage >
class ITK_EXPORT ImageFastLinearConstIterator:public ImageConstIterator< TImage >
{
public:
  /** Standard class typedef. */
  typedef ImageFastLinearConstIterator     Self;
  ...
  itkTypeMacro(ImageFastLinearConstIterator, ImageIterator);

  /** Default constructor. Needed since we provide a cast constructor. */
  ImageFastLinearConstIterator():ImageConstIterator< TImage >()
  {
    m_SpanBeginOffset = 0;
    m_SpanEndOffset = 0;
  }
  ...
  ImageFastLinearConstIterator(const ImageType *ptr,
   ...
   * ImageFastLinearConstIterator. Many routines return an ImageIterator, but for a
   * particular task, you may want an ImageFastLinearConstIterator.  Rather than
   * provide overloaded APIs that return different types of Iterators, itk
   * returns ImageIterators and uses constructors to cast from an
   * ImageIterator to a ImageFastLinearConstIterator. */
  ImageFastLinearConstIterator(const ImageIterator< TImage > & it)
  {
   ...
   * returns ImageIterators and uses constructors to cast from an
   * ImageIterator to a ImageFastLinearConstIterator. */
  ImageFastLinearConstIterator(const ImageConstIterator< TImage > & it)
  {
    this->ImageConstIterator< TImage >::operator=(it);
  ...
  inline bool IsAtEndOfLine(void) {return true;}
...
private:
  ...
  void Decrement(); // go back in a direction other than the fastest
                    // moving
...
#include "itkImageFastLinearConstIterator.hxx"
#endif
