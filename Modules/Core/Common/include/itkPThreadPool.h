...
public:
  ...
  pthread_t GetThreadHandleForJob(int jobId);

  int GetJobIdForThreadHandle(pthread_t pt);
...
private:
  ...
  static SimpleFastMutexLock m_JobThreadMapAccessMutex;
  static SimpleFastMutexLock m_ThreadJobMapAccessMutex;

  std::vector<pthread_t> m_ThreadHandles;
  //holds a map showing which job executed on which thread
  std::map<int, pthread_t> m_JobIdThreadIdMap;
  //holds a map showing which thread is associated with which job
  std::map <pthread_t, int> m_ThreadIdJobIdMap;
