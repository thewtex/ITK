...
int itkSSEAdaptorTest(int , char* [] )
{
  ...
  for (int n = 1; n <= 5; n++)
  {
    for (int rptIx = 0; rptIx < 5; rptIx++)
    {
      ...
      int result = sse.CalculateNPairs(vnlSparseA, vnlSparseB, evCount, 1.0e-14, 0, smallest, magnitude, 10000, sigma);
      if (result != 0)
      {
      ...
      if (!SSEType::Calculate(A, B, evCount,
                              ...
                              smallest, magnitude, 1.0e-14, 10000, sigma))
      {
