...
public:
  ...
  private:
    ...
    virtual void ThreadedExecution( const DomainType& subdomain,
      ...
      this->m_DomainInThreadedExecution[threadId][0] = *(subdomain.m_Begin);
      DomainType::IteratorType it = subdomain.m_End;
      --it;
  ...
  const IteratorRangeDomainThreaderAssociate::TestDomainThreader::DomainType & fullDomain )
{
  ...
  for( itk::ThreadIdType i = 0; i < domainThreader->GetNumberOfThreadsUsed(); ++i )
    ...
    if( i == 0 && subRange[0] != *(fullDomain.m_Begin) )
        {
        std::cerr << "Error: subRange[0][0] should be " << *(fullDomain.m_Begin)
                  << ", but it's " << subRange[0] << ".";
        return EXIT_FAILURE;
        }
    /* Check that we got the end of the range */
    BorderValuesType::const_iterator fullIt = fullDomain.m_End;
    --fullIt;
  ...
  IteratorRangeDomainThreaderAssociate::TestDomainThreader::DomainType & fullDomain )
{
  ...
  getIteratorFromIndex( start, container, fullDomain.m_Begin );
  getIteratorFromIndex( end, container, fullDomain.m_End );
}
...
int itkThreadedIteratorRangePartitionerTest(int, char* [])
{
  ...
  fullDomain.m_Begin = container.begin();
  fullDomain.m_End   = container.end();
