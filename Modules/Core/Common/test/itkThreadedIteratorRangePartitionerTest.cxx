...
public:
  ...
  private:
    ...
    virtual void ThreadedExecution( const DomainType& subdomain,
      ...
      this->m_DomainInThreadedExecution[threadId][0] = *(subdomain.GetBegin());
      DomainType::IteratorType it = subdomain.GetEnd();
      --it;
  ...
  const IteratorRangeDomainThreaderAssociate::TestDomainThreader::DomainType & fullDomain )
{
  ...
  for( itk::ThreadIdType i = 0; i < domainThreader->GetNumberOfThreadsUsed(); ++i )
    ...
    if( i == 0 && subRange[0] != *(fullDomain.GetBegin()) )
        {
        std::cerr << "Error: subRange[0][0] should be " << *(fullDomain.GetBegin())
                  << ", but it's " << subRange[0] << ".";
        return EXIT_FAILURE;
        }
    /* Check that we got the end of the range */
    BorderValuesType::const_iterator fullIt = fullDomain.GetEnd();
    --fullIt;
  ...
  IteratorRangeDomainThreaderAssociate::TestDomainThreader::DomainType & fullDomain )
{
  ...
  IteratorRangeDomainThreaderAssociate::TestDomainThreader::DomainType::IteratorType it;
  getIteratorFromIndex( start, container, it );
  fullDomain.SetBegin( it );
  getIteratorFromIndex( end, container, it );
  fullDomain.SetEnd( it );
}
...
int itkThreadedIteratorRangePartitionerTest(int, char* [])
{
  ...
  fullDomain.SetBegin( container.begin() );
  fullDomain.SetEnd( container.end() );
