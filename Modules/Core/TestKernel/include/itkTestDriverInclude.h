...
typedef std::pair< char *, std::string > HashPairType;
...
void usage()
{
  ...
  std::cerr << "      This option can be used several times." << std::endl;
  std::cerr << std::endl;
  std::cerr << "  --hash TEST md5hash" << std::endl;
...
int ProcessArguments(int *ac, ArgumentStringType *av, ProcessedOutputType * processedOutput = NULL )
{
  ...
  while ( i < *ac )
  {
      ...
      *ac -= 3;
     ...
     else if ( !skip && strcmp((*av)[i], "--hash") == 0 )
      {
      if ( i + 2 >= *ac )
        {
        usage();
      ...
      hashTestList.push_back( HashPairType((*av)[i + 1], std::string( (*av)[i + 2] ) ) );
      (*av) += 3;
      *ac -= 3;
      }
...
std::string ComputeHash( const char *testImageFilename )
{
  ...
  typedef itk::HashImageFilter<ImageType> HashFilterType;
