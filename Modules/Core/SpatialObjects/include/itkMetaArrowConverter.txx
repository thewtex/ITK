...
::MetaObjectToSpatialObject( const MetaObjectType *mo )
{
  ...
  ArrowSpatialObjectPointer spatialObject = ArrowSpatialObjectType::New();
  ...
  spatialObject->GetProperty()->SetName( metaArrow->Name() );
  spatialObject->SetId( metaArrow->ID() );
  spatialObject->SetParentId( metaArrow->ParentID() );
  spatialObject->GetProperty()->SetRed(metaArrow->Color()[0]);
  spatialObject->GetProperty()->SetGreen(metaArrow->Color()[1]);
  spatialObject->GetProperty()->SetBlue(metaArrow->Color()[2]);
  spatialObject->GetProperty()->SetAlpha(metaArrow->Color()[3]);

  return SpatialObjectPointer(spatialObject.GetPointer());
}
...
::SpatialObjectToMetaObject(const SpatialObjectType *spatialObject)
{
  ...
  MetaArrow *mo = new MetaArrow(NDimensions);
