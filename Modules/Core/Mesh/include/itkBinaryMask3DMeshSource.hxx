...
::AddCells(unsigned char celltype, unsigned char celltran, int index)
{
  ...
  switch ( (int)celltype )
    ...
    case 1:
      ...
      output->SetCell(m_NumberOfCells, insertCell);
      output->SetCellData(m_NumberOfCells, 0.0);
      m_NumberOfCells++;
      break;
    case 2:
      tp[0] = 4;
           ...
           IdentifierType **currentframetmp)
{
  ...
  for ( i = 0; i < 3; i++ )
    ...
    if ( m_AvailableNodes[nodesid[i]] != 0 )
      ...
      input->TransformContinuousIndexToPhysicalPoint(indTemp,new_p);
      output->SetPoint(m_NumberOfNodes, new_p);
