...
private:
  ...
  /** Specialized versions of EvaluteAtIndex() method to handle scalar or vector
    pixel types.*/
  ...
  /** Specialized versions of EvaluteAtContinuousIndex() method to handle scalar
    or vector pixel types.*/
  template< typename Type >
  ...
  /** Specialized versions of Evalute() method to handle scalar or vector pixel
    types.*/
  // NOTE: for some unknown reason, making these methods inline (as those above
  // are inlined) makes them run *slower*.
  template< typename Type >
