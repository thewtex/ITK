...
::EvaluateAtIndexSpecialized(const IndexType & index, OutputType & orientedDerivative, OutputTypeSpecializationStructType<OutputType>) const
{
  ...
  for ( unsigned int dim = 0; dim < MaxDims; dim++ )
    {
    // bounds checking
    // this checks if the index is either on the boundary in which case a
    // derivative of zero will be returned for the dimension, or if the
    // point is out of bounds in which case a derivative of 0 is returned
    // for all dimension.
    if ( index[dim] < start[dim] + 1 || index[dim] > ( start[dim] + static_cast< OffsetValueType >( size[dim] ) - 2 ) )
      {
      // check if the point is actually out of bounds
      if( index[dim] < start[dim] || index[dim] > ( start[dim] + static_cast< OffsetValueType >( size[dim] ) - 1 ) )
        {
        orientedDerivative.Fill( 0.0 );
        return;
        }
      else
        {
