...
int itkCentralDifferenceImageFunctionTest(int, char* [] )
{
  ...
  // test that a point just at a boundary will yield 0 derivative
  // this is because it will be on the boundary in index space
  point.Fill( 8.0 );
  ...
  // it should yield a deriviative of 0 along the out-of-bounds dimension
  point[0] = 8.0;
  point[1] = -1.0;
  function->SetUseImageDirection( false );
  origDerivative = function->Evaluate( point);
  function->SetUseImageDirection( true );
  ...
  if( outOfBoundsDerivative[0] != origDerivative[1] || outOfBoundsDerivative[1] != 0 )
    {
  ...
  if( outOfBoundsDerivative[0] != 0 || outOfBoundsDerivative[1] != origDerivative[0] )
    {
