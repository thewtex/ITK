...
int itkCentralDifferenceImageFunctionOnVectorTestRun( )
{
  ...
  zeroVector.Fill( itk::NumericTraits<typename OutputType::ValueType>::ZeroValue() );

  if ( function->IsInsideBuffer( index ) )
    {
    std::cout << "ERROR: Expected index: " << index << " to be outside the BufferedRegion." << std::endl;
    result = EXIT_FAILURE;
    }

  if( ! IsEqual<OutputType>( indexOutput, zeroVector ) )
    {
    std::cout << "ERROR: Expected zeroVector return for indexOutput, but got: " << indexOutput << std::endl;
    result = EXIT_FAILURE;
    }

  // test EvaluateAtIndex out-of-bounds in a single dimension
  index.Fill( 8 );
  index[1] = size[1];
  OutputType halfOutOutput = function->EvaluateAtIndex( index );
  std::cout << "Index: " << index << " Derivative: " << indexOutput << std::endl;

  if( ! IsEqual<OutputType>( halfOutOutput, zeroVector ) )
    {
    std::cout << "ERROR: Index: Expected zero for return for halfOutOutput, but got: " << halfOutOutput << std::endl;
    result = EXIT_FAILURE;
    }

  // test EvaluateAtContinuousIndex out-of-bounds in a single dimension
  cindex.Fill( 8 );
  cindex[1] = size[1];
  halfOutOutput = function->EvaluateAtContinuousIndex( cindex );
  std::cout << "Continuous Index: " << cindex << " Derivative: " << indexOutput << std::endl;

  if( ! IsEqual<OutputType>( halfOutOutput, zeroVector ) )
    {
    std::cout << "ERROR: ContinousIndex: Expected zero for return for halfOutOutput, but got: " << halfOutOutput << std::endl;
    result = EXIT_FAILURE;
    }

  // test Evaluate out-of-bounds in a single dimension
  point.Fill( 8 );
  point[1] = size[1];
  pointOutput = function->Evaluate( cindex );
  std::cout << "Point: " << point << " Derivative: " << pointOutput << std::endl;

  if( ! IsEqual<OutputType>( pointOutput, zeroVector ) )
    {
    std::cout << "ERROR: Point: Expected zero for return for pointOutput, but got: " << pointOutput << std::endl;
    result = EXIT_FAILURE;
    }

  // test fully out-of-bounds with continuous index
  cindex.Fill( size[0] );
  continuousIndexOutput = function->EvaluateAtContinuousIndex( cindex );
  std::cout << "ContinuousIndex: " << cindex << " Derivative: " << continuousIndexOutput << std::endl;

  if( ! IsEqual<OutputType>( continuousIndexOutput, zeroVector ) )
    {
    std::cout << "ERROR: Expected zeroVector return for continuousIndexOutput, but got: " << indexOutput << std::endl;
    result = EXIT_FAILURE;
    }

  // test fully out-of-bounds with a point
  point.Fill( size[0] );
  pointOutput = function->Evaluate( point );
  std::cout << "Point: " << point << " Derivative: " << pointOutput << std::endl;
  ...
  if( ! IsEqual<OutputType>( pointOutput, zeroVector ) )
    {
    std::cout << "ERROR: Expected zeroVector return for pointOutput, but got: " << indexOutput << std::endl;
    result = EXIT_FAILURE;
    }
