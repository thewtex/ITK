...
int itkVTKVisualize2DLevelSetAsElevationMapTest( int , char* [] )
{
  ...
  viewer->SetConstant( 0.1 );
  viewer->Update();

  if ( viewer->GetConstant() != 0.1)
    {
    return EXIT_FAILURE;
    }

  vtkPolyData* ls_surface = viewer->GetElevationMapMesh();
  vtkCellArray*          cells = ls_surface->GetPolys();

  vtkSmartPointer<vtkMassProperties> massProperty = vtkSmartPointer<vtkMassProperties>::New();
  massProperty->SetInputData( ls_surface );
