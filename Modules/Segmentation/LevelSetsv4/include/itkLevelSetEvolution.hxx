...
::RunOneIteration()
{
  ...
  typename TermContainerType::TermIterator termIt = termContainer->TermBegin();
  if( termIt == termContainer->TermEnd() )
    {
...
::Update()
{
  ...
  typename TermContainerType::TermIterator termIt = Equation0->TermBegin();
  if( termIt == Equation0->TermEnd() )
    {
    itkGenericExceptionMacro( << "TermContainer is empty" );
    }
