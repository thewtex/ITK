...
{
  InputImageConstPointer input = this->GetInput();
  const InputImageSizeType size = input->GetLargestPossibleRegion().GetSize();
  ...
  OutputImagePixelType currentOutputPixel;
  InputImageIndexType startIdx;
  ...
  while( !iIt.IsAtEnd() )
    ...
    if ( ( !inputPixel.empty() ) && ( outputPixel == NumericTraits<OutputImagePixelType>::Zero ) )
      ...
      while( !subRegionInputConsistent )
        {
        this->ConsistencyCheck( subRegionInputConsistent, subRegion );
        }

      m_LevelSetMap[segmentId] = LevelSetDomain( subRegion, inputPixel );
