...
void GenerateSolution( char* ImagePath,
                       typename itk::MalcolmSparseLevelSetBase< VDimension >::Pointer Solution )
{
  ...
  try
    ...
    {
    std::cout <<"In GenerateSolution" << std::endl;
  ...
  i2lm->SetBackgroundValue( LevelSetType::PlusOneLayer() );
  i2lm->Update();
  ...
  LayerType layer = Solution->GetLayer( LevelSetType::ZeroLayer() );
...
int itkBinaryImageToMalcolmSparseLevelSetAdaptorTest( int argc, char* argv[] )
{
  ...
  GenerateSolution< Dimension >( argv[3], solution );

  bool comparison =
      itk::CompareMalcolmLevelSets< Dimension >( sparseLevelSet,
                                                 solution );
