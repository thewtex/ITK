...
The following commands are used to generate the suggested symbols on Unix-like systems:

nm lib/libitktiff-*.a 2> /dev/null | grep " T \| D \| R " | awk '{ print substr($3, 2); }' | awk '!/itk_/ { print }' | sed 's \(.*\) \1\ itk_\1 ' | sed 's/^/#define /'
...
The third command prints out only the third column which is the symbol name (the first 2 columns are location and type) and removes the leading underscore on every line
...
The developer will then need to *MANUALLLY* add the symbols to the list below. Please try to keep the symbols in a sorted order (you can use sort utility in Unix)
*/
...
// Section containing symbols which are unique to Windows
#ifdef _WIN32
...
#define _TIFFrealloc itk__TIFFealloc
