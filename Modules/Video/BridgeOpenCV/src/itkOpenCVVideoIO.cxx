...
OpenCVVideoIO::ReadImageInformation()
{
  ...
  if (this->m_ReadType == ReadFromFile)
    ...
    // Query the frame and get frame total (since this is only valid for reading
    // from files)
...
OpenCVVideoIO::Read(void *buffer)
{
  ...
  //
  // Note: This will advance to the next frame in the stream, so
  // SetNextFrameToRead
  //       doesn't need to be called before Read is called again unless you want
  // to
  //       skip to a different location. Be warned, though. SetNextFrameToRead
  // can
