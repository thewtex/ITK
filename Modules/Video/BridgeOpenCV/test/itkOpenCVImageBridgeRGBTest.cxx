...
int itkOpenCVImageBridgeTestTemplatedRGB(char* argv0, char* argv1)
{
  ...
  double iplDiff1 = cvNorm(outIpl, inIpl);
  if (iplDiff1 != 0.0)
    {
    std::cerr << "outIpl != inIpl with difference =  "
      << iplDiff1 << std::endl;
    }

  for (int i=0; i< outIpl->imageSize; i++)
    {
    double localDifference = outIpl->imageData[i]-  dataConvertedInIpl->imageData[i];
    if (localDifference != 0.0)
        {
        std::cerr<< "imageData["<< i <<"] is different: "<< int(outIpl->imageData[i])
           <<" vs " << int(dataConvertedInIpl->imageData[i]) <<std::endl;
        }
    }
