  ...
  // BGR output
    ...
    if (inChannels == 3 )
      ...
      for (size_t r=0;r<temp->height;r++)
        for (size_t c=0;c<temp->width;c++)
          {
          size_t pos = r*temp->widthStep;
          typename ImageType::IndexType pixelIndex;
          pixelIndex[0] = r;
          pixelIndex[1] = c;
          typename ImageType::PixelType  pixel = in->GetPixel(pixelIndex);
          for (unsigned int i=0; i< inChannels; i++)
            temp->imageData[pos + c* inChannels + i ] = pixel[i];
          }
/*    size_t lineLength = typeSize * temp->width* temp->nChannels;

    for (int i=0;i<temp->height;i++)
       {//not suer memcpy is the right way
        memcpy( &(reinterpret_cast<ValueType *>(temp->imageData)[i*temp->widthStep]),
                &(const_cast<InputPixelType*>(in->GetBufferPointer())[i*lineLength]),
                 lineLength);
       std::cerr<<" "<<i<<" "<<std::endl;
       }
