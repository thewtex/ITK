...
TemporalProcessObject::GenerateInputRequestedTemporalRegion()
{
  ...
  // This should always be a whole number because of
  // EnlargeOutputRequestedTemporalRegion
...
TemporalProcessObject::UpdateOutputInformation()
{
  ...
  OffsetValueType outputStart = Math::Ceil<OffsetValueType>(
      (double)inputLargestRegion.GetFrameStart() / (double)m_FrameSkipPerOutput)
    + m_InputStencilCurrentFrameIndex;
...
TemporalProcessObject::GenerateData()
{
  ...
  for (SizeValueType i = 0; i < inputTemporalRegionRequests.size(); ++i)
    {
    // If we have an input, set the requested region and make sure its data is
    // ready
...
TemporalProcessObject::SplitRequestedTemporalRegion()
{
  ...
  // Set up the requested input temporal region set (TODO: NOT PROPERLY HANDLING
  // REAL TIME!!!!!!!!)
