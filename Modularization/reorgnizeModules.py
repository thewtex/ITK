...
for line in  open('./Manifest.txt','r'):
 if line[0] != '#':
    ...
    newModuleName =  "ITK-"
    newSubdirName = newGroupName+'/'
    if (groupName != 'utilities'):
       for word in mWords[1:]:
        newModuleName = newModuleName + string.capitalize(word)
        newSubdirName = newSubdirName + string.capitalize(word)
    else:
        newModuleName = newModuleName + string.upper(mWords[1])
        newSubdirName = newSubdirName + string.upper(mWords[1])
    newline = string.ljust(itkFileName,90)+' '+string.ljust(newGroupName,20)+' '+ string.ljust(newModuleName,30)+' '+newSubdirName+'\n'
    o.write(newline)
 else:
    o.write(line)
