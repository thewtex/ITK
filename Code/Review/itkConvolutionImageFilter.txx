...
namespace itk
{
template< class TInputImage, class TOutputImage, class TKernelImage >
ConvolutionImageFilter< TInputImage, TOutputImage, TKernelImage >
...
::ThreadedGenerateData(const OutputRegionType & outputRegionForThread, int threadId)
{
  ...
  if ( this->GetNormalize() )
    ...
    if ( sum != NumericTraits< KernelPixelType >::One )
      {
      scalingFactor = NumericTraits< KernelPixelType >::One / sum;
      }
    }
  ...
  NeighborhoodInnerProduct< InputImageType, KernelPixelType, KernelPixelType > innerProduct;
