...
=========================================================================*/
#ifndef __itkRToITKImage_h
#define __itkRToITKImage_h
 ...
 * \brief Import data from an R table into an itk::Image
 *
 * RToITKImageImport provides a mechanism for importing data into an itk::Image.
 * RToITKImageImport is an image source, so it behaves like any other pipeline
 * object.
 *
...
class ITK_EXPORT RToITKImageImport:
...
public:
  ...
  itkTypeMacro(RToITKImageImport,ImageSource);
  ...
  void SetImportPointer(SEXP ptr);
...
private:
  RToITKImageImport(const RToITKImageImport &); //purposely not implemented
  void operator=(const RToITKImageImport&); //purposely not implemented
...
#include "itkRToITKImageImport.txx"
#endif

#endif
irtual void EnlargeOutputRequestedRegion(DataObject *output);
