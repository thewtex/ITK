 ...
 *
 *=========================================================================*/
#ifndef __itkResampleImageFilterTransduceFunction_txx
#define __itkResampleImageFilterTransduceFunction_txx
...
template< class TInputImage,
          ...
          class TPixelValueSpatialTransformationTransduceFunction >
ResampleImageFilterTransduceFunction< TInputImage, TOutputImage, TInterpolatorPrecisionType,
  TPixelValueSpatialTransformationTransduceFunction >
::ResampleImageFilterTransduceFunction()
{
  ...
  m_TransduceFunction =
    PixelValueSpatialTransformationTransduceFunctionType::New();
  m_ThreaderTransduceFunction.clear();
}
...
::BeforeThreadedGenerateData()
{
  ...
  m_ThreaderTransduceFunction.resize(numberOfThreads -1);
  for ( unsigned int ithread = 0; ithread < numberOfThreads - 1; ++ithread )
    ...
    /** Set the fixed parameters first. Some transforms have parameters which
     *  depend on the values of the fixed parameters. For instance, the
     *  BSplineDeformableTransform checks the grid size (part of the fixed
     *  parameters) before setting the parameters. */
    ...
    //Create copies of the transduce function for each thread
    LightObject::Pointer anotherTransduce =
      this->m_TransduceFunction->CreateAnother();
    PixelValueSpatialTransformationTransduceFunctionType *transduceCopy =
      static_cast< PixelValueSpatialTransformationTransduceFunctionType * >
      ( anotherTransduce.GetPointer() );
    this->m_ThreaderTransduceFunction[ithread] = transduceCopy;
    //Assign the transform to each copy of the transduce function.
    this->m_ThreaderTransduceFunction[ithread]->SetTransform( transformCopy );
    this->m_ThreaderTransduceFunction[ithread]->Prepare();
    }

    //Assign the transform to the transduce function member variable
    this->m_TransduceFunction->SetTransform( this->m_Transform );
    this->m_TransduceFunction->Prepare();
                                ...
                                int threadId)
{
  ...
  PixelValueSpatialTransformationTransduceFunctionType *transduceFunction;

  if ( threadId > 0 )
    {
    transform = this->m_ThreaderTransform[threadId - 1];
    transduceFunction = this->m_ThreaderTransduceFunction[threadId - 1];
    }
  else
    {
    transform = this->m_Transform;
    transduceFunction = this->m_TransduceFunction;
    }
  ...
  if ( m_InterpolatorIsBSpline )
    ...
    while ( !outIt.IsAtEnd() )
      ...
      if ( m_Interpolator->IsInsideBuffer(inputIndex) )
        {
        PixelType        pixval;
        OutputType value = m_BSplineInterpolator
                                 ->EvaluateAtContinuousIndex(inputIndex,
                                                             threadId);
        // Modify using the transduce function. Note that this assumes
        // that m_TransduceFunction has been defined using input and output
        // value types that match OutputType.
        value = (*transduceFunction)( inputPoint, value);
    ...
    while ( !outIt.IsAtEnd() )
      ...
      if ( m_Interpolator->IsInsideBuffer(inputIndex) )
        {
        PixelType        pixval;
        OutputType value = m_LinearInterpolator
                                 ->EvaluateAtContinuousIndex(inputIndex);
        // Modify using the transduce function.
        value = (*transduceFunction)( inputPoint, value);
    ...
    while ( !outIt.IsAtEnd() )
      ...
      if ( m_Interpolator->IsInsideBuffer(inputIndex) )
        {
        PixelType        pixval;
        OutputType value = m_Interpolator
                                 ->EvaluateAtContinuousIndex(inputIndex);
        // Modify using the transduce function.
        value = (*transduceFunction)( inputPoint, value);
...
template< class TInputImage,
          ...
          class TInterpolatorPrecisionType,
          class TPixelValueSpatialTransformationTransduceFunction >
const typename ResampleImageFilterTransduceFunction< TInputImage,
                                    TOutputImage,
