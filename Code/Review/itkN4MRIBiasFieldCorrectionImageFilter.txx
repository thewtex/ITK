...
::GenerateData()
{
  ...
  logInputImage->SetOrigin( this->GetInput()->GetOrigin() );
  logInputImage->SetSpacing( this->GetInput()->GetSpacing() );
  logInputImage->SetRegions( this->GetInput()->GetRequestedRegion() );
  logInputImage->SetDirection( this->GetInput()->GetDirection() );
  logInputImage->Allocate();
  logInputImage->FillBuffer( 0 );

  /**
   * Calculate the log input image.
   */
  ImageRegionIteratorWithIndex<RealImageType> ItL( logInputImage,
                                                   logInputImage->GetRequestedRegion() );

  ImageRegionConstIterator<InputImageType> ItI( this->GetInput(),
                                                this->GetInput()->GetRequestedRegion() );

  for( ItL.GoToBegin(), ItI.GoToBegin(); !ItI.IsAtEnd(); ++ItL, ++ItI )
    {
    if( ( !this->GetMaskImage() ||
          this->GetMaskImage()->GetPixel( ItL.GetIndex() ) == this->m_MaskLabel )
        && ( !this->GetConfidenceImage() ||
             this->GetConfidenceImage()->GetPixel( ItL.GetIndex() ) > 0.0 ) )
      {
      if( ItI.Get() > NumericTraits<typename InputImageType::PixelType>::Zero )
        {
        ItL.Set( vcl_log( static_cast<typename
                                      RealImageType::PixelType>( ItI.Get() ) ) );
        }
      }
    }
