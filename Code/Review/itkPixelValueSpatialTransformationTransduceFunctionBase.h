 ...
 * It functions as an identity transducer.
 *
 ...
 *
 * The user can set a transform via SetTransform, to be used in the ()-operator
 * as needed. By default, the transform is set to NULL. After setting the
 * transform
 * and before use, the Prepare method must be called to perform any required
 ...
 * is made for each thread. Still, the ()-operator must not call any
 * methods of the assigned transform that aren't thread-safe, when used in a
 * multi-threded build. In ResampleImageFilter, the transducer is copied for
 * each thread in use, but the same transform object is assigned to each copy
 * to avoid overhead of copying transforms.
 * Transform::TransformPoint *is* expected to be thread-safe.
 *
