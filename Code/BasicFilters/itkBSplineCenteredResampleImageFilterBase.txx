...
{
  typename TInputImage::IndexValueType i1, i2;

  typename TInputImage::SizeValueType outK, inK;
  typename TInputImage::SizeValueType outTraverseSize = inTraverseSize / 2;

  inTraverseSize = outTraverseSize * 2; // ensures that an even number is used.
  typename TInputImage::SizeValueType inModK;  // number for modulus math of in
  ...
  for ( inK = 0; inK < inTraverseSize; inK++ )
    ...
    for ( int i = 1; i < this->m_GSize; i++ )
      ...
      if ( i1 < 0 )
        ...
        if ( i1 >= (typename TInputImage::IndexValueType)inTraverseSize )
          {
          i1 = inModK - i1 - 1L;
          }
        }
      if ( i2 >= (typename TInputImage::IndexValueType)inTraverseSize ) // originally (i2 > (inTraverseSize - 1)
                                        // )
        {
        i2 = i2 % inModK;
        if ( i2 >= (typename TInputImage::IndexValueType)inTraverseSize )
          {
          i2 = inModK - i2 - 1L;
          }
        }
                ...
                unsigned int inTraverseSize, ProgressReporter & progress)
{
  ...
  typename TInputImage::IndexValueType          inK;
  typename TInputImage::SizeValueType outTraverseSize = inTraverseSize * 2;
  //inTraverseSize = outTraverseSize/2;  // ensures that an even number is used.
  typename TInputImage::IndexValueType inModK; // number for modulus math of in

  inModK = outTraverseSize;
  typename TInputImage::IndexValueType k0 = ( this->m_HSize / 2 ) * 2 - 1L;

  double outVal, outVal2;

  for ( inK = 0; inK < (typename TInputImage::IndexValueType)inTraverseSize; inK++ )
    {
    //outK = inK * 2L;
    ...
    for ( int k = 2; k < this->m_HSize; k += 2 )
      ...
      if ( i1 < 0 )
        {                                                      // provide
                                                               // correct border
                                                               // condition
        i1 = ( 2L * (typename TInputImage::IndexValueType)inTraverseSize - 1L - i1 ) % inModK; // pseudo mirror
                                                               // image
        if ( i1 >= (typename TInputImage::IndexValueType)inTraverseSize )
          {
    ...
    ++out;
    outVal2 = 0;
    for ( typename TInputImage::IndexValueType k = -k0; k < this->m_HSize; k += 2L )
      {
      typename TInputImage::IndexValueType kk = vcl_abs( static_cast< int >( k ) );
      i1 = inK + ( k + 1L ) / 2L;
      if ( i1 < 0L )
        {
        i1 = ( 2 * inTraverseSize - 1 - i1 ) % inModK;
        if ( i1 > (typename TInputImage::IndexValueType)inTraverseSize - 1L )
          {
          i1 = outTraverseSize - i1 - 1L;
          }
        }
  ...
  --out;
  for ( typename TInputImage::IndexValueType j = outTraverseSize - 1; j > 0L; j-- )
    {
