...
::FindCellId(unsigned long id1, unsigned long id2, unsigned long id3)
{
  ...
  std::set< typename InputMeshType::PointIdentifier >           cells3 =  this->GetInput(0)->GetCellLinks()->GetElement(id3);
  std::set< typename InputMeshType::PointIdentifier >::iterator cellIt = cells1.begin();

  while ( cellIt != cells1.end() )
    {
    std::set< typename InputMeshType::PointIdentifier >::iterator found2 = std::find(cells2.begin(), cells2.end(), *cellIt);
    std::set< typename InputMeshType::PointIdentifier >::iterator found3 = std::find(cells3.begin(), cells3.end(), *cellIt);
