                       ...
                       int threadId)
{
  ...
  FixedArray< typename TInputImage::OffsetValueType, 2 *ImageDimension > offset;
  ...
  for ( i = 0; i < ImageDimension; i++ )
    {
    offset[i] = -1 * static_cast< typename TInputImage::OffsetValueType >( bit.GetStride(i) );
  ...
  for ( fit = faceList.begin(); fit != faceList.end(); ++fit )
    ...
    const typename TInputImage::SizeValueType center = bit.Size() / 2;
    while ( !bit.IsAtEnd() )
      {
