...
::ConstructActiveLayer()
{
  ...
  for ( outputIt.GoToBegin(); !outputIt.IsAtEnd(); ++outputIt )
    ...
    if ( outputIt.GetCenterPixel() == m_ValueZero )
      ...
      for ( i = 0; i < ImageDimension; i++ )
        {
        if ( center_index[i] + static_cast< typename OutputImageType::IndexValueType >( m_NumberOfLayers ) >= ( upperBounds[i] - 1 )
             || center_index[i] - static_cast< typename OutputImageType::IndexValueType >( m_NumberOfLayers ) <= lowerBounds[i] )
          {
          m_BoundsCheckingActive = true;
          }
        }
...
::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "m_UpdateBuffer: size=" << static_cast< size_t >( m_UpdateBuffer.size() )
     << " capacity=" << static_cast< size_t >( m_UpdateBuffer.capacity() ) << std::endl;
}
} // end namespace itk
