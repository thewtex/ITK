 ...
 *
 *=========================================================================*/
#ifndef __itkPlainImageFileReader_h
#define __itkPlainImageFileReader_h
 ...
 * to actually do the reading of the data. The user must create the
 * ImageIO class corresponding to the file format to be read, and
 * must connect it to this class via the SetImageIO() method, before
 * attempting to read a file. This class will not attempt to use the
 * ImageIO Factory mechanism for creating an ImageIO. If you want to
 * use the Factory mechanism, you should use the ImageFileReader class
 * instead of the PlainImageFileReader class.
 *
 * Note that the PlainImageFileReader class reads data from a single file;
 * if you wish to read data from a series of files use ImageSeriesReader.
 *
 * TOutputImage is the type expected by the external users of the
 * filter. If data stored in the file is stored in a different pixel type
 ...
 *
 */
/** \class PlainImageFileReader
 */
template< class TOutputImage,
          class ConvertPixelTraits = DefaultConvertPixelTraits<
            ITK_TYPENAME TOutputImage::IOPixelType > >
class ITK_EXPORT PlainImageFileReader:public ImageSource< TOutputImage >
{
public:
  /** Standard class typedefs. */
  typedef PlainImageFileReader             Self;
  ...
  itkTypeMacro(PlainImageFileReader, ImageSource);
   ...
   * output than it was requested to produce. PlainImageFileReader cannot
   * currently read a portion of an image (since the ImageIO objects
   * cannot read a portion of an image), so the PlainImageFileReader must
...
protected:
  ...
  bool m_UseStreaming;
private:
  PlainImageFileReader(const Self &); //purposely not implemented
...
#include "itkPlainImageFileReader.txx"
#endif

#endif // __itkPlainImageFileReader_h
