 ...
 *
 *=========================================================================*/
#ifndef __itkPlainImageFileWriter_h
#define __itkPlainImageFileWriter_h
...
/** \class PlainImageFileWriter
 * \brief Writes image data to a single file.
 *
 * PlainImageFileWriter writes its input data to a single output file.
 * PlainImageFileWriter interfaces with an ImageIO class to write out the
 * data. If you wish to write data into a series of files (e.g., a
 * slice per file) use ImageSeriesWriter.
 *
 * This class does not use the ImageIO Factory mechanism for creating an
 * ImageIO. The user must manually instantiate the ImageIO object and must
 * associate it with the PlainImageFileWriter before attempting to write a
 * file.
 *
 * If you want to use the ImageIO factory mechanism, you should use the
 * ImageFileWriter class instead of the PlainImageFileWriter class.
 *
 * \sa ImageFileWriter
...
class ITK_EXPORT PlainImageFileWriter:public ProcessObject
{
public:
  /** Standard class typedefs. */
  typedef PlainImageFileWriter            Self;
  ...
  itkTypeMacro(PlainImageFileWriter, ProcessObject);
...
protected:
  ...
  /** Give derived classes access to the FactorySpecifiedImageIO flag */
  itkSetMacro( FactorySpecifiedImageIO, bool );
  itkGetMacro( FactorySpecifiedImageIO, bool );

private:
  PlainImageFileWriter(const Self &); //purposely not implemented
...
#include "itkPlainImageFileWriter.txx"
#endif

#endif // __itkPlainImageFileWriter_h
