...
ElementWrapperPointerInterface< TElementWrapperPointer, TElementIdentifier >::
GetLocation(const ElementWrapperPointerType & element)
{
...
ElementIdentifierType
MinPriorityQueueElementWrapper< TElement, TElementPriority, TElementIdentifier >::
GetLocation(const MinPriorityQueueElementWrapper & element)
{
...
Pop()
{
  m_Interface.SetLocation(GetElementAtLocation(0),
                          m_ElementNotFound);
  if ( this->Size() > 1 )
    {
    SetElementAtLocation( 0, GetElementAtLocation( this->Size() - 1 ) );
    this->pop_back();
    UpdateDownTree(0);
    }
  else
...
Update(Element element)
{
  ...
  if( location != m_ElementNotFound )
    ...
    if( location >= this->Size() )
      {
      itkGenericExceptionMacro( <<"Element location is out of range" );
      }
